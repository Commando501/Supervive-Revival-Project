...
- **S156-B (2026-09-08):** ★★★★★★ **BACK-TO-BACK `AdjustHealth` WORKS — SHOT_2 commits identically
  ...
  **★ Floor-firing test flown same day (2026-09-08, `docs/s156b-postshots2-floor600-flight1-FLOOR_FIRES_AS_DESIGNED.md`): [M] safety floor branch fires cleanly at the designed point.** Variant `-postshots2-floor600` (RAW `ca54361966e71afd`, N=2 + `-DKBFPOSTSHOT_FLOORBITS=0x44160000u`=600.0f). SHOT_2 fires normally (750→500, floor 600 above pre 750 skipped, within-run control), SHOT_3 hits **FLOOR_REACHED at PRECHECK** (preHP=500 ≤ floorHP=600, no CallNativeGuarded, no arithmetic) → **POSTSHOTS_COMPLETE fired=1 requested=2 aborted=yes reason=floor finalHP=500.00**. Clean disarm, no FK-32. First-try F1 success. Zero source changes; single build.ps1 variant. Three-flight within-family series now closed for KBFPOSTSHOTS 0..2 range: `-postshots1` (N=1, floor silent, finalHP=500), `-postshots2` (N=2, floor silent, finalHP=250), `-postshots2-floor600` (N=2, floor fires at SHOT_3, finalHP=500). Every within-family delta is single-variable; every predicted receipt hit exact.
  **★ Positive-delta heal test flown same day (2026-09-08, `docs/s156b-heal100-flight1-POSITIVE_DELTA_HEAL_WORKS.md`): [M] AdjustHealth accepts positive delta and heals correctly.** Variant `-postshots1-heal100` (RAW `0b9573215c85f511`, N=1 + `-DKBFPOSTSHOT_DELTABITS=0x42C80000u`=+100.0f). SHOT_2 heal: 750 → 850 exact (`deltaBits=42C80000 deltaHP=100.00 observedDeltaHP=+100.00 arithmeticOK=yes`), no cap interaction (well below MaxHealth 1000), floor silent (pre 750 > 300). Clean disarm, no FK-32. First-try F1 success. Source delta: added `KBFPOSTSHOT_DELTABITS` knob with default = prior hardcoded value; **all 4 prior gate variants BYTE-IDENTICAL** to their committed hashes (knob is truly additive). [M] AdjustHealth polarity-symmetric within tested range (same wrapper/handles/dispatch path handles both signs with exact arithmetic). [M] Both Base and Current updated for heal too (`postBits=44548000/44548000` both slots=850). MaxHealth cap (would need overshoot delta e.g. +500), mixed damage+heal in one arm, fractional deltas, non-Health attributes, and multi-hero all remain OPEN.
  **★ MaxHealth cap test flown same day (2026-09-08, `docs/s156b-heal500-flight1-MAXHEALTH_CAP_ENFORCED.md`): [M] AdjustHealth SILENTLY CLAMPS at MaxHealth.** Variant `-postshots1-heal500` (RAW `b0e29a434eabd87d`, N=1 + `-DKBFPOSTSHOT_DELTABITS=0x43FA0000u`=+500.0f). SHOT_2 requested +500 on 750 HP hero → game APPLIED only +250 (exact delta to reach MaxHealth 1000). `postHP=1000.00 observedDeltaHP=+250.00 arithmeticOK=NO postBits=447A0000/447A0000` — clamp lands at exactly the seeded MaxHealth 1000 bit-pattern. `identityStable=yes issues=0x0` — silent clamp, no fault, no error state, no return-value signal. `POSTSHOTS_COMPLETE fired=1 requested=1 aborted=no reason=none` — arm treats clamp as non-fatal (correct: shot committed, only magnitude differed). First-try F1 success. Zero source changes. **[M] Both Base and Current clamped together (both slots go to 1000)**. **[M] Clamp uses the same MaxHealth slot S156 writes to** (`+0x80 = LokiAttributeSetHealth::MaxHealth`; no hidden cache elsewhere). **[M] `arithmeticOK=NO` is a working signal** (first non-`yes` result, arm correctly captured the delta mismatch). **[M] Cheat-detection implication**: silent-clamp via AdjustHealth is more anti-cheat-invariant-friendly than direct writes (which would leave `Current > MaxHealth`). Six-flight postshots-family series now spans damage/multi-shot/floor/heal/cap; full-coverage empirical validation of KBFPOSTSHOTS + KBFPOSTSHOT_FLOORBITS + KBFPOSTSHOT_DELTABITS knob triad. Undershoot cap, mixed damage+heal, non-Health attributes, multi-hero remain OPEN.
  **★★★★★★★ S189-BOT BOT PLAYABILITY FLOWN 2026-09-09, `docs/s189-bot-botplay-floor-RESULT.md`: [M] THREE first-in-project results across 2 successful flights (+3 FK-31 staging deaths).** Design: workflow `wf_9b8a8446-b39` (6 Understand + 1 Design + 3 adversarial Verify + 1 Adjudicate). Arm `botplay-floor-tinykick` RAW `ec5317d71bc7bf90`, verify_dll PASS. **[M] (1) A floor-spawned `SpawnAIFromClass` LokiBot comes up `MOVE_Walking` at possess time** — `[SNP] BOT-before MovementMode@0x231=1`, GravityScale 1.0, Velocity (0,0,0), at the spawn point (1356,-409,90.15); `[RCPT] GETTER_BOT_AFTER_G mode=1 MaxSpeed=43F38000(487)`. **This SETTLES the S136-vs-S138 hover contradiction: LokiBots hovered at (600,0,13240) since S138 because they were spawned in the AIR — `SetDefaultMovementMode`'s `FindFloor` (engine `0x35F9B30`/`0x35E01D0`, real, not Loki-overridden) succeeds over the tutorial floor and never reverts to Falling. The air-vs-floor spawn is the whole difference; the bot's AI and mover were never broken.** **[M] (2) The bot walked ~2135 uu HORIZONTAL under its own AI wander, no kick** — `[SNP] BOT loc (-288.5,954.7,-541.0) moved 2227.685 uu` at the first worker sample, `|Acc|=50000` rotating through headings, AIM=1, `FALLBACK NOT NEEDED ... AI-driven`, `fallbackFired=0`; then it walked off the island edge and fell (S140 behavior). **[M] (3) THE `+0xF08`-ONLY K1-WIRED PLAYER WALKED UNDER WASD AT EXACTLY ITS SEEDED 487 uu/s** — the design's biggest `[S]` (UNFLOWN wiring) resolved: `+0xF00` left as the live KWIREGAS ASC (`[GASX] PLR ... MASKED OUT` for +0xF00/+0xF10, `written 1/3`), only `+0xF08` -> CDO set; W drove `AccY=-50000`, slid off the ledge to the floor (Z 386→90.15, Mode 3→1), walked with `|Vxy|` pinned at **487.000** (690 Walking samples, in-shim `[SNP] PLR atCap=BAND delta=0.000e+00`); the K1-broke-input signature did NOT fire. ⇒ **THE FIRST VERSUS-AI STATE: a WASD-driven player hero and an AI-wander-driven enemy hero both alive and MOBILE, both Walking on the tutorial floor, in the SAME world.** ⚠ They walked a few seconds apart, NOT the identical instant (bot left the floor ~4s before the player's fall+slide arrived); strict simultaneity (P8) not captured. ⚠ **P4 partial**: the bot's peak-velocity-==-487-bit-exact was NOT densely sampled — the on-floor Walking window from (1356,-409) is only ~2s, faster than the worker sampler (starts ~10s post-arm) or the external auto-discovery watcher (~2-4s GUObjectArray sweep). `[M]` for the wire (`[RCPT]=487`), `[I,strong]` for "walked at 487" (2135uu in ~6s ≈ 355 uu/s avg). The fix (a direct-address `--pawn/--cmc` watcher started at the `[SNP] BOT` marker line during the game-thread hold) is coded and ready; the 3 F3 attempts all died in FK-31 staging. ⚠⚠ **GATE DIGEST CORRECTION [M this session]: the CLAUDE.md-recorded `botai 5e47c13cf7f0a158` / `gasattr 2fcc2536e21f18e3` / `gasattr-ctrl 4465ebc4d7168c03` / `play 9bc10a4552c596e1` DO NOT reproduce from HEAD `4e2f609` and were STALE BEFORE this session (source drifted at the S176 VEH-cleanup `4824c77`, which touches the shared Worker-exit path in every variant). TODAY'S HEAD before-state: `botai b620e0ff3279e673`, `gasattr 29f0fa4efffd01da`, `gasattr-ctrl 4c0ede610baa4629`, `play 31af7667355f39d1`, `armk 3ff3ba9f77ae1438`, `axisab 01593c0f8f73f1ae`, `sentinel_big 8347a0df17354239`; `-seedmax b47d1bfd04e44921` and `-mv-play 87c764b6d235b7a3` DO still reproduce.** New rules R-S189-BOT-a..d banked (floor-spawn -> Walking; +0xF08-only is WASD-drivable; a floor bot walks off in ~2s so sample it via the `[SNP] BOT` marker during the hold; a 1 uu/s seed breaks the post-gravity hover in ~5s vs 115s). Rollback is one knob per variant (default 0 = flown behaviour); `botplay-floor/-tinykick/-noai/-5th` + `botwalk` are the new variants (all distinct, no new `--dupes` group). ★★★★★ **AND THE STRATEGIC FINDING THAT REFRAMES THE WHOLE VS-AI MISSION [M]: the bot's combat AI is ENTIRELY CLIENT-SIDE — read `docs/bot-ai-client-side-settled.md`.** The full brain ships as Blueprint in `/Game/Loki/Characters/AI/Bots/`: `BT_HeroBots` (behaviour tree), `BB_HeroBots` (blackboard with `TargetEnemy`/`TargetEnemy_LastSeen`/`AggressionState`/`MaxCombatDistance`/`CurrentSpell`/`BotGoalType` — a COMBAT schema, not a wander stub), services (`BTService_PickSpell`, `ApplyAimError`, `UpdateTargetCombatDistance`), tasks (`BTT_CastSpell`, `BTT_Dodge`, `BTT_InitiateMoveTo`), EQS (`EQS_FindCombatPosition_Bot`, `EQC_TargetEnemyContext`), difficulty GEs (`GE_TrivialBot`..`GE_NormalBot`), and **26/26 REAL `ALokiBotController` native helpers** (`GenerateAimError`, `GetUsableSpells`, `GetExactSpell`, `TryToDash`, ...); S137 F4 measured it instantiating **21 live tree nodes + 15 blackboard keys** on our standalone client. ⇒ **"the bot fights back" needs NO AI authored or recovered — only the two walls the mission already tracks: WALL E (hostility/teams, stripped) so `TargetEnemy` can be populated, and WALL P (ability activation) so `BTT_CastSpell` runs.** What we observed this session (native `Tick` random wander) is the fallback when the tree has no target; the tutorial world has no hostile target (no teams) so the combat tree never engages. ⚠ [I,strong] not measured-in-combat: no session has watched `BT_HeroBots` run a combat cycle (no target ever existed; S137 had no `LogBehaviorTree` positive control). The one offline step that would map it: `bpdump`/`rawfile`-extract `BT_HeroBots` + its `BTService_*`/`BTT_*` nodes. ★ **THE CURRENT FRONTIER HANDOFF IS `docs/next-session-prompt-s190-versus-ai.md`** — the predicate reduces to walls E+P; the damage mechanism (S156-B AdjustHealth) and bot spawn/movement/AI are all [M]; Track D (spawn a floor bot, seed its ASC, AdjustHealth it to death) proves the E+damage half end-to-end and isolates WALL P as the sole remaining critical-path item.

  **★★★★★★ S190 TRACK D FLOWN 2026-09-09 — A SPAWNED ENEMY BOT DIES FROM THE GAME'S OWN AdjustHealth. Read `docs/s190-trackd-bot-killed.md`.** [M] `RM_BOTSPAWN` + `KBFTRACKD`: floor-spawned LokiBot (ARM D) → **build a REAL ASC on the bot** (`AddComponentByClass(LokiAbilitySystemComponent)` + `K2_InitStats` for both sets + `InitAbilityActorInfo`, replicating `EnsureHeroAffiliatedCarrier` minus the failing carrier spawn) → seed Health+MaxHealth=1000 → S153-validated `AdjustHealth`. **Non-lethal `DMG_APPLIED_NONLETHAL` 1000→750 (`443B8000`, exact); lethal `BOT_KILLED` 1000→0 (`00000000/00000000`, cleanZero, arithmeticOK, faulted=no), reproduced twice** (re-inject + clean staged probe). **[M] AdjustHealth silently clamps at the 0 floor** (requested −5000, applied −1000, `arithmeticOK=yes`) — same clamp semantic as S156-B's MaxHealth. **[M] the zero-crossing did NOT crash the game** — the kill fired at ~200s uptime, game ran ~90s more, then died of ambient FK-32 (`0x0000DEAD` at 290.8s/293.4s, no crashpad; every PID this session died ~285–293s uptime regardless of shim activity — decoupled from the kill). ⚠ SCOPE: Track D **BYPASSES WALL E** (AdjustHealth on the bot's OWN ASC, not a player ability through hostility). ⇒ **WALL P (durable player ability cast, the S158/S175–S177 FK-32-protector problem) is now the sole remaining critical-path item for the full predicate.** Key findings: (a) the roadmap's `botfight-kill 0x31` is DEGENERATE (K_WIREBOT refuses + seeds nothing); (b) `WireAbilitySystem` cannot wire a bot ASC (no HeroAffiliated carrier, spawn fails/crashes S80); (c) the ARM-G-borrowed CDO ASC is NOT AdjustHealth-applicable (empty `SpawnedAttributes` → GetSet null-deref FAULT; null `AbilityActorInfo` → InitAAI FAULT → no AvatarActor → AdjustHealth no-ops); (d) THE FIX is a real `AddComponentByClass` ASC. Rules R-S190-a/b/c banked. Variants `trackd-kill`/`trackd-kill-nonlethal` (RM_BOTSPAWN, KBSPSARMS=0x20, KBFTRACKD=1; all code behind `#if KBFTRACKD`, no `#else`; gates `botai b620e0ff3279e673`/`play 31af7667355f39d1`/`-seedmax b47d1bfd04e44921`/`-postshots1 1925e9e80646a4fb` BYTE-IDENTICAL). Cost: 8 launches (2 clean stages, 4 FK-31 staging deaths — a cluster, + 2 aged-PID FK-32s). Evidence markers preserved at `docs/s190-trackd-BOT_KILLED-staged-marker.txt`.

  **★★★★★ S190 RECON (2026-09-09, ZERO-LAUNCH) — THE BOT'S CAST IS A PLAYER CAST [M]: WALL P IS THE SINGLE LEVERAGE POINT.** `bpdump BTT_CastSpell` (`tools/extractor/out/bpdump_ExecuteUbergraph_BTT_CastSpell.txt`) resolves the bot's spell-cast bytecode: `GetLokiAbilitySystemComponentFromActor` → `EX_Context{ obj:<ASC>, EX_FinalFunction TryActivateAbilityByClass(SpellClass) }`, guarded by `Spell_IsActive`+`IsValidClass`, with `FinishSpell`/`WaitForSpellCompletion` (full cast-and-wait) and `ForcedFollowupSpell` combo logic. `BTService_PickSpell` (BTService_BlueprintBase: `FilterByType`/`GetDistanceToEnemyCapsule`/`HasType`) writes `CurrentSpell`. ⇒ **[M] `ULokiAbilitySystemComponent::TryActivateAbilityByClass` is the IDENTICAL activation entry point for a player cast (DoBotFight K_ACTIVATE) AND the bot's `BTT_CastSpell`.** Fixing WALL P (durable `TryActivateAbilityByClass`) unlocks BOTH the player casting and the bot fighting back. Upgrades `docs/bot-ai-client-side-settled.md`'s [I,strong] → [M] (see its 2026-09-09 UPDATE). Full bot combat loop mapped: perceive enemy (WALL E, stripped) → PickSpell (BP + real native helpers, S153) → CastSpell via TryActivateAbilityByClass (WALL P). No AI missing; walls E+P are the whole job, P shared with the player. ⇒ **WALL P is now the highest-leverage next target for the full predicate.**

  **★★★★★★★ S191 E4A FLIGHT 1 FLOWN 2026-09-10 — R-S191-h CONFIRMED LIVE + TryActivateAbilityByInputID NON-LETHAL [M]. Read `docs/s191-e4a-flight1-RESULT.md`.** First-try flight success (no launches lost to FK-31). Six [M] results: (1) **R-S191-h upgraded from offline [I,strong] to LIVE [M]**: `GetAbilityByInputID(Ability1=3)` returns `0x0(NULL)` despite our K_GRANT'd spec being at `Handle=1, InputID @+0x24=3` in the ASC's Items TArray — proves plain native `GiveAbility` populates Items but NOT the separate InputID→spec map that `GetAbilityByInputID`/`TryActivateAbilityByInputID` consult; (2) **[M] `ASC.TryActivateAbilityByInputID(3)` at `base+0x52971A0` returned `false` CLEANLY** (fault=no, no 0xDEAD) — **a distinct WALL-P surface point from S147/S158's `TryActivateAbilityByClass`-fatal shim-originated call**; the InputID activation surface is safe to call from a shim; (3) minion Health unchanged (100→100 exact); (4) all E4 hardening fixes fired correctly (rotation-read Yaw -49.7° forward (0.647,-0.763), AvatarActor BOUND, SpawnedAttributes registered); (5) clean disarm `restored=17563 of 17563`, game alive at 237s past the FK-32 window; (6) my offline design's disjunction message (`activation refused: either InputID map missing our spec, or a downstream gate`) allowed both branches, and the map-null branch is now confirmed as ONE of the causes. **⇒ Option B is the definitive path forward**: swap K_GRANT from plain `GiveAbility` to `BP_AuthGiveAbilityWithInputID(GS_Ronin_LMB_Selector_C, Level=1, LokiAbilityInputID=Ability1(3), hero, InputIDPriority=0)` at impl `0x5294B50` (S153 REAL). Post-Option-B the same E4A probe re-fires to test whether the InputID map is now populated AND whether `TryActivateAbilityByInputID(3)` returns `true` + drops minion HP — if both, the E4 predicate is met from a direct shim call, bypassing OS input. Rules R-S191-h1 (upgraded)/R-S191-j (TryActivateAbilityByInputID non-lethal from shim)/R-S191-k (three-datapoint single-flight diagnostic pattern) banked. Cost: **1 launch, 0 losses.** Regression gates BYTE-IDENTICAL: `botai b620e0ff3279e673`, verify_dll PASS, --dupes clean, `e4-a RAW=2d7efa4cde249f2e`.

  **★★★★★★★★ S191 LMB INPUT BINDING PATHWAY MAPPED 2026-09-10 (offline, zero launches). Read `docs/s191-lmb-input-binding-mapped.md`.** The E4 flight 3 negative on the natural LMB cast is now REFRAMED with a full offline trace: [M] the game uses LEGACY `UInputComponent::BindAction` via `K2Node_InputActionEvent` for gameplay (NOT `BindAbilityActivationToInputComponent`; the runtime binary has 0 hits for that string vs 11 hits for `EnhancedInputAction` which is used only for UI/menus). [M] the LMB→ability dispatch chain: `LMB K2Node_InputAction → BP handler → EX_CallMulticastDelegate LMBPressed(OnLMBPressed) → Comp_PlayerController_Abilities.HandleAbilityActivation(byte 3)` where `3 = LokiAbilityInputID::Ability1` (verified — the ByteConst value in the ubergraph is literally 3, corroborating my Ability1=3 inference). `HandleAbilityActivation` is a BP-event wrapper that calls into the abilities-component ubergraph at index 591 → check `Can Upgrade Ability` + `IsLevelAbilityModifierPressed` for level-up, else → `Self.TryActivateAbility(AbilityID)` (LocalVirtualFunction) OR `ASC.TryActivateAbilityByInputID(AbilityID)`. [M] THREE reflected activation primitives all graded REAL by S153: `ULokiAbilitySystemComponent::TryActivateAbilityByInputID(LokiAbilityInputID)` @ `0x52971A0`, `ULokiAbilitySystemComponent::BP_AuthGiveAbilityWithInputID(TSubclassOf, int Level, LokiAbilityInputID, UObject Source, int Priority=0)` @ `0x5294B50`, `ULokiAbilitySystemComponent::TryActivateAbilityBySourceObject(UObject, bool, TSubclassOf)` @ `0x5297230`. **[I,strong] the E4 flight 3 null root cause is architectural**: GAS has TWO parallel registration structures — the Items TArray (populated by plain `GiveAbility`) and the InputID→spec map (populated by `BP_AuthGiveAbilityWithInputID`); our K_GRANT populates only Items, so `GetAbilityByInputID(3)` may return null and `TryActivateAbilityByInputID(3)` may fail-silent. 4 concrete follow-up options (A/B/C/D) documented, all flyable in one arm as gated sub-steps: A=`ASC.TryActivateAbilityByInputID(3)` (cheapest single call, tests InputID-map presence), B=swap K_GRANT to `BP_AuthGiveAbilityWithInputID` (proper InputID-aware grant), C=`Comp_PlayerController_Abilities.HandleAbilityActivation(3)` (exact BP-dispatcher mimic), D=RPM-watch abilities-component timestamped state around a natural LMB tap to disambiguate "OS input isn't reaching the BP handler" from "OS input reaches BP handler but the spec resolution fails". Rules R-S191-g/h/i banked. Cost: 0 launches (pure `bpdump` + asdump investigation over ~11 KLOC of BP bytecode).

  **★★★★★★★★★★★★★★★★★★★★ S191 E4T ARC FLOWN 2026-09-10 — WALL P DEFINITIVELY DEFEATED, STRICT E4 KILL PREDICATE MET FOR BOTH ABILITY3 (MiniDash) AND ABILITY1 (LMB / Ronin's LightAttack1) [M]. THE WALL THAT BLOCKED EVERY PROJECT SESSION'S SHIM-DRIVEN ABILITY ACTIVATION FOR ~40+ SESSIONS IS GONE. Read `docs/s191-e4t-a9-flight1-RESULT.md` and `docs/s191-e4t-lmb-a9-flight1-RESULT.md`.** 8 [M] flights on ONE PID over ~70 min uptime, 0 FK-32 across 6 sequential injects. One workflow (`wf_82319383-e54`, 5 agents, 2M tokens) identified the phase byte offset via UHT FPropertyParams; a second workflow (`wf_23186e2b-083`, 5 agents, 1.8M tokens) identified the LMB damage sub-ability. The E4 predicate ("player LMB casts an ability that damages and kills a hostile enemy") is fully MET — every real Loki system responds correctly, indistinguishable from a natural cast (damage GE fires, impact GameplayCue fires, Ronin's Execute-Highlight passive fires on low-HP target, Selector combo-advance GE fires). ★ **THE MECHANISM (9 bytes + 3 ProcessEvent calls, generalizes across `ULokiGameplaySpell` family):** (1) K_GRANT the target ability class at any InputID (`GS_Ronin_MiniDash_Charges_C` or `GS_Ronin_LightAttack1_C` — the KBFABIL FindObjExact fallback resolves any class name); (2) poke `[inst+0xF58] = 2` (1 byte — `ULokiGameplaySpell::CurrentStateSpecPhaseType` u8 enum: 0=Inactive 1=Targeting 2=Warmup 3=Channeling 4=Invoke 5=Winddown 6=Count — Dash gate accepts phase ∈ {2,3,4}); (3) poke `[inst+0xF48] = *(inst+0xF20)` (8 bytes — `CurrentPhase UObject*` set to the always-populated WarmupPhase sub-object at +0xF20); (4) `ProcessEvent(Invoke, zero-parm)` — Dash/attack body starts; (5) Sleep(300ms); (6) Loop 6× `ProcessEvent(<HitVerb>, [NULL, &target, zeroed 240B])` with 250ms intervals — each applies ~17-18 damage (`DashHit` for MiniDash, `MeleeHit` for LightAttack1); (7) restore both pokes. **~1.75 seconds to kill 100 HP.** ★ **THE 5 PHASE SUB-OBJECT POINTERS on every `ULokiGameplaySpell` descendant [M]:** `+0xF18 TargetingPhase`, `+0xF20 WarmupPhase`, `+0xF28 ChannelingPhase`, `+0xF30 InvokePhase`, `+0xF38 WinddownPhase`. Also on the class: `+0xC18 ReplicatedPhaseData` (12 B server-authoritative), `+0xF48 CurrentPhase UObject*`, `+0xF50 CurrentPhaseEffectHandle`, `+0xF58 CurrentStateSpecPhaseType u8`, `+0xF59 LastStateSpecPhaseType u8`. Identity/stability controls unchanged from prior work: `+0x3A8 CurrentActorInfo`, `+0x3B0 CurrentSpecHandle`, `+0x408 bIsActive`. ★★★ **THE 8-FLIGHT ARC (single launch, launch-2 of session; launch-1 died to FK-31 in staging):** **E4T-A3** — ProcessEvent bypass reaches MiniDash BP body; Loki.log witnesses `Attempted to dash outside of Invoke, Channeling and Warmup phase` refusal (proves BP body runs and phase-check is the gate). **E4T-A4** — `BP_AuthGiveAbilityWithInputID`-style compound chain: `BP_AuthBeginWarmup` adds a real `GE_GlideSuppressd_CallerScale_C` Warmup GE (proves BP method executed real code), but the phase-check flag stays 0 (the state-write decoupled from the GE side effect). **E4T-A5** — adding `BP_AuthBeginChanneling` between Warmup and Invoke doesn't help (chain of zero-parm BP events doesn't advance the phase-check flag). **E4T-A6** — workflow `wf_82319383-e54` identifies `+0xF58 CurrentStateSpecPhaseType` via UHT FPropertyParams; poke works (readback OK, persists through Invoke) but `Attempted to dash...` refusal STILL fires — gate is COMPOUND, not phase-byte-alone. **E4T-A7 [WALL P DEFEATED]** — compound poke (phase byte + `+0xF48 CurrentPhase` set to `+0xF20 WarmupSubobj`) SATISFIES the gate; Loki.log witnesses full Dash body execution: `Pre Dash Start → GameplayCue GC_GlobalImpulse_DASH → Post Dash Start → Dash Ended` over 4.3s, NO refusal. **E4T-A8 [E4 DAMAGE HALF MET]** — after compound poke + Invoke, wait 300ms, call `ProcessEvent(DashHit, [NULL, &minion, zeroed 240B])`; minion HP `100.00 → 81.78` (`-18.22`). Loki.log: `GE_Ronin_MiniDash_Damage_C` fires + `GC_Ronin_MiniDash_Impact` cue + `GE_Ult_PreventMultiHits_C` defensive GE. **E4T-A9 [E4 KILL HALF MET]** — loop 6× `DashHit` at 250ms intervals; each applies ~18 damage (PreventMultiHits GE does NOT block subsequent hits because it's a per-hitbox overlap tracker and we bypass the hitbox entirely); minion `100.00 → 0.00 (-100.00)` in ~1.75s. **E4T-LMB-A9 [STRICT LMB E4 MET, first-try]** — same mechanism against `GS_Ronin_LightAttack1_C` (the LMB damage sub-ability inside `GS_Ronin_LMB_Selector_C`); KBFABIL FindObjExact fallback resolves the class by name (bypasses hero-slot lookup), KBFE4T_HITVERB="MeleeHit" swaps the damage verb; minion `100.00 → 0.00 (-100.00)` in ~1.75s via Ronin's REAL LMB damage chain (`GE_Ronin_LightAttack_Damage_C` + `GC_Ronin_LightAttack_Impact` + `GC_Ronin_Execute_Highlight_C` low-HP passive + `GE_Ronin_Selector_Attack_2_C` combo advance). ★★★★ **RULES BANKED (R-S191-E4T-*):** the ProcessEvent-slot-78 bypass reaches BP body on any `ULokiGameplaySpell` instance; BP `BP_Auth*` events run real code but do NOT set the state-tracker flag (the flag is decoupled from the GE side effects); the Dash-refusal gate is COMPOUND (phase byte AND CurrentPhase ptr, not phase-byte-alone); the 5 phase sub-object pointers are CDO-configured at construction (present on every instance without prior activation); zero-parm Invoke DOES NOT damage the target — a separate damage-verb call with target parm is needed; the mechanism GENERALIZES across `ULokiGameplaySpell` descendants with identical offsets; only the damage-verb name changes (`DashHit` MiniDash / `MeleeHit` LightAttack1 / `MeleeHitSweetSpot` for bonus / `ApplyGameplayEffectSpecToActorWithOnHit` for base-class ASC-side fallback per workflow's PATH B). ★★★★★ **BLAST RADIUS:** every `ULokiGameplaySpell`-derived damage ability in the game should be killable via this pattern with only the class name and hit-verb name as per-ability variables. Bot combat (S190 recon: `BTT_CastSpell` uses `TryActivateAbilityByClass` — the SAME wall) should now also work: with WALL P defeated, the bot's own cast path unblocks in principle (a shim wrapper that pre-pokes on the bot's ASC before each `TryActivateAbilityByClass`). ⚠⚠ **SCOPE:** this is a DIAGNOSTIC arm, not a shipping fix — it pokes live ability-instance state and drives ProcessEvent from a shim. To ship for real gameplay, the pokes need to be applied per-cast on natural input firing, which needs either (a) a durable ProcessEvent hook that intercepts before the phase-check reads or (b) a mechanism to make the game's own state machine set +0xF58/+0xF48 correctly on natural cast (the ORIGINAL question, i.e. WHY the natural path leaves these fields 0 — still open). ⚠ **HOSTILITY (WALL E):** the tutorial minion (`BP_Minion_KaijuAxeLeader_Stagger_Tutorial_C`) is `TeamID=100` vs player teamIndex −1 (per S191 E4 F3), and the Ronin Execute-Highlight cue fires on it — confirming WALL E FAVORABLE for this minion class. This applies to shim damage via `DashHit`/`MeleeHit`. General-hostility (arbitrary NPC + player being enemies) may need more work. **Session cost:** 2 launches (1 lost to FK-31 staging, 1 endured 70+ min with 6 sequential injects), 8 flights, 6 commits pushed to `origin/dedicated-server-stub`: `84291e1..b5a476c`. ★★★★★ **THE E4 PREDICATE — WHICH HAS BEEN THE ARC-FRONTIER SINCE ~S143 — IS FULLY MET. WALL P IS NO LONGER A WALL.**

  **★★★★★★★★ S191 E4 FLIGHT 3 FLOWN 2026-09-10 — A HOSTILE ENEMY MINION KILLED ON THE TUTORIAL WORLD [M]. Read `docs/s191-e4-flight3-RESULT.md`.** ⚠⚠ **PARTIALLY SUPERSEDED BY THE S191 E4T ARC ABOVE — the "BLOCKED ONLY on LMB→Ability1 input binding" conclusion is REFUTED. WALL P's phase-check gate was the real blocker; E4T-LMB-A9's compound-poke + MeleeHit mechanism damaged and killed the minion via Ronin's LMB damage sub-ability (bypassing Selector). Input binding is a separate question (about routing OS mouse events → InputID → spec) and is still open, but it is NOT what blocked the E4 predicate.** Flight 3 of 3 (2 lost to FK-31 staging deaths, ~27% cluster rate as documented). Chain, all [M]: player ASC wired + Ability1 (`GS_Ronin_LMB_Selector_C`) GRANTED at InputID 3 (Items 0→1, handle=1) + K_ALIVE + K_GASATTR 6/6 → spawn **`BP_Minion_KaijuAxeLeader_Stagger_Tutorial_C`** (a residency-candidate-list picked it after `BP_Minion_TargetBot_TTK_C` was measured NOT LOADED; 6 hostile tutorial minions census'd RESIDENT) 200uu forward along the hero's ACTUAL rotation (Yaw −167.5° → forward `(-0.976,-0.217)` — not the hardcoded −Y) → minion's constructor-built ASC InitAAI'd (`AvatarActor@+0x410==minion (BOUND)` — my hardening fix caught the S190 F3/F4 no-op case) → HealthSet resolved + `setInSpawned=yes` (constructor-registered, no DxAppend needed) → seed 100/100 exact → **KE4DIRECTGE `preHP=100.00 postHP=0.00 postBits=00000000/00000000 faulted=no *** DIRECT-WRITE DAMAGE APPLIED ***`** — the game's own AdjustHealth path via the S153-validated wrapperExact (thunk `base+0x5294270` → tail → impl `0x5516610`) killed the minion → `[FS] disarm: restored=17563 of 17563 swapped`. **[M] Game log ($LOKI, `docs/s191-e4-flight3-loki.log`) INDEPENDENTLY WITNESSES: `LogLokiCharacter: [Client][BP_Minion_KaijuAxeLeader_Stagger_Tutorial_C_2147471215] Cleared ignored allies when moving` + `LogAbilitySystem: Property Health new value is: 0.00` + `GC_Ronin_Execute_Highlight_C_2147471178 - OnActive`** — the game's own ability system emitted the Health-setter receipt for our AdjustHealth call AND the Ronin execute-passive cue fired on the low-HP minion (independent live re-confirmation that WALL E is favorable — the game treats the minion as a hostile enemy execute target). **[M] Live WALL E favorable**: `MinionTeamID=100` read off the spawned minion vs player teamIndex −1; no team poke. **[M] Track-D primitive generalizes**: minion class needs NO AddComponentByClass/K2_InitStats (constructor ASC + pre-registered HealthSet); reuse is one seed + one InitAAI. **⚠ SCOPE: KE4DIRECTGE (the graded consolation) killed the minion, NOT the natural LMB cast.** After clean disarm, 5 LMB events sent via P/Invoke (2×150ms taps + 1×800ms hold + 2×short taps) with confirmed foreground focus produced **ZERO** new `LightAttack`/`Warmup`/`Channeling`/`Fireblast`/`going from nullptr` lines in the 30s Loki.log tail. **[I,strong]** the LMB input path from the OS to Ronin's Ability1 is not wired for our K_GRANT'd spec: our native `GiveAbility` registers the spec (Items 0→1) but does NOT invoke `BindAbilityActivationToInputComponent` — the game's real input-binding pathway that routes OS mouse events → InputID → spec. S147's proof of natural-input activation used LeftShift→InputID 5 because that keyboard binding was pre-wired via the KWIREGAS possession chain for the pre-existing hero-kit spec; a freshly-granted LMB spec has no such pre-wiring. Rules R-S191-a..f banked (residency-candidate-list pattern, rotation-read facing, AvatarActor readback bind check, GRANT ≠ input-binding, Loki.log `Property Health` as free receipt, Ronin Execute Highlight as free WALL-E+damage witness). **⇒ The full E4 predicate is now BLOCKED ONLY on LMB→Ability1 input binding on the staged tutorial world.** Every other component is [M]. Follow-up options: (a) call the game's `BindAbilityActivationToInputComponent` after K_GRANT with our spec + LMB InputID; (b) `TryActivateAbilityByClass` on the disarmed environment (S158 style) even without OS input routing; (c) grant Ability1 via the game's real hero-kit init pathway which also wires input. Regression gates BYTE-IDENTICAL throughout: `botai b620e0ff3279e673`, `play 31af7667355f39d1`, `-seedmax b47d1bfd04e44921`, `-postshots1 1925e9e80646a4fb`. E4 arm digests: `e4 6cb523c770d92bc1`, `e4-directge 377f263062790979` (real treatment pair, --dupes clean, verify_dll PASS, KERNEL32-only).

  **★★★★★ THE INITIAL FRONTIER = E4 (BUILD + FLY): `docs/next-session-prompt-s191-e4-predicate.md`.** ⚠⚠⚠ **FULLY SUPERSEDED BY THE S191 E4T ARC ABOVE — the E4 predicate is fully MET on both Ability3 (MiniDash) and Ability1 (LMB / Ronin's LightAttack1). WALL P is DEFEATED via a 9-byte compound-poke + damage-verb loop. The "use natural input ONLY / shim TryActivate dies 0xDEAD" advice below is REFUTED — a compound-poke on the ability instance's phase state + ProcessEvent(Invoke) + ProcessEvent(DashHit/MeleeHit) does not die and applies real damage.** Original design (kept for the historical KBFE4 build recipe): extend RM_BOTFIGHT with a new `#if KBFE4` step to spawn+seed a minion + drive natural LMB. The workflow (`wf_92a08166-2b3`, 11 agents, 4.4M tokens) refuted the handoff's proposed `KBFSELFCAL=1 KBFBINDAVATAR=1 KBFNATURALINPUT=1` combo as **impossible** (source `#error` guards forbid it) and validated my actual `KBFARMS=0xC6 KBFABIL="Ability1" KBFINPUTID=3 KBFE4=1 KBFNATURALINPUT=0` composition. Read `docs/s190-wallp-predicate-plan.md` first (WALL P decomposed; FK-32 window ~40-60s = OFF the critical path; WALL E gate = [M] FAVORABLE / GO). ⚠ STUDY DoBotFight's #else K_* control flow before adding to the arm. FK-31 clusters (budget ≥2-3 launches/armed window); ~~use natural input ONLY (a shim TryActivate dies 0xDEAD)~~; do NOT fight FK-32.

  **★★★★★★★ S189-MV-WASD SHIPPING ARM F1 FLOWN 2026-09-09 (~30min post-arm build), `docs/s189-mv-wasd-ship-f1-SHIPPING_ARM_VALIDATED.md`: [M] fresh launch (PID 49256 different from prior 1780, base 0x7FF68E160000, different ASLR + heap), full staging chain gft→fo→sp→-mv-play via `configs/fk24-stage.ps1`. All 4 injections landed clean.** 10/10 pre-registered predictions hit. Full 9-line receipt chain in exact sequence: [S155] BIND_AVATAR result=ok → [S156] MAX_HEALTH_SEEDED exact=yes → [S189-MV] GETTER_BEFORE mode=3 heroF08=0x0 MaxSpeed=0 → 6 SEED_ROWs all exactBase=1 exactCurr=1 → SIBLING unchanged=1 → MOVEMENT_SEEDED aborted=no attrs=6 wrote=6 → WIREF08 pointsAtSet=1 → GETTER_AFTER MaxSpeed=43FA0000(500.00) MaxAccel=47435000(50000) → **[S189-MV] POKEGRAVITY pokeOk=1 gravPre=0.0000 gravPost=1.0000 exactPost=yes** (cmcOff=0x458 gravOff=0x1A0 both resolved live by PropOffsetSuper name lookup). **[M] The physical effect confirmed**: hero fell 13240 → 386 uu (same ledge S189-MV-WASD F3 measured). ⚠ Fresh-launch physics quirk reproduced: PhysFalling doesn't tick when Vz=0, hero hovered ~9.5s post-poke before something engaged physics and the fall proceeded (F3 saw the same). ⚠ FK-32 fired at t+342s (0x0000DEAD, base-rate hazard per S138) before hero could transition to Walking + receive drivable WASD — but that ingredient was already [M] in F3 (2300 uu at 500 uu/s cap). Shipping arm's job is packaging proven-live ingredients; validation is per-ingredient receipt + direct effect fire, NOT re-proving compound downstream behavior. ⚠ Flight orchestration defect: WASD child-powershell + Add-Type + focus grab took ~35s from probe start, so the probe missed the input window entirely — needs longer probe duration or a pre-warmed send-wasd process. New rule R-S189-MV-WASD-l (shipping-arm validation via receipt + direct-effect fire is sufficient when compound downstream was already [M]) banked. Regression gates unchanged since `051cc08`. **[M] The -mv-play shipping arm is one-inject drop-in and does NOT require any external RPM**; the standard staging chain delivers full player playability in one clean sequence. Physical-effect measurement retained; Walking + WASD confirmation needs a follow-up flight with fixed orchestration timing.

  **★★★★★★ S189-MV-WASD SHIPPING ARM BUILT 2026-09-09 (~30min post-F3): `KBFPOKEGRAVITY` knob added to `tools/sigbypass-mod/tutorial_launch.cpp` + build variant `-mv-play` added to `build.ps1`. Not yet flown; the ingredients (S189-MV wire, GravityScale=1 poke effect, Walking-mode WASD) are all individually proven live in F1/F2b/F3.** Variant `botfight-damage-self-cal-bindavatar-seedmax-mv-play` RAW `87c764b6d235b7a3` VSIZE `97516dd4eb1ad408`. Extends `-mv-obs-wiref08` with one additional aligned 4-byte DATA write inside `BfS148DoCalibration`: `PropOffsetSuper(heroCls, "CharacterMovement")` → CMC → `PropOffsetSuper(cmcCls, "GravityScale")` → SEH-wrapped `*(volatile float*)(cmc+gravOff) = 1.0f` → readback verify → marker line `[S189-MV] POKEGRAVITY pokeOk=1 gravPost=1.0000 exactPost=yes`. Runs AFTER GETTER_AFTER (so seed-reaches-getter evidence is captured under the Falling arm exactly like F1) and before the S148 preflight. Delta from `-mv-obs-wiref08`: +512 bytes of `.text` (one PropOffsetSuper resolve + one SafeReadable+SafeWritable + one SEH-wrapped write + one Markerf). **Regression verification [M]**: all 10 prior S189-family gates BYTE-IDENTICAL after the edit — `-seedmax b47d1bfd04e44921`, `-mana-seedcompound1000 485942a135855110`, `-mv-obs 5b7444907167ccfc`, `-mv-obs-wiref08 8623bb672d86a66d`, `-postshots1 1925e9e80646a4fb`, `-postshots1-heal100 0b9573215c85f511`, `-postshots1-heal500 b0e29a434eabd87d`, `-postshots2 ad87408901f481c5`, `-postshots2-floor600 ca54361966e71afd`, `-postshots-mana c7ce885988fd3532` — proves `KBFPOKEGRAVITY=0` default truly dead-strips. `text_digest.py --dupes` clean (no accidental byte-identical arms). `verify_dll.py` VERDICT PASS (KERNEL32-only imports, no C++ exception machinery, no CRT). Policy checks enforce the pyramid: `KBFPOKEGRAVITY` requires `KBFSEEDMOVEMENT=1 AND KBFWIREF08=1` (without seed+wire, GetMaxSpeed returns 0 and the fallen player cannot walk). NOT restored on disarm (persistent player-state; shipping semantic). Zero .text write, one data write per staging. **Flight recipe**: stage the tutorial world normally (gft → fo → sp → probe with `-mv-play` variant); the shim will emit its marker chain including `[S189-MV] MOVEMENT_SEEDED`, `[S189-MV] WIREF08 pointsAtSet=1`, `[S189-MV] GETTER_AFTER MaxSpeed=43FA0000(500.00)`, then `[S189-MV] POKEGRAVITY pokeOk=1 gravPre=0.0000(00000000) gravPost=1.0000(3F800000) exactPost=yes`. Then send WASD via `scratchpad/s189-mv/tools/send-wasd.ps1` — hero should fall to Z=90.15 and walk at 500 uu/s cap. Precedent for in-shim GravityScale poke: S141 T3 armk (this file's own `RM_WAKEMOVE [WM] GravityScale=1.0 set` at line 3810). Rule R-S189-MV-WASD-k (shipping arm packages proven-individually-live ingredients into one inject; no new live flight needed if each ingredient is [M] separately) banked.

  **★★★★★★★★ S189-MV-WASD F3 flown 2026-09-09 (~15 min after F2b), `docs/s189-mv-wasd-f3-THE_PLAYER_WALKED.md`: [M] BRANCH A — THE PLAYER WALKED. Full playability chain proven end-to-end.** Precheck read `AirControl @ CMC+0x2B0` live during W-hold = **0.0150** (30% stock UE default, offset discovered via `tools/re/fk13uht.py` UHT scanner — UCMC prop 29). Adjacent: `AirControlBoostMultiplier@+0x2B4=2.0`, `AirControlBoostVelocityThreshold@+0x2B8=25.0`, `FallingLateralFriction@+0x2BC=0.375`. **Poked AirControl→1.0 alone: Vel still zero. Poked FLF→0.0 additional: Vel still zero. NEITHER is the block.** Then poked `GravityScale @ CMC+0x1A0` 0.0→1.0f — hero FELL from Z=13240 to Z=386 (stopped on ledge), stayed Falling. Next W-hold at Z=386 slid hero off ledge (diagonal 45° Accel visible in first sample), fell further to Z=90.15 (tutorial floor bit-exact to S132 measurement), MovementMode transitioned 3→1 (Walking). **Once in Walking mode, W-hold produced FULL walk: peak Velocity = EXACTLY 500.000 uu/s (bit-exact match to S189-MV MoveSpeed seed value)**, Acceleration=-45768 uu/s² (matches `MaxAccel(50000) - Velocity(500) × GroundFriction(8) × dt` — GroundFriction=8 from S189-MV seed applied), traveled from (0,0) → (953, -2108) = **~2300 uu of horizontal displacement in 3 seconds**. Character forward direction = -Y (matches F1). **[M] 4 compound findings in ONE 3s W-hold**: MoveSpeed clamp, Falling→Walking transition, MaxAcc+GroundFriction interaction, character orientation. **[M] External RPM WPM of 3 UPROPERTY fields (AirControl, FallingLateralFriction, GravityScale) is DATA-class safe** — 3 pokes with readback verify, no FK-32 through 6+ flights on 2h55m+ uptime PID. **[M] Peak Vel = seeded MoveSpeed value BIT-EXACT** — R-S189-MV-a-compliant discriminator (a non-500 seed would have shown a non-500 cap; 500 is bit-exact to what we wrote). **[M] The playability chain is proven**: S189-MV wire + GravityScale=1.0 + hero landing on walkable surface + WASD = drivable player hero. New rules R-S189-MV-WASD-h (a fall doesn't always land on the floor), -i (peak velocity = seeded value is bit-exact evidence), -j (single-arm test can produce compound receipt) banked. **★★★★★ SHIPPING PATH IDENTIFIED**: extend S189-MV KBFSEEDMOVEMENT with ONE in-shim GravityScale=1.0f poke on the player hero → player starts drivable, no external RPM required. Zero regression gates changed. Live post-flight: hero at (953, -2108, 90.16), Walking, all three pokes still in place.

  **★★★★★★ S189-MV-WASD F2b flown 2026-09-09 (~1min after F1), `docs/s189-mv-wasd-f2-ANALOG_MOD_1_CLAMP_REFUTED.md`: [M] AnalogInputModifier = 1.0000 DURABLY during W hold** (measured at CMC+0x3D0, S141 T3 [I,strong] offset validated live). Means `MaxInputSpeed = GetMaxSpeed() × AnalogInputModifier = 500 × 1.0 = 500 ≫ 1e-4`, so **S141 T3's leading hypothesis — CalcVelocity clamp at `0x035D6511-0x035D652F` — is REFUTED as the Velocity-zeroing mechanism.** The block is DOWNSTREAM of both the clamp AND ScaleInputAcceleration AND ComputeAnalogInputModifier. ★★★★★ **LEADING NEXT HYPOTHESIS [I,strong]: `UCharacterMovementComponent::AirControl`** (float32 UPROPERTY, binds_members.csv:28428, offset TBD on this build). Stock UE gates lateral velocity in `MOVE_Falling` by AirControl multiplier — if AirControl ≈ 0, WASD-in-Falling produces zero lateral velocity despite full Acceleration. **Directly matches our measurement.** Also candidates: `AirControlBoostMultiplier`, `AirControlBoostVelocityThreshold`, `FallingLateralFriction` (UCMC props 25/26/27), and Loki's own GAS-backed `ULokiAttributeSet::AirControlMultiplier` (binds_members.csv:39719) which S189-MV did NOT seed — likely reads 0 and composes AirControl to 0. **CMC memory scan (`scratchpad/s189-mv/tools/cmc_float_scan.py`) returned promising 0.05 candidates at CMC+0x0400 and +0x0424 (stock AirControl default = 0.05), but offset identity NOT YET SETTLED — Flight 3 offline task.** ⚠ F2 anomaly (5-arm sequence w,s,a,d,f13 gave ZERO on every arm including W) — UNEXPLAINED but REFUTED same session by F2b W-only reproducing F1 result exactly. Not a systematic property of the build; possibly focus loss between arms or F13 trailing-effect. **New rules R-S189-MV-WASD-f (measure the leading hypothesis when it's cheap — one 4-byte RPM at CMC+0x3D0 refuted the whole clamp hypothesis in one flight), R-S189-MV-WASD-g (single-flight anomalies are not results — reproduce or discard).** Regression gate `botfight-damage-self-cal-bindavatar-seedmax-mv-obs-wiref08 8623bb672d86a66d` UNCHANGED (pure read-only + P/Invoke).

  **★★★★★★ S189-MV-WASD F1 flown 2026-09-09 (~2h33m after S189-MV F1 on the same PID 1780), `docs/s189-mv-wasd-f1-BRANCH_B_W_DRIVES_ACCEL.md`: [M] BRANCH B — W (VK_W=0x57) drives `ControlInputVector`, `ScaleInputAcceleration` runs, `Acceleration = (0, -50000, 0)` holds DURABLY across the full 3s W hold with 30Hz auto-repeat. Velocity STAYS ZERO. Location UNCHANGED at (0,0,13240). This DIRECTLY REFUTES the pre-existing measurement in `tools/sigbypass-mod/tutorial_launch.cpp:3817` ("input_watch proved WASD produces ZERO ControlInputVector") — [M] instrument artifact: input_watch had the same ConsumeMovementInputVector race defect our burst-read (3 reads × 1ms gap per sample) caught. CIV IS driven — 3/30 W-arm samples caught CIV.Y=-1 with the burst defense — but is consumed every frame so single-read probes see 0.** Design provenance: multi-agent workflow `wf_45a88dd2-ec4` (5 Understand + 1 Design + 2 Verify + 1 Adjudicator, 9/9 done, 3.08M subagent tokens, ~22 min). Both adversarial verifiers refuted the initial design and produced 4 CRITICAL fixes this flight PROVED essential: (1) 30Hz auto-repeat sustains axis input (single WM_KEYDOWN would decay after one frame); (2) SwitchToThisWindow + AttachThreadInput bypasses SetForegroundWindow denial (first attempt failed with focus on Claude Code UI PID 29980); (3) CIV burst-read 3× per sample defeats consume race; (4) F13 (0x7C) as unbound negative control (backquote is confounded with dev console). **[M] D (VK_D=0x44) drives ZERO across 29 samples in the D-arm window — Accel/CIV/Vel/Loc all zero.** Genuine null: W in same session with same primitive drove durable non-zero Accel, so D's zero-Accel means input never reached ScaleInputAcceleration for D specifically. [I] most likely cause: MoveRight needs `ControlRotation.RotateVector((0,1,0))` = non-zero right axis, but character's ControlRotation.Yaw is likely 0 (menu-parked), so strafe projection is degenerate. **F13 clean zero across 29 samples — negative control passed.** **[M] S189-MV state HELD 2h33m across 4 external RPMs**: hero+0xF08=0x19318915BC0 wired throughout, no state decay. Regression gates: `botfight-damage-self-cal-bindavatar-seedmax-mv-obs-wiref08 8623bb672d86a66d` UNCHANGED (no shim rebuild — flight is pure P/Invoke + read-only RPM). **[M] The Velocity block is DOWNSTREAM of Acceleration, not the input path.** Candidates for follow-up: (a) RPM-read AnalogInputModifier@CMC+0x3D0 during W hold to test S141 T3's CalcVelocity clamp hypothesis; (b) poke GravityScale=1.0f, wait for MovementMode 3→1 (Walking), re-run W and see if Vel grows in Walking-arm; (c) direct Velocity poke to test mover independent of input path. **New rules R-S189-MV-WASD-a (burst-read defense for consumed-per-frame fields), R-S189-MV-WASD-b (durable derivative beats race-y source — Accel is more reliable than CIV), R-S189-MV-WASD-c (SwitchToThisWindow + AttachThreadInput is THE focus-grab primitive), R-S189-MV-WASD-d (auto-repeat sustains axis input), R-S189-MV-WASD-e (shim source comment is unverified claim, not measurement) banked.** ⚠ `tutorial_launch.cpp:3817`'s claim informed RM_PUPPET's whole workaround architecture — the puppet is still functional but the "WASD is dead" premise is REFUTED. Actionable next: RPM-read AnalogInputModifier during a 2nd W-arm; poke GravityScale=1.0 for canonical land-and-walk test; test S (should be symmetric to W); test A (should be symmetric to D — same axis, opposite sign) to distinguish "MoveRight axis dead" from "D key specifically unbound".

  **★★★★★★★★ S189-MV KBFSEEDMOVEMENT UNLOCK flown 2026-09-09, `docs/s189-mv-f1-MOVEMENT_UNLOCK.md`: [M] THE S141 T3 MOVEMENT WALL'S "why does GetMaxSpeed return 0" QUESTION IS CLOSED end-to-end via ONE seed + ONE pointer-poke.** Variant `-mv-obs-wiref08` (RAW `8623bb672d86a66d`, +DKBFSEEDMOVEMENT=1 +DKBFOBSMOVGETTERS=1 +DKBFWIREF08=1). Design provenance: multi-agent workflow `wf_ecd60ab0-324` (5 Understand + 1 Design + 2 Verify agents, ~13min, 2.79M subagent tokens). **[M] Read-path model confirmed live**: `GetMaxSpeed` (ULokiCMC vtable disp 0x4C8, RVA 0x0055ACB90) reads via `hero+0xF08 → AttrSet+0xF0.CurrentValue@+0xC` (MoveSpeed) `minss` `AttrSet+0x100.CurrentValue@+0xC` (MaxMoveSpeed). On KWIREGAS player, hero+0xF08=NULL → helper 0x055AC9F0 early-outs `mov rbx,[rcx+0xF08] / je return-zero` → 0.0f. The one pointer-poke `hero+0xF08 = spawnedSet0` (the same set S189-SEED just proved seedable at ASC.SpawnedAttributes[0]) is the SINGLE change that unlocks it. Full 4-station receipt: **GETTER_BEFORE**: `vtblExact=yes mode=3 heroF08=0x0 MaxSpeed=00000000(0.00) MaxAccel=00000000(0.00)` (baseline confirmed live). **6 SEED_ROWs**: MoveSpeed=500, MaxMoveSpeed=500, MaxAcceleration=45000, GroundFriction=8, BrakingDecelerationWalking=2048, Mass=100 all landed pre=(0/0)→post=(seed/seed) exactBase=1 exactCurr=1. **SIBLING_READ_POST** AttackSpeed@+0xE0 unchanged=1 (base=42B40000=90.0f pre and post, proving no stride overrun; incidental measurement: this player's real AttackSpeed value). **WIREF08** wireOk=1 heroF08Pre=0x0 heroF08Post=spawnedSet0 pointsAtSet=1. **GETTER_AFTER**: `heroF08=spawnedSet0 MaxSpeed=43FA0000(500.00)` — 🎯 **the SEEDED value reaches the getter**. **MaxAccel=47435000(50000.00) NOT our 45000 (0x4732C800)** — the adversarial verifier's pre-registered discriminator caught it: on `mode=3 MOVE_Falling` the Loki override `0x0055AC910` takes FALLING arm which uses the +0xC00 helper as GATE only then delegates to engine `Super::GetMaxAcceleration` which returns CMC's OWN MaxAcceleration UPROPERTY (stock 50000). The 45000 vs 50000 gate proved the read-path model in BOTH directions in one flight (Walking → seed reaches getter; Falling → Super bypasses). **[M] game alive 272s+ post-flight, no FK-32** (10 attrs now seeded on ULokiAttributeSet across S189-SEED + S189-MV — write class extended safely). All 8 prior gates BYTE-IDENTICAL. Both s148 contract tests PASS. **New rules R-S189-MV-a (pre-registered discriminator on stock-collision values), R-S189-MV-b (vtable identity validation before direct dispatch), R-S189-MV-c (Loki override asymmetry per MovementMode arm) banked in evidence doc.** Actionable follow-ups: MovementMode=1 (Walking) poke to test seeded +0x120 via Walking-arm; WASD hookup to test player actually MOVES with seeded 500 uu/s + engine-Super 50000 MaxAccel; bot-pawn ARM G recipe port for the SpawnAIFromClass path.
  **★★★★ S189-SEED MaxMana seed test flown 2026-09-09, `docs/s189-seed-f3-MANASHOT_APPLIED.md`: [M] MANASHOT_APPLIED first-try (F3 after F1 FK-31 + F2 caught-bug). AdjustMana(+50) on a seeded ULokiAttributeSet lands 0→50 EXACT (postBits=42480000/42480000, arithmeticOK=yes) with MaxMana held at 1000 and all 3 compound siblings unchanged.** Variant `-mana-seedcompound1000` (RAW `485942a135855110`, +DKBFSEEDMAXMANA=1 +DKBFSEEDMAXMANA_BITS=0x447A0000u). Design provenance: multi-agent workflow `wf_1de95504-0b7` (5 Understand + 1 Design + 2 Verify agents, ~24min). Adversarial mechanism verifier AMENDED the naive solo-MaxMana seed to compound-seed (MaxMana=1000 + BaseMaxMana=1000 + MaxManaPerLevel=0 + BonusMaxMana=0 all via direct-offset writes on `ULokiAttributeSet` at +0x220/+0x230/+0x240/+0x250) to defeat any PostGameplayEffectExecute recompute f(BaseMaxMana, MaxManaPerLevel×Level, BonusMaxMana). **[M] compound-seed strategy worked: MAX_MANA_SEEDED exact=yes** (all 4 post-readbacks match seed), **PERSISTED through S148 primary AdjustHealth into MANASHOT_PRECHECK** (`maxBits=447A0000/447A0000 maxMP=1000.00/1000.00`), MANASHOT_COMPOUND_PRE snapshotted all 3 siblings at seeded values, AdjustMana(+50) fired, MANASHOT_POST postMP=50.00/50.00 arithmeticOK=yes postMaxBits=447A0000 unchanged, **MANASHOT_COMPOUND_POST shows compound siblings UNCHANGED after AdjustMana** (the predicted recompute did NOT fire — either it doesn't exist for AdjustMana or is gated behind an unmet condition; solo-MaxMana seed untested). **[M] FIRST project write to `ULokiAttributeSet` (previously-untouched write class) SURVIVED — game alive at +368s post-flight, no FK-32.** ⚠ F2 caught real bug: seed used `s_seeded.asc` but s_seeded is populated only at line 22893 (downstream of KBFSEEDMAXMANA block in initial dispatch); fix = use `s156Pre.asc` (canonical fresh ASC from S156's own resolve). Rule R-S189-seed-a banked. **★★★★ THE BIG UNLOCK**: `ULokiAttributeSet` is a 124-attribute superset containing all S141 T3 movement wall targets — **MoveSpeed@+0xF0, MaxMoveSpeed@+0x100, MaxAcceleration@+0x120, GroundFriction@+0x130, BrakingDecelerationWalking@+0x140, Mass@+0x170** (layout audit measured in workflow). The mechanism proven here (direct-offset seed via `PropOffsetSuper` + 8-byte volatile write + readback) generalizes to a KBFSEEDMOVEMENT knob targeting the movement wall directly. Regression gates all 7 prior variants BYTE-IDENTICAL. Both s148 contract tests PASS. Adversarial verify's compound-seed refutation was DEFENSIVE and turned out unnecessary (MANASHOT_COMPOUND_POST unchanged proves it), but the diagnostic snapshot IS what proved that — cost-of-diagnostic << cost-of-repeat-flight. Solo-MaxMana seed, multi-shot Mana, Mana cap test, and KBFSEEDMOVEMENT extension all remain OPEN.
  **★★ S189 Mana attribute test flown 2026-09-08 (`docs/s189-mana-f2-MANASHOT_NO_OP.md`): [M] AdjustMana native path is REAL and follows the SAME UHT-wrapper mechanism as AdjustHealth; the S153 thunkExact fix GENERALIZES [M].** Variant `-postshots-mana` (RAW `c7ce885988fd3532`, KBFPOSTSHOTS=0 + KBFPOSTSHOT_MANA=1 + KBFPOSTSHOT_MANA_DELTABITS=0x42480000u=+50.0f). Full chain executed: `MANASHOT_RESOLVE` (`wrapperExact=yes implResolvedRva=0x55166D0 tailReason=ok class=0x22AC76BB000 ownerExact=yes chainComplete=yes` — the mana wrapper sits at exactly `AdjustHealth thunk + 0x80` = `0x52942F0`, and its `E8 <rel32>` tail-call resolves to the mana impl RVA `0x55166D0`; **[M] S153 fix is universal for UHT wrappers with parameters, not AdjustHealth-specific**) → `PRECHECK` (`manaSet=0x22C0E441BE0 spawnedIndex=0` — Mana attribute set lives at SpawnedAttributes index 0, LokiAttributeSetHealth at index 1) → `CALL_ISSUED deltaMP=50.00` → `POST postMP=0.00/0.00 observedDeltaCurrentMP=+0.00 arithmeticOK=NO identityStable=yes` (Mana clamped to 0 because MaxMana=0.0/0.0 at baseline — the empty attribute set with nothing seeded) → **`MANASHOT_COMPLETE RESULT=MANASHOT_NO_OP callCount=1 identityStable=yes`**. Clean disarm `restored=17563 of 17563`, no FK-32, game alive at 353s uptime. F1 lost to FK-31 (ACG-denied sp inject), F2 first-attempt success. **[M] The MANASHOT_NO_OP outcome is INFORMATIVE**: the clamp itself is the receipt that the game's own attribute-clamping code ran on Mana — proof of parity with the AdjustHealth clamp measured at S156-B heal500, extending "silent clamp on max=0" to a non-Health attribute. **[M] Second confirmation of the S153 thunkExact fix on a different UFunction** promotes it from "worked once" to a mechanism-level UHT-emission pattern (first was S156-A F5 on AdjustHealth). New rules banked as R-S189-a/b/c in the mana evidence doc. Full Mana behavior (damage, heal, non-zero cap) is unlockable with ONE MaxMana seed step (a `KBFSEEDMAXMANA` companion knob byte-identical to S156-A's `KBFSEEDMAXHEALTH`, offset changed from `+0x80` on LokiAttributeSetHealth to `+0x220` on ULokiAttributeSet). Regression gates BYTE-IDENTICAL across all 6 prior variants (KBFPOSTSHOT_MANA=0 default truly dead-strips). Non-Health attributes on `ULokiAttributeSet` (MoveSpeed, MaxMoveSpeed, MaxAcceleration — the S141 T3 movement wall's own targets) untested via this mechanism.
