...
- **`docs/next-session-prompt-*.md`** — chronological handoffs. **Latest: `docs/next-session-prompt-s151.md`** (S150 → S151): ★★★★★ **Three unflown arms ready for versus AI**: (A) DLP drop-chain-in-one-injection (`d2ebf30`, `docs/dlp-flight1-PREREGISTERED.txt`); (B) Move 3 BINDCENSUS — the WALL P H1/H2 discriminator via a read of pawn+PC InputComponent.ActionBindings TArray + FsDisarm heartbeat, `KBFBINDCENSUS_AB_OFF=0xd0` pinned [M] (`852fbfd`, `docs/s147-move3-flight1-PREREGISTERED.txt`); (C) Move 4 WALL E Phase 1a — chain S149-bind-only + frozen S148 self-damage in one sitting, gated by a read-only RPM verifier (`0fe28f0`, `docs/s148-move4-flight1-PREREGISTERED.txt`). All three are unflown; the recommended fly-order is Move 4 first (highest info yield, four pre-registered informative outcomes). This session also shipped the S149-fix capture-gen gate (`e8e757f`) that all three depend on via `launch-redirect.ps1 -ResetCapture`, and a pre-commit hook policy (`048ac99`) that blocks >100 MB blobs at commit time. **Predecessor S147/S146 blocks below are unchanged history**; read them for WALL P context but the s151 doc is the current opener.
