...
- **S156-B (2026-09-08):** ★★★★★★ **BACK-TO-BACK `AdjustHealth` WORKS — SHOT_2 commits identically
  ...
  **★★★★★ THE CURRENT FRONTIER = E4 (BUILD + FLY): `docs/next-session-prompt-s191-e4-predicate.md`.** Read `docs/s190-wallp-predicate-plan.md` first (WALL P decomposed; FK-32 window ~40-60s = OFF the critical path; WALL E gate = [M] FAVORABLE / GO). The predicate reduces to ONE unproven thing: a PLAYER ability's damage EFFECT (a real GameplayEffect, not a direct write) landing on a target's HP. **E4 arm** = extend RM_BOTFIGHT (`KBFSELFCAL=1 KBFBINDAVATAR=1 KBFNATURALINPUT=1 KBFABIL="Ability1"` → GS_Ronin_LightAttack1 the LMB basic) with a new `#if KBFE4` step: `SpawnActorCls("BP_Minion_TargetBot_TTK_C")` in front of the player (a team-100 minion = ENEMY by raw-index compare, no team poke, [M]), seed its HealthSet LOW (reuse the committed Track-D/S156 spawn+seed+InitAAI), then SETUP_ABORT (S158) and drive a natural LMB TAP via `scratchpad/s190/tools/send-lmb.ps1` (<240ms = melee cone, not the fireblast). `KE4DIRECTGE` fallback (by-handle GE / AdjustHealth) keeps it productive under WALL P. ⚠ STUDY DoBotFight's KBFNATURALINPUT control flow before injecting (the K_* arms compile out; an S147 state machine runs — the minion spawn+seed must land in phase-1 before SETUP_ABORT). FK-31 clusters (budget ≥2-3 launches/armed window); use natural input ONLY (a shim TryActivate dies 0xDEAD); do NOT fight FK-32.
