...
| Drop-in / DropPlane | **OPEN — the S93 falsification does NOT generalise (FK-22, 2026-08-16, `docs/fk22-dropphase-reachability.md`).** `SpawnPlane` is a per-variant BP override: 3 sibling `Comp_GameMode_DropPlane*` classes, all deriving directly from native `ULokiGameModeDropPlaneComponent`, **no BP-to-BP inheritance** — so an S93 measurement on the `_Tutorial` override cannot transfer. The **general** variant queries no markers (9 stmts, 0 `GetAllActorsWithTag`; its spawn lives in `OnDeathCircleSet`, derived procedurally from the death circle). The Tutorial variant's 3 markers **DO exist in `LVL_Tutorial`** (`Tags` arrays, 3 separate WP cells) ⇒ S93's stated reason is refuted on its own map; its `FAULTED` was a bare SEH catch through a primitive with an uninitialised `FFrame.FlowStack`. ⚠⚠ **THE TWO BLOCKERS THIS ROW USED TO NAME ARE BOTH SUPERSEDED — do not cite them.** (1) *"the round phase never leaves `EGP_BeginInit(1)` (193/193 log files)"* — **SOLVED S124**: one direct `GoToPhase` call self-drives the round all the way to `EGP_Combat` (six transitions, three of which the game fired itself), and the 193/193 corpus measures `GoToPhase`'s **argument**, i.e. a fact about its seven callers, not about the stored byte. (2) *"the player→plane / pod→hero handoffs are empty C++ impls"* — still true, but **not what stops the drop today**. **Blocker chain as measured through S130's first half [M] — ⚠ its last two links are SUPERSEDED further down this row (the pod now spawns; the frontier moved to the RIDER handoff):** markers refuted → phase solved → **subscription dead by construction** (`ServerOnly` is `mov byte [rdx],0; ret`, and the DropPlane component was measured absent from the 7-subscriber invocation list, S124 §15) → `SpawnDropPodForTeam` runs and returns false → **the pooled spawn returns NULL** → ⚠ **NOT because the actor pool is disabled — REFUTED S130** (`docs/s130-actor-pool-gate-settled.md`): the acquire `TMap::FindOrAdd`s and a pool miss falls to a shipped fallback into a normal `UWorld::SpawnActor`, so an unprimed pool cannot produce that NULL. **The live frontier is C7 `CDO->byte@0x6C != 0` / C8 `PoolMgr->GetWorld()==null` / C9 `SpawnActor` null-or-skipped inside `0x5648050`, and C7 costs ONE read-only RPM read.** ⚠⚠ **THAT SENTENCE IS SUPERSEDED — the read was taken, C7 WAS the answer, and bail 2 is FIXED (S130, 2026-08-20, `docs/s130-actor-pool-gate-settled.md` §13).** Poking `CDO->bCanEverReplicate = 0` on the drop-pod CDOs — **one heap byte, readback-verified, zero `.text` writes** — makes `SpawnDropPodForTeam` return **`true`** with a DropPod census delta of **+2**, and both pooled spawns return live `BP_DropPod_Tutorial_C` actors. ⛔ It is a **DIAGNOSIS, not a shipping fix**: it mutates a class default for the process lifetime and may break the pod's replication. ⚠ **C8/C9 never fired — unexercised, not excluded.** ★ Separately settled S130: the pool gate IS `ALokiGameState::bSupportsActorPoolPriming` (`+0x898`), false because `BP_LokiGameState_Tutorial`'s shipped class default sets it `False`; and the hand-spawn bypass hits a **FIFTH wall**, `AuthPlayerEnterWorldAttachedToRidable` (`0x55CD510`), a real body that always fails on a stripped fold ⇒ a pod and no rider. ★★★★★ **AND THE CHAIN RAN THREE MORE STEPS — S131/S132 (2026-08-20), on the tutorial route, zero `.text` writes, zero PI hooks.** **(a) THE POD IS FUNCTIONAL, NOT MERELY COUNTED [M]** (`docs/s131-pod-functionality-settled.md`): `InitializeDropPod` ran and all three discriminating writes landed against a within-run, same-class negative control of three sibling pods — `PodTeamIndex` −1→0, `bIsTeamLeaderPod` False→true, and `CurrPodDestination` (0,0,0)→the exact `LandingLocation` that arm computed, a **payload fingerprint** no other code path in the process holds. The pod then **flies**, at the cooked asset's `InitialSpeed = MaxSpeed = 20000` uu/s (`ComponentVelocity = (20000,0,0)`, Y and Z exactly constant) while all three control pods read 0.0 — **because `StartPodGameplay` never ran**, whose first act on the movement component is `Deactivate()`: `Loki::LokiIsServer()` impl `0x0F7EB60` is `xor al,al; ret`, always FALSE. **(b) THE FIFTH WALL IS NOW MEASURED, AND IT IS ONE GETTER, NOT ONE FUNCTION [M]:** called directly on the pod's own `LokiRideable` component with a live valid PlayerState, its bail line went **0 → 2, exactly one per call**, against a verified zero baseline, a same-object positive control and both IsValid preconditions read out passing — and `AuthPlayerPreSpawnOnAddToPlane` (`0x55CD800`) fails the **same** way on its own distinct string. `AuthPlayerEnterWorld` (`0x55CCE70`) is **not a way round it**: it calls the same stripped `0xF7EB50`, merely un-gated. ⇒ *"a pod and no rider"* stands **for the ENTER path**, and the cause is precisely **one deleted accessor** — the round game mode **EXISTS, is live, and passes the wall's own `IsA<ALokiRoundGameMode>` check** (cached at `Comp_GameMode_DropPlane_Tutorial+0xE0`; `UWorld::AuthorityGameMode @ UWorld+0x250`). ★★★★★ **(c) THE EXIT PATH IS OPEN — THE DISMOUNT RUNS AND IS USABLE (S132, `docs/s132-dismount-settled.md`).** Appending the PlayerState to `ULokiRideableComponent::PlayersAttached` (`+0x130` Data / `+0x138` Num / `+0x13C` Max) with the game's own `ResizeGrow` `0x00F988D0`, then calling `AuthPlayerDetachPlayerFromRidable` (impl `0x55CCCB0`) through the S55 direct thunk, takes the hero **out of the pod**, un-hides it, restores its collision and movement, and **teleports it to a chosen landing actor**. Measured over four dismounts in one armed window (hero X tracked the flying pod's X at each call's own time; run 2's hero Y bit-identical to the pod's), then twice more with a `LokiPlayerStart` as the `LandingLocationActor` (in flight 2 that PlayerStart was **1,488,146 uu** from the pod at the moment of the call; flight 3 fired with the pod at `X = 1,256,845`), where the hero landed on it and — flight 2 — **held position bit-for-bit for 9 s** while the pod flew on. Risk class **DATA**; a within-run negative control — same call, same component, `PlayersAttached` empty — ran before every call and never moved the hero. ⚠ S131's *"expect a PARTIAL dismount"* was **too pessimistic**: the teleport, un-hide, collision restore and movement restore are every one a REAL body, and the two `0xF7EC20` folds on the path are void side effects whose returns are never tested. ⚠ **Whether the hero is PLAYABLE at the landing point is still OPEN** — the single arm aimed at it was degenerate (`KFLYMODE` defaulted to `MOVE_Flying`, so the hero hovered: 2,926 uu travelled at constant Z = 13,240). | S93 / FK-22 / S130-S132 |
...
No geometry, no spawn points, no POI coordinates, no streaming layout. 1 of 91 `LVL_*` ever force-opened; the BR world (`Skylands_WP`, 2,216 packages) has never been loaded. This is the layer the DropPlane descent faults against and the reason "playable map" means "the one tutorial map." ⚠ **"the layer the DropPlane descent faults against" is the S93 marker story, and it is REFUTED (FK-22, S124) — annotated here because the same belief lived in two places** (method rule 9). [M] the three markers the Tutorial `SpawnPlane` reads **DO exist in `LVL_Tutorial`** as `Tags` entries in 3 separate WP cells, and it is `Skylands_WP` that carries **0 in 2,216/2,216 parsed packages** — the census returned the opposite of its expected answer. ⚠ Present-in-map ≠ streamed-in at call time; only the first is established. The blockers measured since are a **dead delegate subscription** (S124 §15) and a **stripped round-game-mode accessor** (S131/S132), not missing world data — see the §3.6 *Drop-in / DropPlane* row and `docs/s132-dismount-settled.md`. The rest of this item — 0 of 7,300 `.umap`, 1 of 91 `LVL_*` ever force-opened — **stands**.
...
| 0.2 | ✅ **DONE 2026-08-14 (S121)** — re-merged **all 11** dumps, cross-base included | `usmapdump mergedumps dumps/merged2.dump.exe dumps` — ⚠ write **`merged2`**, never overwrite `merged.dump.exe` (the strxref index was validated against it). The +2,044,822 B / 49.70 % prediction for the 8 same-base dumps **reproduced to the byte**; the full 11-dump merge gives **50.44 % by non-zero byte / 54.90 % by decrypted page** (15,833 → 16,625 pages) | Landed. `docs/fk18-fk19-multistate-merge-settled.md`. Ghidra re-import still pending, as is rebuilding `tools/strxref/index/` against `merged2` |
