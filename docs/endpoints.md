...
| WS | `/notifications/players/{id}` | 🧪 | messenger socket; sends binary `hb` heartbeat — we now echo `hb` back (it was closing+reconnecting every ~5s without it) |
| GET | `/party/players/{id}/voice` | 🟡 | empty → Vivox `req_account_anonymous_login failed: Access Token Service Unavailable`. Needs a Vivox token shape |
