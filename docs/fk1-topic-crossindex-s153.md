...
> **Provenance.** Produced by Workflow `wf_d7a1c52f-50d`: 8 fan-out agents (one per FK
> topic) + 1 synthesizer. 9/9 agents completed, 0 errors, 2.5 M subagent tokens,
> 4.5 min wall clock. Per-agent findings preserved at
> `scratchpad/s153_topic_crossindex_agent1..8.md`; synthesizer verbatim output at
> `scratchpad/s153_topic_crossindex_synth.md`.
>
> **Source data.** `scratchpad/s153_native_ufunction_sweep_v2.csv` (16,490 native
> UFunctions, 318 STRIPPED via v2 with capstone classifier + security-cookie fix).
> Each fan-out agent read the CSV, `CLAUDE.md`, and
> `docs/fk1-native-sweep-s153.md`, then identified which of the 318 stubs block
> its topic's critical path. Synthesizer merged the 8 topic reports.
>
> **Not live-verified.** Every "GENUINELY_BLOCKING" / "CANDIDATE_*" verdict is a
> static analysis claim. A call-and-observe pass on a running game is required
> before promoting any stub to actionable status.
