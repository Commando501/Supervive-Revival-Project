...
| 4 | **Does guarding the catalog scan remove the 15–45 s menu-route death class?** ✅✅ **CONFIRMED 2026-08-06 by a pre-registered 60-launch A/B** (`docs/s111-scanfix-ab-campaign.md`). Matched condition `-NoMissions -InjectGapSeconds 3`, 30 runs/arm, alternating batches, one boot: **scan faults 8/30 (pre-fix) vs 0/30 (fixed), one-sided Fisher p = 0.0023.** Endpoint, test and the ~30/arm requirement were all pre-registered before any data. Fix = both scans moved onto a `ReadProcessMemory`-backed `SafeCopy` (kernel-mode probe ⇒ cannot raise); ⚠ **SEH was REJECTED and this row's original suggestion to use it was the trap** — the packer's VECTORED handler runs before any SEH frame handler. Offline control `tools/sigbypass-mod/tests/scan_race_test.cpp`: old arm segfaults 3/3, new survives 3/3. ★ **The generating condition is `-NoMissions`** — the crash does NOT reproduce with the default shim set (0 faults in 6 pre-fix launches at gap 20 and gap 3), which matches the corpus, where every family-B death came from a non-default config. ⚠ **SECONDARY, UNRESOLVED: protector deaths ran 11/30 (fixed) vs 5/30 (pre-fix), p = 0.072** — the `ReadProcessMemory`-in-a-protected-process concern flagged *before* the data; total deaths are nonetheless LOWER with the fix (11 vs 14), and competing risks explain it equally well. Settle with a scan-disabled third arm. ⚠ `catalog_ready_fix.cpp` (2 sites) + `catalog_purchasable_fix.cpp` (1) still carry the defect; banner-warned, neither injected by default. | **DONE — remaining: the scan-disabled arm for the protector question** |
