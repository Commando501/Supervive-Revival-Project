...
| `0x3495973` **and** `0x349596d` | **the same** `0x3494B40` (4,336 B) | `"Ticking Group [%s] GroupLeader [%d]"`, `"Invalid position from Leader %d. Trying next leader"` | **the ~~tick task-graph dispatcher~~ [RETRACTED — it IS animation code]** — matching its `Foreground Worker #0` crashed thread |  
...
| `docs/fk7-crash-settled.md:402-417` | the 10-crash 173–201 s table, "CAMERA family" / "**ANIM family**" | Band reproduces exactly **in scope**. Two corrections: (1) **"ANIM family" is a misnomer** — `0x3495973` and `0x349596d` are the *same* function `0x3494B40`, ~~the ~~tick task-graph dispatcher~~ [RETRACTED — it IS animation code]~~ **[RETRACTED — it IS animation code]** (`"Ticking Group [%s] GroupLeader [%d]"`), matching its `Foreground Worker #0` thread; they are one family, not two. (2) 9 of the 10 rows are a single 2 h 55 m sitting — effective N ≈ 2 configurations. |
...
| 4 | **Does guarding the catalog scan remove the 15–45 s menu-route death class?** ✅✅ **CONFIRMED 2026-08-06 by a pre-registered 60-launch A/B** (`docs/s111-scanfix-ab-campaign.md`). Matched condition `-NoMissions -InjectGapSeconds 3`, 30 runs/arm, alternating batches, one boot: **scan faults 8/30 (pre-fix) vs 0/30 (fixed), one-sided Fisher p = 0.0023.** Endpoint, test and the ~30/arm requirement were all pre-registered before any data. Fix = both scans moved onto a `ReadProcessMemory`-backed `SafeCopy` (kernel-mode probe ⇒ cannot raise); ⚠ **SEH was REJECTED and this row's original suggestion to use it was the trap** — the packer's VECTORED handler runs before any SEH frame handler. Offline control `tools/sigbypass-mod/tests/scan_race_test.cpp`: old arm segfaults 3/3, new survives 3/3. ★ **The generating condition is `-NoMissions`** — the crash does NOT reproduce with the default shim set (0 faults in 6 pre-fix launches at gap 20 and gap 3), which matches the corpus, where every family-B death came from a non-default config. ✅ **SECONDARY NOW CLOSED by a scan-disabled third arm** (`docs/s111-scanfix-armc-control.md`, 30 more launches, 90 total): protector deaths **A 11/30 (37 %) vs C 9/30 (30 %), p = 0.392** — removing the scan ENTIRELY does not lower the protector rate, so **`ReadProcessMemory` is exonerated**. Arm B's low 5/30 was **competing risks**, not safety: 8 of its runs died of the scan at 12–33 s before the protector could act (C vs B p = 0.180). Net: the fix eliminates 8/30 self-inflicted deaths and adds none; total deaths 14/30 → 11/30. ⚠ `catalog_ready_fix.cpp` (2 sites) + `catalog_purchasable_fix.cpp` (1) still carry the defect; banner-warned, neither injected by default. | **DONE — 90 launches, both endpoints settled** |
| 5 | **Which of A/B/C actually fires the protector kill** — the self-restoring `.text` jz-NOP, the manual-mapped images, or the PI prologue writes? ✅ **HALF-CLOSED 2026-08-06** (`docs/s111-nohook-control.md`): the `-NoHook` run this row asked for has now been flown — **11 launches × 320 s, 0 protector deaths**, vs **25/90 (28 %)** across the injected arms (p = 0.036) and **9/30 (30 %)** for arm C (p = 0.041). ★ **The protector kill is caused by OUR INJECTION, not by the game.** Arm C isolates it cleanly: arm C also leaves the roster/store unpopulated, so the discriminating variable is the injected DLL itself, not workload. ✅ **Arm D flown 2026-08-06** (`docs/s111-armd-nulldll.md`): ONE completely inert manually-mapped DLL, **11 launches × 320 s, 0 protector deaths** — identical to `-NoHook` (0/11), vs arm C 9/30 (p = 0.041). **Manual mapping itself is EXONERATED**; the pessimistic reading (that the fix needs a whole new loading strategy) has lost its evidence. ⚠ Arm D moves TWO variables vs the injected arms — inert-vs-active AND 1-vs-5 images — so it clears *one inert image*, not five. ★ **Next is one line and fully controlled: arm E = `-Hook catalog_store_fix_noscan.dll`** (that build already exists — it is arm C's primary), which does the `.text` jz-NOP and nothing else at arm D's image count. E vs D is a true one-variable test of the patch. ✅✅ **ARM E FLOWN 2026-08-06** (`docs/s111-arme-shim-activity.md`): ONE `catalog_store_fix_noscan` mapped alone, **8 launches × 320 s, 7 protector deaths (88 %)** vs **arm D 0/11 at the identical hold and image count — p = 0.00016**. ★ **The trigger is the primary shim's own worker activity, not mapping and not image count.** ⚠ Arm E's 88 % is NOT a higher hazard than the 5-DLL arms' 28 % — that is a hold-time artifact (320 s vs 60 s); within 60 s arm E is 1/8 = 12 %, p = 0.92 vs pooled. ⚠ Arm E still leaves FIVE behaviours running: marker I/O, `SnapshotModules`, **`AddVectoredExceptionHandler`**, **`BuildStub` exec allocation**, and the **slot-110 vtable hook + `.text` jz-NOP**. Next: bisect with `KNOVEH` / `KNOSLOT` / `KNOJZ` at 320 s holds, ~10 runs/arm — and note all three are droppable in production (the VEH is only a diagnostic logger, the slot hook fires the AssetManager scan once, and the jz already has the data-only `[+0x354]` poke as an alternative). ★★ **BISECT COMPLETE 2026-08-06** (`docs/s111-bisect-jz-is-the-trigger.md`, 17 launches): **the `.text` `jz`-NOP is the trigger.** One-variable builds at 1 image / 320 s — `KNOVEH` (no VEH) died 2/2, `KNOSLOT` (no exec stub, no vtable hook) died 2/2, **`KNOJZ` (no `.text` write) died 0/5**; patch-standing 11/12 vs no-patch 0/5, **p = 0.00097**. This is CLAUDE.md's oldest rule ("no standing `.text` patch") finally measured one-variable. ⚠ It also exposes a flaw in arms C/E/E1/E2: `catLoadedAt` is set only on a successful scan (`:501`) and the self-restore is gated on it (`:512`), so under `KNOSCAN` **the restore never fires and the patch stands all run** — they were testing "permanent patch", not the shipped shim, which is why arm E hit 88 % vs production's 30 %. ★★ **One mechanism explains the whole ladder**: rate tracks how long a `.text` modification is left standing (`-NoHook` 0 %, inert 0 %, production patch-restored ~30 %, patch-standing ~90 %) — and arm A's 11 deaths (12/24/24/24/27/27/27/33/33/36/61 s) fall inside its computed 5–45 s patch window (INFERRED). ★ **NEXT, one line:** drop the `jz` patch and rely on the `[+0x354]` **data** poke the shim already performs — a heap write is not a `.text` modification, so if roster/store still populate the entire death class disappears. Manual-map / `.text` jz-NOP / PI prologue writes are still confounded — arm C both mapped a DLL and patched the jz. Next: a one-variable ladder D(map only, do-nothing DllMain) → E(+jz) → F(+PI), ~10 runs each. **Arm D is the most informative single run left**: if a do-nothing DLL already dies at ~30 %, the protector reacts to manual mapping itself and the fix must be a different loading strategy. ⚠ Also: the ~285 s code-integrity kill did NOT fire in 11 runs that all crossed it — first direct support for 'it catches a STANDING `.text` patch' rather than a timing inference. | **~30 min for arm D** |
...
| 10 | **Index `dumps/tutorial-hero/SUPERVIVE-Win64-Shipping.dump.exe`** (captured **in-world** 2026-08-05 19:13 — the first such image the project has) with strxref. ⚠ **UPDATED 2026-08-14 (FK-19): do NOT index it standalone — `mergedumps` now folds it in.** It contributes **570 `.text` pages `merged.dump.exe` lacks**; index `dumps/merged2.dump.exe` (16,625 pages, 54.90%) instead. Several `func` queries currently return "body in a non-decrypted page". | ~20 s per rebuild |
