...
| c | CLAUDE.md lists extractor subcommands *"enumerate, names, namesall, dump, raw, schema, assetregistry"* | `CLAUDE.md` Tooling | `Program.cs:22` accepts `dump, names, namesall, schema, assetregistry, wherefile, mkpak, peekpak, bpdump`. There is **no `raw`** and no `enumerate`; `bpdump` (which drove several breakthroughs) is undocumented. **Load-bearing:** audit items 0.4 and 1.8 both assume a raw-file path out of IoStore that must be *written*, not invoked. | minutes |
...
| 26 | Diff the two `schema.txt` files; dump the third usmap's schema; record the canonical MD5 | FK-14 |
...
| unopened-artifacts | "Is `Saved/Engine.ini bVerifyPeer=false` ours?" | ACTUALLY_KNOWN — `launch-redirect.ps1:271-284` writes it and `:167-173` strips it |
