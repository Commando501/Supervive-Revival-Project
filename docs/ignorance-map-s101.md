...
> ★★ **S136 (2026-08-21) — AN AI-CONTROLLED HERO PAWN EXISTS, AND IT IS **NOT** A BOT.**
> `SpawnAIFromClass` → `APawn::SpawnDefaultController` (slot 280) builds a **generic engine
> `AAIController`** that possesses a spawned `BP_HERO_Ronin_C`; the handshake is bidirectional and
> `AController::Possess` ran. **[M] `obj_by_chain BotController` = 0 LIVE** and `AIControllerClass`
> is the engine default **on the player hero too**, so `MakeNewBotController → BotController →
> ServerSetHeroClass / SetPlayerTeam` is UNTOUCHED. **Any statement below that the AI-controller
> path is unreachable is HISTORICAL** — but do not upgrade it to "bots work".
> ★★★ **The next blocker is measured and it is ONE CDO BIT, not a stripped stub:**
> `AController::InitPlayerState = 0x36DEE20` (slot 273) is **REAL, 778 B, 14/14 callees REAL, zero
> folds**; it never runs because **stock UE clears `bWantsPlayerState` (bit `0x20` @ `+0x488`)** in
> `AAIController`'s and `ALokiBotController`'s constructors. ⚠ A **second** wall sits past it (two
> stripped folds). Start at `docs/s136-ai-controller-settled.md` — **its CORRECTIONS block governs
> the rest of that file** — then `docs/next-session-prompt-s137.md`.
> ★★★ **S139–S141 (2026-08-23/24) — THE ENGINE MOVER CHAIN RUNS, AND AN AI-CONTROLLED HERO PAWN
...
> | **FK-21** — "Career→Stats/→Ranked/→History show authentic empties" | **S123** | ✅ **CLOSED — belief FALSE in the same direction on all three panels.** Each was empty because we served nothing, and each renders the moment it is fed: Stats **S121**, Ranked **S122**, History **S123** (`VICTORY · Basic Training · 1/16`, hero portrait, ALLY row, full stat panel — screenshot). Backend-only, four arms on one client up 3h15m, **no relaunch**, canaries 0/0/0/0. ⚠ Sharpen the failure: S119 had already excluded "broken deserialization" for History via the `+0x68` Version discriminator; what was never shown until now is that the panel is **live**. ★ Two findings that generalise: **`TeamInfo.Placement` is 1-INDEXED while its sibling `FPlayerHeroStats.Placements` is ZERO-indexed** (same backend, opposite conventions — carry neither across), and **the damage tiles read `Effective*`, never the raw `Damage*`** (5/5, distinct values; the giveaway was healing rendering while damage read 0, because healing has no `Effective` variant). ⚠ Two instrument artifacts caught pre-publication, one a NEW variant: **the grep window is part of the instrument** — `-A 3` paired a request with a neighbour's `User-Agent`. `server/internal/interactive/matchhistory.go`, `tools/re/matchhistory_readout.py` |
...
> | **FK-40** — S122 activity/queue false-knowns (batched) | **S122** | ✅ **NEW + ALL FIVE FALSIFIED, zero launches.** ★★★★★ The headline is a **workaround that hid its own defect for ~60 sessions**: the S60 queue trim stopped `"Unable to modify activity"` — but only because it deleted the level-*gated* queues, leaving nothing lockable to click. Its stated mechanism (a `CanControlQueue` loop over queues) **does not exist in the bytecode**. Removing the trim exposed the actual residual cause, `POST setTargetQueues`, which had **no handler** and was *unobservable* while one activity was selectable. ⚠ A **third feature-toggle key category** was found — keys assembled at RUNTIME (`"queue.restrictions."+QueueID`), invisible to both S121 censuses — and **flown**: it unlocks ARENA, and withdrawing it reproduces the S60 error string verbatim. ⚠ Two of the five dead beliefs were manufactured by S122 itself, incl. *"S120 serves `AccountPass.Level = 10`"* (the live server serves 0) committed to `CLAUDE.md` and a commit message before being checked. `docs/s122-player-rank-career-badge.md` |
...
> | **FK-44** — S135/S136 bot-controller false-knowns (batched) | **S136** | ✅ **NEW + NINE FALSIFIED, and the most expensive one was an ARM THAT COULD NOT RUN.** ★★★★★ **(a) The `botai` arm S135 built, digested, documented and handed off as flight-ready contained NO CALL SITE** — `BsResolve` returns early under `#if KBSAI` so every `g_bsFn`/`g_bsComp` store is unreachable, clang `-O2` folded the dispatch guard to constant-true, and `BsCallAI()` was **dead-code eliminated**. [M] the 53-byte banner literal had **0** byte occurrences against a passing positive control. It burned a staged tutorial world proving it. ⚠ The mechanism is **NON-UNIQUE** — either guard term alone folds true. **(b) Its runtime VERDICT LIED**, asserting `(KBSARMS gated it off)` while `KBSARMS = 0x0F` had bit 2 SET — a hardcoded cause on a generic failure path. **(c) The census predicate was BLIND to the very controller the route creates** (`BotController`-chain only; an engine `AIController` chain contains no such substring), so flight 2's `dCtl=0` was **UNINTERPRETABLE, not a null** — recorded as P1 it would have published *"the controller is unreachable by ANY spawn entry point"*. Settled S135's own deferred blind spot: same world, same object, only the predicate changed ⇒ **0 vs 1**. **(d) "There is no canonical `.text` digest recipe on disk"** — FALSE; **two** exist (`fk24-stage.ps1:77` + `fk7-ab-run.ps1:94` RAW, which *emits* it to a CSV; `method-rules.md:213` VIRTUALSIZE), and the root cause is a **broken pointer in `CLAUDE.md`** to `verify_dll.py` (0 hash lines) and `s109 §23` (no snippet) that two sessions followed. **(e) S136's own retraction:** *"the four S135 bot digests appear nowhere in the repo"* — FALSE, all four are recorded; the error came from a `grep -rl` that **TIMED OUT at 2 min** and whose partial output was read as a negative. **(f) Two of S136's own supports were worthless**: `Z = 90.15` as a payload fingerprint (floor flatness — the strong form is bot2 resting ON bot1, ΔZ = 2 × 88.0504) and `Outer = PersistentLevel` (98.8 % of live actors). **(g) "The KBSARMS banner did not print" as runtime evidence** — [S], constant-folded, absent BY CONSTRUCTION; an instrument artifact committed in the section describing them. **(h) A lane's "third gate"** at `[PS+0x8C8]` — REFUTED; `+0x8C0` is `PlatformPlayerID`, not PlayerName (`+0x450`), so the block RUNS. The lane inferred the field from the write it saw rather than the property table it had open. **(i) `play-nopimutex` and `play-strictroot` are DEGENERATE CONTROL ARMS** — byte-identical to `play`, their knobs inert since **S112** and **S123**, each shown with a passing positive control. **Flying either measures nothing.** ⚠ Also: `InternalIndex` is **not monotone** (slot reuse) and must never be used for creation order — the real oracle is **`FName.Number` (obj+0x24)**, a strictly-decreasing spawn counter the repo's own tools discard. ★ And `dumps/s133-phase2-{BASE,AFTER}` are mis-named (content confirms the swap) but **no published S133 conclusion is wrong**. `docs/s136-ai-controller-settled.md`, `docs/next-session-prompt-s137.md` |
> | **FK-45** — S137 PlayerState / Loki-bot false-knowns (batched) | **S137** | ✅ **NEW + TWELVE FALSIFIED, and the most expensive one was THE HANDOFF'S OWN HEADLINE ARM.** ★★★★★ **(a) `docs/next-session-prompt-s137.md` §1.2 — "poke `CDO(AIControllerClass)+0x488 |= 0x20` before spawning … the same risk class as S130's `bCanEverReplicate`" — is MEASURED REFUTED.** The poke lands (readback OK, dword delta exactly `0x20`) and the spawned instance still reads the bit CLEAR, on a within-run control of two spawns milliseconds apart. **An offline lane predicted it BEFORE the flight and named the mechanism:** `FObjectInitializer::InitProperties` branches into the **PostConstructLink** chain only, and `UStruct::Link` never puts a property **owned by a NATIVE class** into that chain; there is no bulk CDO→instance memcpy on the allocation path. ⇒ **the reusable rule: a CDO poke reaches a new instance ONLY IF THE CONSUMER READS THE CDO DIRECTLY.** S130's precedent never established otherwise — its consumer read the CDO *directly* (`cmp byte [CDO+0x6C],0`) — and the handoff generalised it to a case it does not cover. ★★★ **The same session then applied the rule CORRECTLY and it worked**: `Default__Pawn+0x3D0` IS read by hand in `APawn::APawn` (`0x3B80C0D`), so poking it makes `SpawnDefaultController` build an **`ALokiBotController`** — full A-B-A, two instruments, the first `BotController`-derived object in this project's history. **Same idea, opposite outcome, and the difference is who reads the CDO.** **(b) "THE BLOCKER" (singular) — there are THREE gates** in `AAIController::PostInitializeComponents` (stock UE's `bWantsPlayerState && !IsPendingKillPending() && GetNetMode()!=NM_Client`); the handoff records only the first, so a successor would not know to check the others. **(c) "gates 2 and 3 are already known to hold" was [I], not [M]** — the existing netmode measurement is on the PAWN, gate 3 tests the CONTROLLER. Caught by adversarial review; settled anyway from another direction (ARM B ran 3× through the same guard on a controller). ★★★★★ **(d) `[M] obj_by_chain BotController = 0 LIVE (the CDO is present as a passing search-term control)` IS A DEGENERATE QUERY.** No class in this hierarchy is *named* `BotController`; run live **with a `LokiBotController` possessing a pawn in that very process**, `=BotController` returns `found 0` **AND `CDOs matched and EXCLUDED: 0`** — i.e. **it has no positive control either**, so the stated control cannot have passed. `=LokiBotController` returns `found 1`. **The `'='` exact form the tool's own header tells you to PREFER is the wrong instrument for this question.** ⚠ S137 shipped the SAME defect in its own new probe and fixed it. **(e) The `62 vs 69` AController fold-slot "discrepancy" is not one** — 42+17+3 = 62 under the four-fold set, +7 (`0xB9E1F0`) = 69 under the five-fold set. **Quote the fold set with the number.** ★★★★★ **(f) THE NETMODE OPEN QUESTION — recorded TWICE and never actioned — IS ANSWERED BY ONE GREP, AND THE ANSWER HAS BEEN IN EVERY LOG THIS PROJECT EVER TOOK:** `LogWorldPartition: … World NetMode = Standalone, IsServer = 0` — present in this session's log AND in `Loki-s124`/`s125`/`s127`. Corroborated from the bytes (`AActor::GetNetMode 0x338E750` defaults to `xor eax,eax` = 0). A textbook **method-rule-#2** instance. ⚠ **It moves NO wall** — FK-1's empty impls and the exec-pin gates never consult UE's netmode; it upgrades a FRAMING that was already [I]. **(g) "no `TerminateProcess`/`ExitProcess` in any shim source"** is true of the SOURCE and false of the **import table**, where both appear in every `tutorial_launch` DLL incl. the clean-flying `play` and `dismount` (clang/link.exe scaffolding). Conclusion unaffected, support misleading. **(h) A verification lane cited tooling at `scratchpad/lanev3/` that DOES NOT EXIST** (positive control: three sibling scratchpad paths found by the same `find`) — the S131 defect, recommitted; its eight offsets were all independently reproduced by the refuter, so the conclusions held and only the provenance was fabricated. **(i) A lane's evidence line contained an ADDRESS ARITHMETIC ERROR** (`ImageBase + 0x45CFA10` written as `0x7FF6AF45CFA10`, correct value `0x7FF6B35CFA10`); the scan it claimed to have run returns 0 hits as written. **(j) S137 DESTROYED ONE OF ITS OWN EVIDENCE FILES** by re-running a probe with `| tee` after the client had died, overwriting the capture with `RUN IS VOID`; rewritten as a labelled transcript reconstruction. **Do not `tee` over an evidence file.** **(k) `mergedumps` printed "44 pages"** for what it took from donors while the coverage gain over `merged12` was **+28** — different quantities, and only the second is a coverage claim. **(l) S137's own `BsPsPrecondition` World latch is unreliable** — it takes the FIRST object whose class leaf name is exactly `World` and read `AuthorityGameMode = 0` **while `InitPlayerState` was concurrently succeeding** (345 live objects have `World` in the class name). It cost nothing only because it flagged itself ("a null below is UNINTERPRETABLE"). **(m) S137 REFUTED ITS OWN CLAIM WITHIN THE SESSION:** on flight 3's `0xDEAD` death it wrote *"repeated injection into one process appears to accumulate FK-32 risk"* (citing S132's kill on its 7th injection). **Flight 4 died at the 4th manual-map after 334 s** vs flight 3's 6th after 1144 s. Across the three recorded kills the counts are **7 / 6 / 4** with unrelated elapsed times ⇒ **no dose-response; "accumulates" is unsupported [I, weak].** Written from ONE death, refuted by the NEXT one, ~40 minutes later. **(n) A throwaway probe reported an ARTIFACT AS A NULL** — the movement read printed `UNREADABLE` for all three pawns because the client had already been killed, and **it had no RUN-IS-VOID liveness check**; `playerstate_readout.py`, written the same day, has one and refuses. **The discipline has to be in the scratch tool too, not just the shipped one.** ★★★ **AND THE SESSION'S BIGGEST POSITIVE CAME WITH A TWO-SIDED WITHIN-RUN CONTROL:** the `LokiBotController` builds its own `BehaviorTreeComponent` (`NodeInstances=21`) and `BlackboardComponent` (real `BlackboardData`, `KeyInstances=15`) while **both** plain `AIController`s spawned by the same code milliseconds either side read **NULL** for both, with `PathFollowingComponent`/`GameplayTasksComponent` present on all three as the shared baseline. ⚠ It shows the tree was BUILT, **not that the bot acts** — movement was never measured, and `LogBehaviorTree` = 0 has **no positive control** and is UNINTERPRETABLE. **(o) ⚠⚠ TWO OF S137'S OWN OFFLINE LANES CONTRADICTED EACH OTHER, AND THE LIVE MEASUREMENT WAS THE TIEBREAKER.** One transcribed `ALokiBotController::OnPossess` as calling `RunBehaviorTree` via vtable disp `0x940`; the other concluded it *"calls neither `RunBehaviorTree` nor `UseBlackboard`"* and therefore that the S137 bot *"was ticking with a NULL Blackboard"* — which would have made its whole tick analysis a statement about a crippled bot. **REFUTED three ways:** the bytes (`0x55655F6 call qword [rax+0x940]` — the dispatch is **INDIRECT** and the components are created in the **CALLEE**, so a scan for a *direct* call or for *direct* writes to `+0x498`/`+0x4B0` inside `OnPossess` is **blind BY CONSTRUCTION**); the adversarial pass, unprompted (*"THE NEGATIVE HAS NO POSITIVE CONTROL THAT COULD HAVE FIRED, AND THE THING IT DECLARES ABSENT IS PRESENT"*); and the live component table. ★ Naming closes from both ends — the disp-`0x940` callee loads the wide literal `BTComponent` and the live object is **literally named `BTComponent`**. ⚠ `0x3316AF0` is NOT `RunBehaviorTree`'s body but a **4-way ICF-folded dispatch shim**, so the winning lane's *stated* naming chain was itself the folded-RVA error and was correctly DOWNGRADED. **(p) A LANE CLAIMED `SpawnBot` IS CALL-ONLY; ITS REFUTER REFUTED IT** — an exhaustive operand scan finds **7 non-stack writes across FOUR objects**. Also corrected: 39/25/22 → **43 call instructions (39 direct + 4 indirect), 28 distinct direct targets, 25 REAL / 2 FOLD / 1 DARK**. **(q) A LANE CLAIMED A "SECOND GATE THE S136 WRITE-UP NEVER RECORDED" — REFUTED, and so was MY OWN FIRST DRAFT OF THE SAME CORRECTION.** `docs/s136-ai-controller-settled.md:524` records gate 3 verbatim. The true and more useful finding: **gate 2 was never recorded anywhere, and gate 3 was recorded in the settled doc and DROPPED when it was compressed into the handoff** — i.e. **the digest is the instrument that lost it**, this repo's own S115-d pattern operating one level down (settled-doc → handoff), against a reader who by definition has only the handoff. **(r) ⚠⚠★ WE KILLED ONE OF THE REPO'S OWN NEGATIVE CONTROLS, AND ONLY A RE-GRADE SWEEP CAUGHT IT.** `docs/fk22-dropphase-reachability.md` designated `ALokiGameState::AuthSetDeathCircle` impl `0x55653E0` as FK-22's **coverage negative control** ("0/4096 non-zero in 13/13 images"). It shares `.text` page `0x5565000` with `ALokiBotController::OnPossess` `0x5565470` — **0xB0 bytes away** — so S137's own bot flight decrypted it as a side effect. It now reads **3,782/4,096** with a real `jmp 0x338C990` at its entry, having been 0/4096 in `merged2`, `merged10` AND `merged12`. **Nothing about the drop path changed; a neighbour ran.** ⇒ **a coverage control is only valid until something on its PAGE runs** — choose one with no plausible neighbour, re-verify before each use, and name the image. A SECOND control was independently dead: `docs/fk-playability-audit-s134.md` offers `0x5A6AC40` **or** `0x556D910`, and `0x556D910` (SpawnBot) has been LIT since `merged12`. **(s) THE RE-GRADE SWEEP ITSELF: 43 stale claim-instances / 15 files / 14 RVAs**, reproduced row-for-row by an adversarial pass — but the shape is the opposite of S133's: **only 3 first went stale in `merged13`; 29 have been stale since `merged10`.** ⇒ **the problem is not missing captures, it is that `regrade_blocked.py` has been emitting these for sessions and nobody edits them.** ⚠ A lane's claim that three were *"never flagged by any prior audit"* is **REFUTED** — the S133 tool emits all three verbatim. **Four sat in `CLAUDE.md` itself, two contradicting the same file elsewhere**; all four now annotated. ⚠ The sweep is a **FLOOR** — 294 of 431 keyword lines carry no same-line address and were never graded. **(t) THE 'AUTHORITY-TAKEN BRANCH ⇒ CLIENT-ONLY ARM' READING IS WRONG, AND ITS FLAGSHIP COUNTER-EXAMPLE IS `AController::Possess`.** At `0x36E2B99` the `je` on `Role==3` jumps to `0x36E2E13` — the **main body**, which reads `Pawn+0x3F8` and dispatches `call [rax+0x868]` = slot 269 `OnPossess`; the FALLTHROUGH is the warning. Stock `if (!bCanPossessWithoutAuthority && !HasAuthority()) return;` ⇒ **authority is exactly WHY S137's possession works**, and it retires a pre-flight question ([M] by consequence: the gate passed on our controller). **Semantics require reading the branch TARGET, not the condition.** Derived independently twice — by the session lead and by a lane — and both prior counts of the 'hazard' (14, then 21/23) are REFUTED: the census is **145 sites, 32 authority-taken**, and the lane's own '17 enabling' partition was refuted for not summing. ★ **Follow-up adversarial totals: 64 CONFIRMED / 25 DOWNGRADE / 10 REFUTED** across 4 more lanes, then **41 / 17 / 5** across 4 more. ★ **Adversarial totals across the first 10 lanes: 79 CONFIRMED / 36 DOWNGRADE / 13 REFUTED** — every claim the flown arms depended on survived; what fell was supporting apparatus. `docs/s137-playerstate-and-lokibot-settled.md`, `docs/next-session-prompt-s138.md` |

> | **FK-46** — S138 bot-motion / LivingState false-knowns (batched) | **S138** | ✅ **NEW + NINETEEN FALSIFIED OR CORRECTED, across nine flights, and THREE of them were instruments I wrote during the session.** ★★★★★ **(a) THE PRE-REGISTRATION'S OWN RULE U2 IS REFUTED, structurally AND empirically.** *"a non-null return with `0x5556D50` STILL DARK ⇒ execution did NOT pass the PlayerState gate"* is a **non-sequitur**: machine-exhaustive disassembly by two independent parties shows all three PlayerState gates (`0x556DD73/83/90`) jump **FORWARD to `0x556DE58`**, and the E5 receipt sits `0x4A` bytes PAST that join behind three further guards. Then measured: the gate WAS passed (`[PS+0x8C8]` 0→5, string `'bot0'`) and the page is **still dark**. ⇒ the divert is **`0x556DE6A`** (`GetTeamState` NULL). **A pre-registered rule can be wrong; write them so they are falsifiable.** ★★★★★ **(b) I PUBLISHED A CLAIM READ OFF THE WRONG OBJECT.** `docs/s138-flight2-arme-fired.md` cited *"`ALokiGameState.TeamStates (+0x600) Num = 0`"*; the marker actually said `LokiGameState=0x… **'LokiGameStateUAVComponent'**` — an **ActorComponent** — because `BsClassify` latched it with `PhChainHas` (**`strstr`**, first match) and `"LokiGameState"` is a substring of `"LokiGameStateUAVComponent"`. **I quoted the line with the class name STRIPPED**, so the one field that falsifies the read was removed in compression — the *"a digest is an instrument"* pattern (S115-d) at the doc level. Both G3 and G4 were VOID. **Fixed** (exact chain match + all candidates + full chains + a `World->GameState` cross-check + an explicit NOT MEASURED state), and the fix's own output then exposed **(c) a duplicate-candidate defect** — `BsScanWorld` runs THREE passes and the latches were never reset, so one object was counted once per pass and tripped a **false-positive AMBIGUOUS warning**. Fixed with a per-pass reset AND dedupe-on-insert, verified by a standalone 10/10 logic test, then **flown: 5/5 predictions incl. `candidates: 2 → 1`**. ★★★★★ **(d) MY AUTO-STAGE DRIVER KILLED THREE PERFECTLY STAGED CLIENTS AND NEARLY BECAME A FALSE FK-31 FINDING.** It tested the marker for `[SP] done step=4` **immediately** after `fk24-stage.ps1` returned — but `stage complete` means *"finished INJECTING sp"*, and sp writes its receipt **12 s later** (measured on the next run). It `Stop-Process`'d three staged clients, and I was **one attempt from recording a fourth consecutive FK-31 death — the exact p=0.005 threshold the crash classifier had itself nominated for calling FK-31 systematic.** Caught only by reading the stager's own log, which plainly said `stage complete`. ⇒ **a completion message from a launcher means "I finished my step", NOT "the injected code finished its work". Gate on the payload's own receipt.** ★★★★★ **(e) A VERDICT LINE IN MY OWN TOOL PRINTED A FALSE "YES".** `livingstate_poke.py` reported `P3 gate flipped 0→1 : YES` while its own samples, six lines above, showed `GATE=0` at every timepoint — the predicate `a[1]==0 and any(True for _ in [1]) and c is not None` has **two always-true terms** and collapsed to the experiment's own precondition. A **degenerate always-true guard**, S136's constant-folded dispatch guard relocated into a verdict line. **Read the samples, not the verdict.** ★★ **(f) A HARDCODED OFFSET THAT "AGREED" WITH A BY-NAME READ WAS STILL WRONG** — a velocity series hardcoded `CMC+0xE0`; the by-name resolve is **`+0xE8`**. Both read zero, so the agreement was worthless as corroboration. The conclusion survived (re-measured by name over 25 samples) but `docs/s138-f9-velocity.txt` carries a wrong-offset column. ★★★ **(g) THE FK-31 "NEW ERAS" WERE NOT NEW — and I SEEDED THE ERROR.** A corpus sweep reported discovering kill addresses `0x7FF8F0400001` / `0x7FF90E000001`; `docs/fk8-crash-clusters.md` §4.1 (committed **18 days earlier**) already contains both, **plus a sixth (`0x7FFB9EE00001`)**, the ntdll-base-as-boot-key method, and the crashpad-vs-UECC asymmetry named as an instrument artifact. **I briefed the lane from `CLAUDE.md` without grepping `docs/` first.** Method rules #2 and #9. Corrected corpus: **six addresses, 385 files / 128 distinct reports**; **five FK-31 reports are invisible to a minidump-only instrument** (0-byte `UEMinidump.dmp` with a readable sibling XML). ★★ **(h) MY OWN "NEW INSTRUMENT" CLAIM WAS REFUTED.** I wrote that the crashpad `MemoryInfoListStream` is *"a third, purely offline route to the protector signature"*. It **tiles the entire user address space** (0 gaps; a fabricated address gets a hit too), so "an entry covers the fault" is **vacuous** — only the SHAPE discriminates — and **34/34 non-kill dumps carry the identical shape**, because the protector is always mapped. It also carries **no module name**, so it is a joint inference with the UECC ModuleList, not an independent route. ★★ **(i) THE STANDING FK-31 EXPERIMENT'S PAYOFF IS FORECLOSED.** `CLAUDE.md` promises that mapping an executable page at the kill address may let the process survive *"and the stack names the caller"*. **111/111 FK-31 reports have `[rsp]` = `KERNEL32!BaseThreadInitThunk` with nothing below** ⇒ a `ret` returns into the thread entry and names nothing. Only the *does-it-survive* half is live. ★★★ **(j) `TeamStates` CAN NEVER BE NON-EMPTY ON A CLIENT [M]** — `GetOrCreateTeamState` (impl `0x5634BD0`) is 16 bytes that call `GetWorld`, **discard it**, and `xor eax,eax; ret`; `SetNumTeams` is the void fold; no native writer, no BP access, AS read-only, 0 of 83 PROPS files. ⇒ anything past `0x556DE6A` is unreachable. ⚠ **A SIXTH STUB SHAPE** (`sub rsp,0x28; call <GetWorld>; xor eax,eax; ret`, 5 sites) **grades REAL under a two-state fold test and is not DARK** — only reading the instructions is correct. ★★ **(k) `SpawnBot 0x556D910` IS `PAGE_NOACCESS` UNTIL IT HAS EXECUTED**, so ARM E's prologue-signature gate was **unsatisfiable by construction** and the arm could never have fired. `merged12`/`merged13` grading it LIT is a **UNION across processes**, not a live state. Driving any path that calls it (inject `botspawn`) decrypts it — **6/6 pre-registered predictions, twice.** ★ **(l) FOUR SUCCESSIVE BOT-MOTION HYPOTHESES REFUTED BY MEASUREMENT**: the gate/`LivingState` (it opens); `MovementMode` is wrong (**bot and player are BOTH `MOVE_Falling`**); forcing `MOVE_Flying` (poked, held 25 s, no effect); the component is deactivated (`bIsActive=True` on both). ⚠ **(m) `ELokiLivingState` DOES NOT EXIST** (0 occurrences, passing controls) — it is `ELivingState` (**Dead=0 Alive=1**), and a *different* enum `EPlayerLivingState` has **Alive=3**; carrying the value across is wrong by two. ⚠ **(n) `merged13` is NOT a strict superset** — 2 pages are lit only in the S138 captures. ⚠ **(o) A coverage bound EXCLUDED ITS OWN EVIDENCE** (the "91.7 % of the `ALokiCharacter` surface" figure does not contain the page holding one of the two writers it was used to bound). ⚠ **(p) `OnRep_LivingState` was graded REAL but is a VIRTUAL-DISPATCH SHIM** (real body at vtable disp `0xC60`) — the exact error class the brief warned about. ⚠ **(q) "the only engine writer is `FRepLayout`" is FALSE** — native C++ writes `+0x1090` directly (both writers store 0); correct form is *the only writer through the reflection system*. ⚠ **(r) `SetTowardAliveState` / `SetTowardDeadState` DO exist**, refuting "zero `Set*` functions" — they are ICF-shared pointer setters for the state **objects**, and a type-based sweep on `ELivingState` **cannot** see a setter that takes no such param, so it is not a completeness argument. ⚠ **(s) `botspawn`'s recorded digest was STALE by ~17 h of source drift** (`b2203efd62161182` now); the flown DLL was not a HEAD build. ★★★ **AND THE SESSION'S POSITIVE, WITH CONTROLS AT EVERY STEP:** `SpawnBot`'s **premade path runs** (`MakeNewBotController`, and with it FK-22's stripped getter, never called; `OnUnPossess` page DARK→LIT); `UpdateCharacterControllable` **opens the gate** from `LivingState=Alive` (treatment vs a compiled-out control arm); and the bot's **AI runs end to end** — 44 distinct horizontal unit directions over 97 s / 194 samples, `ControlInputVector` matching in **193/194** across two different objects. **The pawn still never moves**, and the wall is now the movement component's tick. ⚠ Grade the whole thing honestly: **none of it happens without pokes the game never performs itself**, and `ServerSetHeroClass` / `SetPlayerTeam` remain stripped folds. `docs/next-session-prompt-s139.md`, `docs/s138-flight9b-flymode-refuted.md`, `docs/s138-livingstate-writers-settled.md`, `docs/s138-offline-followup.md` |
> | **FK-47** — S139–S141 movement-arc false-knowns (batched) | **S140–S141** | ✅ **NEW + SIX FALSIFIED, and one of them was the founding premise of three sessions.** **(a) `CMC+0x16C8` IS NOT A STICKY LATCH** — the belief that `latch == 0` proves `ULokiCMC::StartNewPhysics` never ran. `ULokiCMC` vtable disp `0xA50` = **`0x0530ABF0`** (`80b9c816000000 / 7407 / c681c816000000 / e98bbb2cfe`) **clears** it, and engine `PerformMovement` calls that slot at **`0x035EB569`** with `rcx = this` — later in the same call, on a path the `StartNewPhysics` call site **DOMINATES**. ⇒ it reads `0` whether the step runs every frame or never. **Named from its own consumer:** `.data 0x09BC9AD0 = {"GetRecentVelocity", 0x0530C7E0, 0x0530AC10}` → `cmp byte [rcx+0x16c8],0 / mov eax,0x16b0 / mov r8d,0xe8 / cmove eax,r8d` = a per-frame `TOptional<FVector>` validity flag over the `+0x16B0` Velocity snapshot. Derived independently **three times in one session**. ⚠ **The MEASUREMENT was correct** (it really did read 0 on all 37 components); only the **inference** is dead, and everything resting on it is **UNGRADED, not negative**. ★ Root cause (`S140T1-a`): **the field was named from the site that SETS it and nobody enumerated the sites that CLEAR it.** ★★ A free coherence check was skipped — 37/37 zeros means *nothing in the world can move at all*. ★★★ **The doc's own pre-registration caught it and was overridden**: `docs/s139-f1-BOT.txt` prints `THE BISECTOR IS UNINTERPRETABLE THIS SITTING` **34 lines below** the number that became `[M] banked` (`S140T1-g`). **(b) "the player does not fall" — IT WAS OURS.** `sp`'s LIFT-TO-SEE step (`tutorial_launch.cpp:12877-12890`) sets `GravityScale = 0`, and **`CMC+0x1A0` IS `GravityScale`** (engine `GetGravityZ 0x035E3680 mulss xmm0,[rbx+0x1a0]`, three instruments). `docs/s138-flight9-movement-not-simulating.md:17` had recorded **BOT 1.000 / PLAYER 0.000 the day before**. S132's dismount is explained too — it was a `GravityScale` **restore**, which is why that hero fell with X and Y frozen. **(c) "`Velocity == 0` is a fixed point that stops the mover" — DEAD.** The player fell from `Velocity.Z` exactly zero. The real gate is **2-D**: `PhysFalling`'s `SizeSq2D` clamp zeroes only the gravity-space horizontal components (the store at `0x035ED9AC` is `movups` — 16 bytes over a 24-byte `FVector` of doubles), and gravity integrates **before** it every iteration. **(d) "a seventh exit may hide in engine `PerformMovement`" — REFUTED.** Four independently written CFGs: **1461 instructions, 0 indirect jumps, 0 decode failures, 0 coverage gaps (6538/6538 bytes), 2 backward edges and NEITHER able to reach the call.** Five of the six exits **dominate** the call. ⚠ But **`CMC+0xC0 WorldPrivate` has never been read live** despite three docs implying it was — grade that exit **[I, strong]**. **(e) the S139 "next step" ranking — REFUTED IN BOTH ITEMS.** Loki `PerformMovement` reaches its Super **unconditionally** (142/322 reach `0x055B85C1`, zero edges leave the set), and both flagged branches target `0x055B85B4` — **13 bytes BEFORE the Super call** — so they skip a **loop**. And `IsSimulatingPhysics()` **had** been read. ⚠ The handoff said "three gates"; it is **five mandatory plus a non-mandatory sixth** — the settled docs were right and the compressed digest line was stale. **(f) "pin `LogCharacterMovement` for a per-frame line" — FALSE.** All three reachable sites are on arms that do not execute here. ★ But the `FLogCategory` is at **`.data 0x9F85E68`** (two agreeing derivations) ⇒ **one RPM byte says whether a category is suppressed without it having to emit.** ⚠⚠ **New instrument blindness:** capstone 5.0.7 reports `movups` **stores as READS** via `regs_access` (29/29 wrong in one function, hiding 16 CMC-field stores incl. the recommended receipt `0x055C244F`) — classify from `operands[0].type == MEM`; `pdata_union.csv` has **no row at all** for `0x055C2430` ⇒ union a pdata sweep with a **vtable** sweep; and a rip-relative `lea` scan **cannot see UE log strings** (they are reached through a 32-byte record struct) — **a positive control validates the MECHANISM IT EXERCISES, not the question you are asking** (`S140T1-b/c/d`). ⇒ **STILL OPEN: the wall MOVED, it did not close.** The AI pawn is the only thing that does not move **and it is the pawn that HAS INPUT** — a **Z-only** kick leaves `Velocity (0,0,0)` and 0.000 uu, a **horizontal** kick walks it 13,196 uu. The discriminator is the **kick axis**. ⚠ **Not a bot:** `ServerSetHeroClass` / `SetPlayerTeam` are still stripped folds and every capability here rests on pokes the game never performs itself. `docs/s140-tier1-cfg.md`, `docs/s140-tier2-sentinel.md`, `docs/s140-t2-armj-THE-BOT-WALKS.md`, `docs/s141-tier3-settled.md`, `docs/s141-t3-arml-result.md`, `docs/next-session-prompt-s142.md` |
> |
> | **FK-22** — "the drop phase is FALSIFIED as reachable" | **S130** | ✅✅✅ **BAIL 2 IS FIXED — THE DROP POD SPAWNS.** `SpawnDropPodForTeam` returns **`true`** with a DropPod census delta of **+2** (S127 measured `false` / `+0`), and both pooled spawns return live `BP_DropPod_Tutorial_C` actors (`dP1 +1`, `dP2 +2`; S128 measured NULL / `+0`). **The lever is one heap byte: `CDO->bCanEverReplicate = 0` on the drop-pod CDOs**, readback-verified, **zero `.text` writes**. ★★ The leaf CDO is now MEASURED (it is not loaded at the menu, so §12 could only infer it) — **nothing in the chain is inferred any more**. ★★ The pool was **still disabled** throughout (`bSupportsActorPoolPriming` untouched, `PrimePools` never called), which confirms the §25 refutation **live**. ⚠ The probe records `E1 RAN BUT IS NOT ATTRIBUTABLE` — that is about the **dispatch mechanism** (E0c is unsatisfiable on this class chain) and was **identical in S127, so it cancels in the differential**; the `poolspawn` arm carries no such caveat and settles C7 on its own. ⛔ **This is a DIAGNOSIS, not a shipping fix** — it mutates a class default and may break the pod's replication; **C8/C9 did not fire and are unexercised, not excluded**; nothing shows the pods are functional. `docs/s130-actor-pool-gate-settled.md` §13, `docs/fk22-dropphase-reachability.md` §28. — *previously, and still true of the route as a whole:* **the blocker moved for the FOURTH time.** markers (**refuted** S124) → phase (**solved** S124 — one `GoToPhase` call self-drives the round to `EGP_Combat`) → subscription (**dead by construction** S124) → the pooled spawn returns NULL → **⚠ NOT because the actor pool is disabled — REFUTED S130, see FK-41a**. **[M] The pool gate IS named:** `ULokiActorPoolManager` vtable slot 90 returns `Cast<ALokiGameState>(GetWorld()->GameState)->bSupportsActorPoolPriming`, a `bool` at `ALokiGameState+0x898`, and it reads false because **`BP_LokiGameState_Tutorial`'s own shipped class default sets it `False`** while the C++ ctor sets it `true` (3 of 6 GameState BPs override it, **all three to False**). But **an unprimed pool cannot produce the NULL**: the acquire `TMap::FindOrAdd`s (never null) and a pool miss falls to a shipped fallback into a normal `UWorld::SpawnActor`. ★★★★★ **AND C7 IS NOW SETTLED — OFFLINE, NO LAUNCH: `AActor+0x6C` is `bCanEverReplicate`, `AActor`'s ctor sets it **1** (`0x03371841`), neither Blueprint overrides it, and the acquire refuses ANY replicating class.** `SpawnDropPodForTeam` (`LokiDropShip.as:153`) calls the pooled deferred spawn on `TeamDropPodClass` and bails on the null **with no else branch** ⇒ **that is bail 2, explained end to end with no reference to the pool.** ★ The gate is real and satisfiable: **80 classes** have pooling∧¬replicate and **every one sampled is a cosmetic projectile visual**. ⚠ C8/C9 are now **untested rather than excluded** (C7 returns first). ✅✅ **AND THE RUNTIME READ WAS FLOWN — the runtime CDO byte IS the cooked default, so C7 fires and the last [I] is now [M].** One clean `-NoHook` MENU launch, read-only RPM, zero injection, no staging: **8/8 pre-registered predictions, 0 failures**, with a **two-sided control** on `+0x6C` (6 read 1, 2 read 0, split exactly along the cooked value) and an **unpredicted second** two-sided control on `+0x2D3` (`Default__Actor` reads 0 while every poolable class reads 1). `Default__Actor+0x6C = 1` is `AActor::AActor 0x03371841` confirmed live. ⚠ The **leaf** `BP_DropPod_Tutorial_C` is not loaded at the menu, so it remains one inheritance hop of inference (all three ancestors read 1; it overrides neither flag; the cooked→runtime mapping is validated 3/3 in both polarities). ★ **New trap:** `LogActorPooling: Adding <X> to list of poolable actors` is an **AssetRegistry query and does NOT load the class** — “registered as poolable” is not evidence a class is loaded. ★ A **free per-attempt receipt** (`Failed to spawn actor of type %s.`, fired twice in S128) proves World / GameState / `IsA` / manager-fetch **all passed live**. ★ The hand-spawn bypass has a **FIFTH wall** — `AuthPlayerEnterWorldAttachedToRidable` (`0x55CD510`), a real body that always fails on a stripped fold ⇒ **a pod and no rider**. `docs/s130-actor-pool-gate-settled.md`, `docs/fk22-dropphase-reachability.md` §25 |
> | **FK-41** — S130 actor-pool false-knowns (batched) | **S130** | ✅ **NEW + ALL TEN FALSIFIED, zero launches.** (a) *"the actor pool is the wall"* — **the project's own working hypothesis**, correctly graded [I] and resting on an **absence-of-alternatives** argument, refuted at the mechanism; (b) the handoff's *"strongest lead"* `ActorPoolManagerPrimingConfig` is a **USTRUCT with zero reflected properties and no UHT consumer**; (c) *"slot `0x2D0` needs a staged launch"* — **`tools/strxref/vtables.py` answered it in one command**, and a whole launch had been budgeted for it; (d) `fkdis.py findptr` **caps at 200 rows**, so its count is a floor (**26,444** actual); (e) `strxref.py func`'s `EXACT` extent is **per-`.pdata` ROW** — 1,086 B reported vs **3,702 B** real, and the tail is exactly where the answers were; (f) a **blank** `fkdis.py d` is a query failure, not an undecrypted page; (g) `propscan.py`'s `gen=`/`off=` columns are wrong for variant record types, which is why naming `AActor+0x6C` offline was **abandoned rather than answered**; ★★★ (h) **the ORCHESTRATION itself was the instrument** — the workflow's synthesis prompt never interpolated its lane packet, so the synthesiser re-derived everything solo and *said so*; a less careful agent would have published a confident synthesis of nothing while six lanes and six verifiers sat unread in `journal.jsonl`.; and ★★★ (j) **this session manufactured one of its own** — the `grep` command written to re-derive the instrument-artifact tally **under-counted it by half**, because `grep` quantifies the last BYTE of a multi-byte character, and it was caught only by RUNNING the instruction before publishing it; and (k) **`bpdump @props` was gated behind UFunction exports**, so a data-only Blueprint's `No matching UFunction '@props' found` read as *“no such property”* on the session's load-bearing question. |
>
...
> 70 TABULATED as of S130** — ⚠ re-derived by counting table rows, not retyped; the tally has
> diverged three times — use `grep -cE '^\| \*\*[^|]*S[0-9]+-[a-z]+\*\*' docs/method-rules.md`, and
> **not** the obvious `★+` form, which under-counts by half (that is instance **S130-f**).
> S130 added seven (FK-41 d–h plus S130-f and S130-g): four are blind spots in the offline RE
> toolchain the project now uses daily, and one is **the orchestration itself** — a workflow whose
> synthesis prompt never received its lanes' output and re-derived everything solo.
> **It stood at 48 as of S120**, which added five — and, notably, **four of the five were caught by the session
...
| **★★★ S132 UPDATE (2026-08-20) — THE STRIPPED-AUTHORITY CUT REACHES FURTHER THAN THE FOUR** → `docs/s132-dismount-settled.md` §3 | **[M, strong] `ULokiRideableComponent` has FOUR empty `Auth*` stubs, not one:** `AuthAddPlayer` (exec thunk `0x2C2CE30`, impl **`0x0F7EC20`**), `AuthRemovePlayer` (same thunk, same impl), `AuthSetCanJump` (thunk `0x5296F30`, impl `0x0F7EC20`) and the already-known `AuthPlayerEnterWorldNew` (thunk `0x5456460`). Found independently by the session lead and by an offline recon lane. ★★★ **The consequence is what matters: the ONLY reflected writers of `PlayersInside` and `PlayersAttached` do nothing in this client** — which is why both arrays read `Data=0 Num=0 Max=0` in a fully staged world, and why a data poke is the only route to a rider *by construction*, not by preference. ★ It also killed the obvious shortcut before a launch was spent on it: `AuthAddPlayer` would have replaced S132's entire hand-built append. **Checked, not assumed.** ⚠ **Grade is [M, strong], not [M]:** the `.data` record table has no class column, so the rows are matched by NAME — but each name occurs **exactly once** in the whole 16,277-record table and `ULokiRideableComponent` is the only class in `binds_members.csv` declaring either. Structural corroboration: the class's exec thunks are emitted **alphabetically** in one contiguous UHT block (`0x5455F40 AddGameplayEffect` … `0x5457940 SetPlayerDisassociationFromPhase`), and `AuthAddPlayer` would sort at ~`0x5456050` where the bytes are real code, not a thunk — its thunk is absent from the block **because it was ICF-folded onto the shared one-object-param stub `0x2C2CE30`**, which is exactly what a stripped impl does. ★ This fits S131's census finding that the enriched category is **`Auth*`, not "drop"** (gradeable 42.4 % vs 8.30 %, Fisher p = 1.6e-28): there was one decision to remove *server authority*, and the rideable component is inside it like everything else. ⚠ Note the unit on FK-1's own "1.2 % (78/6,669)" base rate — that counts **distinct exec thunks**, which are heavily ICF-folded; per RECORD the rate is 3.16 %, and in FK-1's own unit it is 170, not 78 (S131). FK-1's conclusion — an empty impl is informative, not ambient — is unaffected. ★★ **AND THE CUT IS NOW DEMONSTRABLY ROUTABLE-AROUND:** S132 drove `AuthPlayerDetachPlayerFromRidable` to completion by supplying, from outside, the one piece of state the stripped writers would have supplied. FK-1's "the deploy door is shut in C++ and possibly open in script" gains a third answer: **shut in C++, and openable by hand where the missing writer is a pure state mutation.** |
...
| **★★★ S132 UPDATE (2026-08-20) — THE KILL PRIMITIVE'S OWNER IS FOUND, AND THE OBVIOUS SEARCH METHOD IS DEFEATED BY THE TARGET** → `docs/s132-dismount-settled.md` §9 / `scratchpad/s132/lanes/L6-*` | Offline, adversarially verified, zero launches. **[M] `runtime.dll` located and identity-confirmed** at its recorded path (67,511,496 B, ImageBase `0x200000000`, SizeOfImage `0x4066000`), with FK-10's `packer/3.3.1` Sentry DSN **byte-identical at its recorded file offset `0x7C1BEC`** — an independent re-confirmation of the identification. **FK-10's kill primitive re-verified byte-exact AND [M] ITS OWNER FOUND:** RVA `0x80F7F0` is **slot 4 of a 5-method vtable at `packer0 RVA 0x1831C0`**, installed by a **constructor at RVA `0x7F86F0`** — that table's only xref image-wide. ⚠ **INDEX-BASE AMBIGUITY, FLAGGED NOT RESOLVED (S132):** FK-10 describes the **4th entry** of the `packer0 0x1831C0` table as `NtCreateThreadEx`, while S132 describes the `0xDEAD` kill primitive as **slot 4** of the same 5-method table. Those reconcile only if one is 0-indexed and the other 1-indexed. **Neither source states its convention**, so do not build on either index until one is re-read from the bytes. Caught by an independent verifier, not by either author. **That constructor is the next thing to read.** ⚠⚠ But *"`0x80F7F0` **is** `NtTerminateProcess([this+0x10], 0xDEAD)`"* is **[I], not [M]** — adversarial verification found the syscall number is computed at runtime and on disk evaluates to `0xFFFFFFFF`, not a valid service number. **The `0x0000DEAD` exit code is measured; the syscall identity is inferred.** **[M] The protector uses a COMPUTED-TAIL-JUMP architecture:** 4,769 of 18,580 functions end in `jmp <reg>`, with targets carried as `movabs reg, -(ImageBase + RVA)` folded into an MBA polynomial ⇒ **[I] a jump to `base + 1` is the NATIVE OUTPUT SHAPE of that dispatch when the resolved target RVA is 1**, which gives FK-31 a mechanism class rather than a mystery. ⚠⚠ **AND A HARD INSTRUMENT LIMIT THAT DEFEATS THE SEARCH THE S132 HANDOFF PRESCRIBED: `ImageBase == 0x200000000 == 2^33`.** Every *"is this constant the ImageBase?"* test is therefore aliased with ordinary MBA arithmetic on bit 33 — a property of the target, not of the tool. ⚠⚠ **BUT DO NOT RESTATE THAT AS "a constant search cannot be decisive", and do not repeat "all 13 candidate sites are individually refuted" — the lane wrote both and its own verifier REFUTED both.** The hits are individually adjudicable and the search **was** decisive: ★★ it produced **`packer31 0x03C8EDF2`, which computes `variable + ImageBase + 1` and whose result is DEREFERENCED at `0x03C8EFF3`** — the concrete successor lead this lane actually generated. ⚠ [S] on its role: it matches an MSVC inline-buffer idiom, so a biased pointer is likelier than the kill. ⚠ Also refuted: **`packer30` holds 0 of the 4,769 computed tails**, so "start in `packer30`" is wrong. ★ **The negative is reported WITH its denominator: 48,129,536 executable bytes scanned** (`.rwx` + `packer1` + `packer30` + `packer31`), blind spots enumerated — which is what makes it a contribution rather than an unqualified "not found". |
...
| **★ DONE — captured S111, first spent S114** | The **Steers** row above is **stale**: the capture was taken in **S111** as `dumps/tutorial-hero/` (staged world, hero spawned and possessed — `.text` **53.2 %**), and **S114 is the first session to actually spend it**: the whole FK-13 settlement rests on it. ⚠⚠ **TWO CLAIMS THAT WERE IN THIS ROW ARE RETRACTED (S133, 2026-08-20).** (a) *"The two **cannot** be merged (different ImageBase)"* — **REFUTED by S121**; `.text` carries 0 of 1,403,750 base relocations and `mergedumps` has been ImageBase-agnostic since 2026-08-14. `dumps/merged6.dump.exe` contains `tutorial-hero` (base `0x7FF6505C0000`) merged onto a `0x7FF6AF000000` seed. (b) The *"Standing rule: run every `.rdata` presence/absence claim against `dumps/tutorial-hero/…`, never `merged.dump.exe` alone"* rested on **100.0 % readable vs 63.1 %** — **two different instruments** (readable-byte vs non-zero-byte). `.rdata` completeness is measured IDENTICAL in every image on disk. **Delete the rule; do not qualify it.** FK-13's conclusions are unaffected. |
| **★★★★★ SETTLED S133 (2026-08-20) → `docs/fk20-coverage-settled.md`** | **The capture side is SATURATED and the defect is somewhere else.** [M], offline, zero launches. **Ten `dumpimage` images had accumulated unmerged; folding all ten in bought +5 pages** — `merged5` 16,689 → **`merged6` 16,694 / 30,281 `.text` pages (55.13 %)**, which independently equals the exact union of all 26 images on disk. **12 of 26 images carry the whole union; the other 14 are worth ZERO pages**, `tutorial-hero` alone carries 96.5 % of it, and only **82 pages** corpus-wide are unique to one image. ★ **An image's own coverage % does not predict its value**: `toggles` (50.61 %, the lowest non-outlier) holds the **most** unique pages (42) while `menu`/`store`/`roster`/`missions` (≈52 %) are worth **0** each. ★★★★★ **THE REAL DEFECT: coverage is EARNED AND NEVER SPENT. 31 lines across `CLAUDE.md` + `docs/` assert a page is undecrypted / coverage-blocked / "100 % zero in every dump" for an address that is READABLE in `merged6` today**, and **~29 of them were already stale against `merged2`** — they never needed a new capture, only someone to look. ⇒ **Restate FK-20: the prescription is not "capture more states", it is "drive new code, THEN RE-GRADE", and the re-grade half has never been run.** ⚠ Two candidate new coverage sources are FORECLOSED with positive controls: Sentry crashpad minidumps hold **0 bytes** inside the game image (thread stacks only), and the 98 UECC crash minidumps hold **`.rdata` only** (520.9 MB of `.rdata` vs **13,824 bytes** of `.text` across all 98). ★ Dark-set shape: zone A `0x1000`–`0xFFFFFF` is **88.4 % dark** and cites no game function (26.7 % of the dark set, likely unreachable [I]); **zone C `0x4D00000`–end is the real target** — 5,988 dark pages / 24.5 MB of Loki code, with the **Angelscript AOT bodies 65.5 % dark**, exactly where FK-1 puts deploy/respawn. |
...
| **★★★★★ S123 CLOSE (2026-08-15) — ALL THREE PANELS ANSWERED, AND THE BELIEF IS FALSE IN THE SAME DIRECTION IN ALL THREE** | The prescribed experiment was run exactly as prescribed — one populated row, ALONE, `minimal` before `full`. **`Career→History` RENDERS IT** (screenshot): `VICTORY · Basic Training · 1/16 · Aug 15, 2026, 10:26:21 PM · (18:00)`, hero portrait, an `ALLY Reviver#6612` row, and the full expanded stat panel. Backend-only — no shim, no injection, no `.text` write; four arms on one client up 3h15m with **no relaunch**. ⇒ **Career→Stats (S121), →Ranked (S122) and →History (S123) were each empty because we served nothing, and each renders the moment it is fed.** The severity line's worry — that this "inflates the 24-LIVE-surfaces count" — resolves the *other* way: the surfaces are genuinely live, they were merely unfed. ⚠ **Sharpen what was actually wrong.** For History the S119 row had already excluded "broken deserialization" via the `+0x68` Version discriminator, and that holds — even at BASELINE the manager held our player id with the gate open. What S123 adds is that the panel is *live*, which no prior measurement had shown. **Arms** [M], readout `tools/re/matchhistory_readout.py` (new; reads the parsed `FMatchHistory` off the live manager): off `Num=0` · minimal `Num=1`, 8 fields verbatim, unserved fields at defaults (built-in negative control) · full `Num=1`, `Placement 1`, `StartingRank`→byte 12 `Gold1` · full+`Effective*` all damage tiles populate. Canaries **0/0/0/0** throughout. Knob `AGS_MATCH_HISTORY=off\|minimal\|full`, default off byte-identical to pre-S123; 4 tests, the byte-identical pin negative-control-verified. `server/internal/interactive/matchhistory.go` |
| **★★ S123 — TWO FINDINGS THAT GENERALISE BEYOND THIS ROW** | ⚠⚠ **`FMatchHistoryTeamInfo.Placement` IS 1-INDEXED — the OPPOSITE of its sibling.** It rendered `1/16` from `Placement: 1`, while S121 measured `FPlayerHeroStats.Placements` on `/player-stats/players/{id}` as **ZERO**-indexed (key 0 == 1st, confirmed by pre-registered prediction). Two placement fields, one backend, opposite conventions — **do not carry either across.** It was only discriminating because the flight served 1-of-**16**; 1-of-1 renders identically under both. ★★ **The damage tiles read the `Effective*` fields, never the raw `Damage*`** — 5/5 with deliberately distinct values (`TOTAL DAMAGE DEALT` 18,000 from `EffectiveDamageDone`, not 21,400 from `DamageDone`; likewise `HeroEffective*` ×2 and `ShieldMitigatedDamage`). The four raw fields are unread by this panel. The clue was that **healing rendered correctly while damage read 0** — same struct, same float type — because healing is the one stat with no `Effective` variant. ⚠ And two labels do not match their fields: `MINIONS KILLED`←`CreepKills`, `GOLD FROM MINIONS`←`GoldFromMonsters`. |
| **⚠ S123 — TWO INSTRUMENT ARTIFACTS, BOTH CAUGHT BEFORE PUBLICATION** | (a) **The archetype trap, 5th member of the class-lookup blind-spot family.** Before the screen was opened, RPM showed exactly ONE live `WBP_UI_MatchHistoryEntry_C` with `Visibility = 4` — which reads as "a row is rendered". It was the widget-tree TEMPLATE with a design-time value; **`MatchHistoryScreen`/`ProfileScreen` had 0 activations in the whole session log**, and that count is what caught it. ⇒ *before reading a widget's state, prove its screen was ever built.* (b) ★ **NEW VARIANT — the grep WINDOW is part of the instrument.** `grep -B2 -A3 match-history` over `capture.log` paired a request with a **neighbouring** request's `User-Agent` and read as `supervive-loadout-shim`, i.e. "the game never refetched"; `-A 12` showed the true pairing, `Loki/UE5-CL-0`. The documented User-Agent rule says *filter by UA* — it does not say *pair each request with its OWN header block*, and a narrow window silently violates that. Verification curls were additionally given the UA `fk21-verify-NOT-THE-GAME` so they can never be mistaken for the client. |
| **⚠ S123 — ONE READOUT ATTEMPTED AND RECORDED AS UNINTERPRETABLE, NOT NEGATIVE** | `Matches.Num()` is also `Comp_MainMenu_Onboarding`'s games-played count ([M] from `bpdump_Get Number of Games Played.txt`: a `Cheat.Onboarding.MatchHistoryCount` cvar override, else `GetMatchHistory().Matches.Num()`), so it promised a second readout on an unrelated subsystem. The ubergraph local reads **0** on the live instance (HAS-RUN 59) — but it is a **per-execution scratch slot whose value is also the default**, so "re-evaluated and got 0" and "holds a value from a run predating our document" cannot be separated. Logged as ambiguous. ★ It also names a blast radius worth knowing: serving N rows makes the client believe the account has played N games, which feeds `Should Show Returning Player Modal`. |
...
| | |
|---|---|
| **Belief** | `docs/coverage-audit-s101.md:269` (⚠ this row said **`:229`** until 2026-08-16 — a miscitation carried since S101): *"Drop-in / DropPlane — **FALSIFIED as reachable** — SpawnPlane faults on absent level markers — S93."* |
...
| **★★★★★ S124 RESOLUTION (2026-08-16) — THE BELIEF IS FALSE AS WRITTEN, AND THE PRESCRIBED EXPERIMENT INVERTED ITS OWN EXPECTED ANSWER.** → `docs/fk22-dropphase-reachability.md` | The experiment was run exactly as prescribed (offline, zero launches, zero injections, zero `.text` writes) and returned the **opposite** of what it was designed to find. It expected "the BR map has the markers, `LVL_Tutorial` doesn't." **[M] `LVL_Tutorial` HAS ALL THREE** (`TrainingStart` → cell `D0E5AKNE…`, `PlaneStartPoint` → `8MF6M4K4…`, `PlaneEndPoint` → `4WUJ1QA2…`, three separate World Partition cells under `Maps/Tutorial/LVL_Tutorial/_Generated_/`), and **`Skylands_WP` — the BR map — has NONE** (0 hits in 2,216/2,216 parsed packages). Census denominator: 7,300 `.umap` packages (unit: packages), 7,300 parsed / 0 failed. ⇒ **S93's stated reason, "drop-path markers that don't exist outside the real deploy", is refuted on the very map it was measured on.** ★ The structural fact that carries FK-22's core objection is independent of all that: **the three `Comp_GameMode_DropPlane*` classes are SIBLINGS**, each deriving directly from native `ULokiGameModeDropPlaneComponent` with its own `SpawnPlane` override and **no BP-to-BP inheritance** — so an S93 result on the `_Tutorial` override cannot transfer to the general component by any mechanism. The **general** variant has 0 `GetAllActorsWithTag` (9 stmts; its spawn is in `OnDeathCircleSet`, derived procedurally from the death-circle radius), and it does not even *have* `GetAutoDropLocation`, S93's second observation. ⚠⚠ **But "reachable" is NOT established and the row does not flip to reachable** — it becomes OPEN with two different measured blockers, neither about markers: (a) **[M] the round-phase machine never leaves `EGP_BeginInit(1)`** — 193 occurrences of `Setting Phase to` across 564 log files, **193 of them `1 (BeginInit)`**, zero transitions ever recorded, while the drop needs phase ≥ 4 and the component's handlers act on 5/6/7; (b) **[M] 13 of 100 (class, func) keys on the 8 drop classes are empty C++ impls**, sitting exactly at the player→plane (`ALokiDropPlane::AddPlayerToPlane`) and pod→hero (`AuthBeginGlideDiveFromDropPod`) handoffs. ★ **[I, strong] S93's observation is also confounded at the instrument:** its `FAULTED` is the boolean returned from a bare SEH `__except`, and `CallBPGuarded` memcpys a captured live `FFrame` without reinitialising `0x48..0x78` (`FlowStack`/`PreviousFrame`) — and `SpawnPlane` is the **only** one of the three functions S93 compared that uses the flow stack (3 push / 2 pop vs 0/0 for both that "ran clean"). The confound tracks the result exactly and was never controlled. ⚠ **Two claims by the investigating agents were themselves REFUTED by adversarial verification and must not be propagated:** "marker dependence is exclusive to the Tutorial variant" (`_PvE_Holdout`'s `SpawnPlane` is a byte-twin — 2 of 3 variants read markers, the *general* one is the odd one out) and "those actors exist in `Skylands_WP`" (true of `BP_BattleRoyaleSpawner_BossLocation_C`, false of `LokiCapturePoint`). ⇒ **FK-22's own error — generalising from the variants you opened to the ones you did not — was re-committed three times by the lines auditing it.** **Next (offline, minutes):** grade the impls of `ALokiRoundGameMode::GoToPhase` and `ALokiGameState::BP_AuthSetCurrentPhase` (both `Final\|Native\|Public\|BlueprintCallable`); if REAL, the phase ladder is drivable **today** by the S55 primitive with no `.text` write. ⚠ Name trap: the registered UFunction is **`BP_AuthSetCurrentPhase`**, not `AuthSetCurrentPhase`. |

| **★★★★★ S124 SECOND UPDATE — FLOWN. THE PHASE IS SOLVED; THE BLOCKER MOVED TO THE SUBSCRIPTION.** → `docs/fk22-dropphase-reachability.md` §14-§15 | Two live flights, one staged tutorial world, **zero `.text` writes, zero PI hooks, zero crashpad handoffs, no relaunch between them.** **[M] `GoToPhase` is reachable and unguarded** by a direct `UFunction.Func` call: the shim called it **twice** (args 1, 4) and the log shows **six** transitions — our `1` (positive control: a SECOND `Setting Phase to 1 (BeginInit)`, where baseline is exactly one per file across 193 files), our `4`, then **`5`/`6 (Lineup)`/`7 (Combat)` ~100 ms apart that we never called.** The game printed `Took 414.264048 seconds to go from EGP_Pre to EGP_Combat` and began mass navmesh generation ~49 s later. ⇒ **one call starts a self-driving cascade to Combat**, exactly as predicted offline from the tail-jmp callers via `[vtable+0xB08] = OnNewPhase`. **[M] The store is dead ([I]→[M]):** all six `Transitioning` lines read `from (EGP_ServerStartup)` and `GameState+0xA44` reads **0** afterwards. ⚠⚠ **BUT REACHING `EGP_Lineup(6)` DID NOT FIRE THE DROP PHASE**, and `BP_AuthSetCurrentPhase(6)` broadcast into a **7**-subscriber list with **zero** effect — until the invocation list was walked: **`Comp_GameMode_DropPlane_Tutorial` (`0x1B3771413C0`) IS NOT A SUBSCRIBER.** ⇒ **the null was about REACHABILITY, not behaviour**, and FK-22's blocker is now **the subscription** (`ReceiveBeginPlay`'s `ServerOnly` gate), not the markers (refuted) and not the phase (solved). ★ Two docs claims REFUTED live: **`GameState` is at `GameMode+0x418`, not `+0x258`** (the `+0x258` value is not a UObject at all — the shim's `GcAlive` guard caught it and aborted rather than poking a stranger), and `+0x7C0 = 4` is confirmed ⇒ the 3→4 gate is **one term short**. **Next:** call the DropPlane handler directly on the live component, or call `SpawnPlane` directly — §2.1 measured it branchless and its three markers are PRESENT in `LVL_Tutorial`, so the S93 fault should not reproduce, and neither route needs the phase at all. |

| **★★★★★ S130 THIRD UPDATE (2026-08-20) — THE POOL GATE IS NAMED, AND THE POOL IS **NOT** THE BLOCKER.** → `docs/s130-actor-pool-gate-settled.md`, `docs/fk22-dropphase-reachability.md` §25 | Offline; zero launches, zero injections, zero `.text` writes. One session-lead thread + six decode lanes, **each adversarially verified by an independent agent that re-ran the commands rather than checking the quotes** (scorecard 11/16, 10/13, 12/22 with **1 REFUTED**, 13/16, 8/9, 7/13 with **1 REFUTED**). **[M] THE GATE IS NAMED:** `ULokiActorPoolManager` vtable slot 90 (`.rdata 0x08877A80 + 0x2D0 → 0x56363F0`, fold multiplicity **1**) returns `Cast<ALokiGameState>(GetWorld()->GameState)->bSupportsActorPoolPriming`, a `bool` at **`ALokiGameState+0x898`** — named from the UHT `FBoolPropertyParams` at `.rdata 0x08983A50` whose **`SetBitFunc 0x053800D0` is `mov byte [rcx+0x898],1; ret`** (multiplicity 1), the **only** bool UPROPERTY at that offset over **13,156** Bool records. **`UWorld+0x258` = `UWorld::GameState` [M]**, confirmed by a third unrelated function (`UGameplayStatics::GetGameState`, `0x38047F0`). **[M] AND THE CAUSE IS A SHIPPED ASSET, NOT CODE:** the C++ ctor sets it TRUE (`0x05676F10 c6 87 98 08 00 00 01`) and `bpdump_BP_LokiGameState_Tutorial_PROPS.txt:52` serializes `bSupportsActorPoolPriming = False`; **3 of 6 GameState blueprints override it and all three override to False** (`_Tutorial`, `_PvE_Holdout`, `_FFA`), the other three inherit `true` from populated dumps. ⇒ the pool is off in the tutorial **by design, in data**. ⚠⚠⚠ **BUT THE HEADLINE IS A REFUTATION OF THIS PROJECT'S OWN WORKING HYPOTHESIS: an unprimed pool CANNOT produce the observed NULL [M].** The acquire's lookup `0x334E7A0` is a **`TMap::FindOrAdd`** (one `ret`, inserts on a miss, never null), and a pool miss takes a **shipped fallback** — `.rdata 0x08B06440 'Failed to find an actor in the pool for %s, spawning a new instance from scratch.'` — then `0x5648E48 call 0x39C3DB0` = `UWorld::SpawnActor`, returning the fresh actor. ⚠ That message's **emit is stripped** (`0x5648D6F` → the `ret 0` fold, 4,972 call sites), so its absence from the 69-file log corpus is **uninterpretable, not negative**. ★ **The pooled spawn never reads the gate [M]**, shown three times by disjoint methods; the gate has **exactly one** call site image-wide in this family (`0x33560C5`, inside `PrimePools`). ★★ **A FREE PER-ATTEMPT RECEIPT nobody had used narrowed the cause:** the non-deferred wrapper logs `Failed to spawn actor of type %s.` (`.rdata 0x08B06390`, 2 `ptr-tbl` refs) on NULL and **fired exactly twice in the S128 flight** naming `BP_DropPod_Tutorial_C` — emitted strictly downstream of every outer precondition, so **[M] World, GameState, `IsA(ALokiGameState)` and the manager fetch ALL PASSED live.** ⇒ the survivors are **C7 `CDO->byte@0x6C != 0`** (`0x5648210`) · **C8 `PoolMgr->GetWorld()==null`** (`0x5648D97`) · **C9 `SpawnActor` null or skipped at `0x5648E34`** (`0x5648E6F`), and **C7 costs ONE read-only RPM read with no launch.** ⚠ [I, strong] the **deferred** arm's null is **SILENT** (it bypasses the wrapper): 2 warnings ≈89 s apart = one per injection, not two. ★ S128's collision-confound elimination **STANDS** — `Collision=2 (declared enum 'ESpawnActorCollisionHandlingMethod')`, `NumParms=8`, enum name read live off the FProperty. **[M] No ini route:** `ActorPoolManagerPrimingConfig` — the S129 handoff's *“strongest lead”* — is a **USTRUCT with ZERO reflected properties and no UHT consumer**; `CPF_Config` clear; **0 of 155** `ALokiGameState` properties are config. **And the bypass has a FIFTH, previously unrecorded wall [M]:** `ULokiRideableComponent::AuthPlayerEnterWorldAttachedToRidable` (impl `0x55CD510`) is a REAL body that ALWAYS fails — `0x55CD572` calls the stripped fold `0xF7EB50` and bails into *“failed to get the round game mode”*; its dead tail has **zero** external rel32 entries in three images. Same wall on `AuthPlayerPreSpawnOnAddToPlane`; `AuthPlayerEnterWorldNew` is an empty fold. ⇒ a hand-spawned pod gets **a pod and no rider**. ★★★ **NEW GENERAL INSTRUMENT, worth more than the finding:** the `.data` `{name_ptr, exec_thunk, impl}` record table gives a REAL/EMPTY verdict **without the code page being decrypted** ⇒ **§2.5's 16 COVERAGE-BLOCKED `(class,func)` keys are an instrument limit, not a fact**, for at least 6 of them (all on page `0x5456000`). Re-running it over all 100 keys is **free, offline and unstarted.** ⚠⚠ **FK-22's own signature error recurred AGAIN, on this exact function family:** an agent claimed `AuthSetSpawnTeamLeader`'s flag feeds only `GetTeamDropLeader`; it has **three** Angelscript readers, one (`QueueCrewForPodSpawn`) on the leader-pod path ⇒ *“the bypass avoids FK-1's stubs”* holds **only under `bIsTeamLeaderPod == false`**, and the route transcribed from `SpawnDropPodForTeam` passes `true`. Caught only by adversarial verification. |
| **★★★★★ S132 FOURTH UPDATE (2026-08-20) — THE DISMOUNT RUNS, AND IT IS USABLE. THE HERO LEAVES THE POD AND STANDS ON REAL GROUND.** → `docs/s132-dismount-settled.md` | **SEVEN detach calls across FOUR launches, SIX of which moved the hero; risk class DATA; zero `.text` writes, zero PI hooks, zero CDO pokes.** The recipe is exactly S131's prediction: append the PlayerState to `ULokiRideableComponent::PlayersAttached` (`+0x130`/`+0x138`/`+0x13C`) with the GAME'S OWN `ResizeGrow` at **`0x00F988D0`** — the same function the fifth wall's own tail calls at `0x55CD75B`, so the ABI and element size are correct **by construction** — then call **`AuthPlayerDetachPlayerFromRidable`** (impl `0x55CCCB0`, thunk `0x5456100`) via the S55 direct `UFunction.Func` thunk. **[M] FLIGHT 1 — four dismounts on one launch:** hero X = 1.45M → 4.86M → 11.65M → 14.43M, each matching the flying pod's X at its OWN call time, run 2's hero Y **BIT-IDENTICAL** to the pod's (`==` on raw doubles in a live read) and Z = 250.0 every time under a pod at Z = 20,100 — a payload fingerprint no static explanation survives. **[M] FLIGHT 2 — IT IS USABLE:** with a `LokiPlayerStart` passed as `LandingLocationActor`, **1,488,146 uu** from the pod at that instant, the hero landed **AT THE PLAYERSTART**, settled to Z = 90.15 and held position **bit-for-bit across 9 s** while the pod flew another 180,000 uu ⇒ **`GetLandingTeleportLocation` CONSUMES its `LandingLocationActor` argument**, and the hero is un-hidden, collided, gravity-affected and standing. **FLIGHT 3 reproduced it a third time.** ⚠ A seventh call, on a fourth launch, FAULTED without moving the hero — no PlayerState candidate passed GATE 5 that run and the arm proceeded anyway; `GetLokiCharacter` FAULTS on a template PlayerState rather than returning null, so GATE 5 is not a clean early-out for a bad argument. **The no-candidate branch must REFUSE.** ★ The safety design held: SEH caught it, the client survived 428 s, and the arm's restore step detected and removed the entry the aborted call had left in `PlayersAttached`. **The within-run NEGATIVE CONTROL — same detach, same component, same primitive, `PlayersAttached` EMPTY — never moved the hero, before every call**, against a prediction the shim printed first. ★★ **FREE THREE-WAY RECEIPT on a function with ZERO log strings:** `PlayersAttached.Remove(PS)` at `0x55CCE23` runs on **every** path past GATE 4, so `Num` staying 1 vs dropping to 0 separates "bailed at a gate" from "ran past GATE 4" with no log dependence — observed **1 → 0** on every successful run. ⚠⚠ **S131's "expect a PARTIAL dismount" was TOO PESSIMISTIC:** the teleport, un-hide, collision restore and movement restore are every one a real body, and the two `0xF7EC20` folds are void side effects whose returns are **never tested**. ⚠⚠ **TRAP: `ContainsPlayer` scans `PlayersInside` (`+0x120`), NOT `PlayersAttached` (`+0x130`)** (measured at `0x55D0270`) — using it as the append receipt manufactures a false negative on a working append; it was disassembled before being trusted and turned into a pre-registered "MUST still be false" prediction. ★★★ **NEW [M, strong]: `AuthAddPlayer`, `AuthRemovePlayer` and `AuthSetCanJump` are ALSO stripped folds** (`impl 0x0F7EC20`) ⇒ the component has **FOUR** empty `Auth*` stubs and **the only reflected writers of either player array do nothing in this client** — which is why both read `Data=0 Num=0 Max=0` in a fully staged world and why a data poke is the only route *by construction*. The obvious shortcut was **checked, not assumed** (see the FK-1 S132 row). ⛔ **`AuthPlayerEnterWorld` (`0x55CCE70`) is FORECLOSED** — its two terminal actions are direct calls to the stripped `0xF7EB50` and it writes **no** actor or component transform. ⛔ **This is a DIAGNOSIS, not a shipping fix**: it writes a live component's state array by hand and drives an authority-only entry point. ⚠ Corrected in-session: `PlayersAttached` is **NOT** replicated (no `CPF_Net`), which makes the write safer than the first write-up described. ⚠⚠ **STILL OPEN, and the arm that looks like it answers it is DEGENERATE — measured:** *is the hero playable AT the landing point?* `RM_PLAY` relocates the hero first (hardcoded teleport to `(-65,-1770,393)`), and with that skipped (`KNOTELE=1`) `KFLYMODE` still defaults to `5 = MOVE_Flying`, so the hero **hovers** — the control moved **2,926 uu at CONSTANT Z = 13,240, 13 km in mid-air**. Only `play-atlanding-walk` (`-DKFLYMODE=1`) can answer it; built, hashed and pre-registered in `docs/next-session-prompt-s133.md` §1.2. Regression gates `play` / `dropplane_b1only` / `droppod-pe-cdopoke` all MATCH throughout. |
...
**★ S132 UPDATE (2026-08-20) — ONE MORE STAGING DEATH, AND THIS ONE LEFT A MINIDUMP.**
Five launches this session; **flight 4 died during staging with `0xC0000005`, only `gft`+`fo` resident**
— the canonical FK-31 shape. ★ Unusually for this class, **`crashwatch` caught it and a 41 MB
crashpad minidump was archived** to `dumps/crashpad-20260820-143225`, which is the artifact this row has
been asking for. The other four launches all staged cleanly and three of them produced dismounts.
⚠⚠ **One thing it does NOT settle, and the temptation is real.** The crash-era `dump.exe` reads
`.text` **51.8 %** against a healthy dump's **53.0 %**, which looks like a clean refutation of the
pre-registered *"a crash-era image holds MORE decrypted `.text`"* prediction (`crashwatch` prints that
prediction in its own INFO file). **The comparison is NOT matched:** the crash dump died at 141 s having
exercised far less game code than a client that had run the whole dismount chain, so the two differ in
**how much code they ran**, not in crash-vs-healthy. **It is uninterpretable for that purpose, not a
refutation.** A matched test needs a crash from a client that reached the same state.
...
WARNING — **N = 2 when this was written. Suggestive, not established.** The instrument is now
permanent in `configs/fk7-ab-run.ps1`, so **harvest this corpus; do not spend launches on it.**
Open residual: **3/36 armed windows (8 %)** still die with no module-image write anywhere.

**★ S132 UPDATE (2026-08-20) — A THIRD `0x0000DEAD`, HARVESTED FOR FREE, EXACTLY AS PRESCRIBED.**
A `play-atlanding` arm died during its own init and `crashwatch` named the exit code without a launch
being spent on the question: `exit code 57005 (0x0000DEAD) — PROTECTOR NtTerminateProcess(0xDEAD)`.
**N = 2 → 3.** Still suggestive rather than established, but the instrument has now paid for itself
three times and the class is no longer a mystery at the mechanism level (FK-10 has the primitive at
`runtime.dll` RVA `0x80F7F0`, and S132 found its **owning vtable at `packer0 RVA 0x1831C0` and the
constructor that installs it at RVA `0x7F86F0`**).
⚠ Context that matters for attribution: it was the **7th injection into that process** (gft → fo → sp → b1only → droppod → dismount → play-atlanding), so it is a data point
about the death class and **not** evidence against any one arm. The sitting was recorded **VOID** for
the question it was flying — no `[PL] *** init complete ***` was ever printed — not negative.
Detail: `docs/s132-dismount-settled.md` §6a-2.4.
...
⚠⚠ **SCOPE NOTE (S122, FK-40d): that closure is real but it is about the DECLARATIVE partition only,
and it was read as "the toggle vocabulary is closed".** It is not. A third category exists whose keys
are **assembled at runtime** (`Concat_StrStr("queue.restrictions.", QueueID)`) and therefore appear in
no static key list, so neither the declarative nor the bytecode census could have contained them.
⇒ **an exhaustive census bounds the population it can enumerate, never the population.**
...
### FK-40 — S122 activity/queue false-knowns (batched)

**Severity: HIGH (one of them was a workaround that hid its own defect for ~60 sessions). Settled
2026-08-15 (S122) → `docs/s122-player-rank-career-badge.md` §9-§11.**
**Status: all DEAD. Two of the five were manufactured by S122 itself.**

| # | Belief | Why it was believed | What is actually true [M] |
|---|---|---|---|
| **a** | ⚠⚠ **"`CanControlQueue` loops over the current queues calling `GetLevelGameFeatureUnlocked`; with account level 0 any level-gated queue fails that loop → every activity click errors"** — `interactive.go:1305`, carried S60→S122 | it explained a real, reproducible error, and nobody read the bytecode | **THERE IS NO LOOP.** `bpdump_CanControlQueue.txt` 181-185: `GetLevelGameFeatureUnlocked` is called **exactly once**, with a **hardcoded** `PrimaryAssetId{GameFeature,"Ranked"}`, behind `EX_PopExecutionFlowIfNot(Not(bIsRankedEligible))`, and only to **format** the "you need level N" Text. The real gate is per-tile: `WBP_UI_PartyQueue_DropdownItem` → `IsQueueIDPremadeOrOverQueueLevel` → {CanQueue, UnlockLevel, Reason} against `GetAccountPassViewModel` |
| **b** | ★★★★★ **"The S60 queue trim was necessary"** — trimming 10 queues → 4 demonstrably stopped `"Unable to modify activity"` | the error really did stop, and stayed stopped for 60+ sessions | **A CORRELATION.** The trim deleted the level-*gated* queues (`deathmatch`, `tournament`), leaving nothing lockable to click. **Proven by reproducing the exact string on demand**: withdrawing `queue.restrictions.deathmatch` brings back `"Unable to modify activity. Note: You must always have one activity selected."` verbatim, with FIND MATCH greyed. ⇒ the trigger is **selecting a level-locked queue**, not queue-list length |
| **c** | ⚠ **"S120 serves `AccountPass.Level = 10`, so the S60 precondition is gone"** — asserted by S122 into `CLAUDE.md` **and a commit message** before being checked | `CLAUDE.md` records S120 *measuring* that serving 10 removed the mastery lock | **The live server serves `Level 0`.** S120 configured 10 for one experiment; it never became the default (it comes from persisted per-player state). ⇒ **a remembered measurement read as a shipped default.** Check the wire |
| **d** | **"The feature-toggle vocabulary closes with no remainder — 50 declarative + 10 bytecode keys"** — S121, FK-37, and A-14's close | two independent exhaustive censuses agreed, and the partition arithmetic balanced | **A THIRD CATEGORY EXISTS: keys BUILT AT RUNTIME.** `bpdump_IsQueueIDPremadeOrOverQueueLevel.txt` 6-12: `Concat_StrStr("queue.restrictions.", QueueID)` → `GetFeatureToggle` → `Map_Find(Config,"Level")` → `Conv_StringToInt`. A concatenated key appears in **no** static key list, so **neither census could ever have seen it**. **FLOWN AND CONFIRMED** (full A-B-A): serving `Level:"0"` unlocks ARENA. ⇒ the vocabulary is **REOPENED**; other parameterized families may exist |
| **e** | **"The unserved-endpoint sweep maps our unserved surface"** — S122's own §1: *"56 client routes, 8 unserved"* | a 74-minute capture parsed exhaustively, with the User-Agent trap correctly avoided | **IT IS A LOWER BOUND, NOT A MAP.** It enumerates only the endpoints the client *happened to exercise*. `POST /party/parties/{p}/setTargetQueues` — the activity-selection call, and the actual cause of (b)'s residual symptom — was **absent**, because nobody clicked a tile during that capture. ⇒ **drive the UI through the interactions you care about, THEN re-run the diff**. ★★★ **CONFIRMED THREE MORE TIMES, S133 (2026-08-20), by doing exactly that:** clicking FIND MATCH exposed `POST /party/parties/{p}/joinQueue`, cancelling exposed `…/leaveQueue`, the privacy toggle exposed `…/setIsOpen/{True|False}` and an emote exposed `…/emote/{Emote:Name}` — **all four absent from the 56-route sweep, all four on the `/` catch-all, and `joinQueue` is why FIND MATCH did nothing at all.** ★ **Free receipt: the client RETRIES an unaccepted party verb** (`joinQueue` re-POSTed every ~10–35 s until the response was accepted, then exactly once). ⇒ the sweep's residual is **≥ 12 unserved, not 8**, and the true number is unknowable by capture alone. `docs/s133-joinqueue-find-match.md` |

**Generalisation, and it is the reusable part.** (a)+(b) are one mechanism seen twice: **a workaround
that removes the TRIGGER is indistinguishable from one that fixes the CAUSE — and it makes the bug it
hides unobservable, so it looks correct forever.** Under the trim there was exactly one selectable
activity, so the missing `setTargetQueues` handler could not *possibly* be noticed: there was nothing
to switch to. Removing the workaround is what surfaced it. ⇒ **when a workaround is load-bearing,
the honest test is to remove it and see what breaks — not to reason about whether it is still needed.**

**Cheapest experiment:** already run, **zero launches** — every arm rode on one client that stayed up
**3h39m**, via `ags` restarts only (the client's WS resync refetches config and party state within
~40 s).

⚠ **Five instrument-artifact instances from this session**, tabulated in `docs/method-rules.md`:
`bpframe_readout.py` taking the **first** class match (a **fourth** member of the class-lookup
blind-spot family — it reported `False` for a graph that had **never run**, has-run 0 of 219, on a
menu live 74 min); `obj_props_dump.py` being **blind to scalars** while offering a decoy
(`ActiveSequencePlayers=2` on the badge — the three *collapsed* siblings read 2, 2, 1); a **port
collision** that let another session's `ags` answer a request recorded as this session's own control
(caught only because the eTag lacked an expected suffix); a **stale log line** matched by "any"
instead of "latest" that briefly confirmed a reversal which had not happened (`Loki.log` is UTC, the
shell is local); and `Start-Process -RedirectStandardOutput` failing at 6.4 GB/49 GB free, leaving an
`ags` that **existed but bound no ports** — ⇒ *"the process exists" is not "the service is up"*.

---

### FK-41 — S130 actor-pool false-knowns (batched)
**Severity: HIGH — (a) pointed two sessions at the wrong subsystem, and (b)/(c) turned two false
handoff premises into budgeted work — and (j) was manufactured by this session, in the act of fixing a
different tally defect. Settled 2026-08-20 (S130) → `docs/s130-actor-pool-gate-settled.md`.**
**Status: all DEAD. FK-22's outcome is UNAFFECTED — these correct where its blocker IS.**

| # | Belief | Why it was believed | What is actually true [M] |
|---|---|---|---|
| **a** | ★★★★★ **"The actor pool is the wall — `PrimePools : Feature is not enabled` is WHY `SpawnPoolableActorFromClass{,Deferred}` return NULL"** — `docs/fk22-dropphase-reachability.md` §23.3 (correctly graded **[I]**, and §23.4 built the whole next-session plan on it) | the log line and the null are genuinely co-present, the collision confound had been properly eliminated, and **no other cause was in evidence** — which is exactly the shape of an absence-of-alternatives argument | **An unprimed pool CANNOT produce that NULL.** The acquire's lookup `0x334E7A0` is a **`TMap::FindOrAdd`** — one `ret`, every path returns `MapData + idx*0x28 + 8`, inserting on a miss. On a pool miss the code takes a **shipped fallback**, `.rdata 0x08B06440 'Failed to find an actor in the pool for %s, spawning a new instance from scratch.'`, then `0x5648E48 call 0x39C3DB0` = `UWorld::SpawnActor`, and returns the fresh actor. **The pooled spawn never reads the gate at all** (3 disjoint methods; the gate has exactly one call site image-wide in this family). ⚠ That message's **emit is stripped** (`0x5648D6F` → the `ret 0` fold), so its absence from 69 log files is **uninterpretable, not negative**. ★ The real cause was narrowed by a **free per-attempt receipt nobody had used** — `Failed to spawn actor of type %s.` fired **twice** in the S128 flight, strictly downstream of every outer precondition ⇒ World / GameState / `IsA(ALokiGameState)` / manager-fetch **all passed live**, and the NULL is **C7 / C8 / C9 inside `0x5648050`**, with **C7 costing ONE read-only RPM read** |
| **b** | **"`ActorPoolManagerPrimingConfig` is the strongest lead; if it is a `UDeveloperSettings` the feature is ini-drivable with no injection"** — `docs/next-session-prompt-s130.md` §0, which made it the session's **first task** | the name, its `refs=1`, and its position in the same module as `PrimePools`. ★ The handoff *did* hedge correctly (*"do not assume it is the gate"*) | **INERT.** It is a **USTRUCT (`UScriptStruct`), not a UClass**, so it cannot be a `UDeveloperSettings` and has no ini section of its own; its `FStructParams` has `PropertyArray = NULL`, `NumProperties = 0`, `SizeOf = 0xA8`; and **no reflected property anywhere in the image has it as a type**. Neither pool-manager UCLASS carries `CLASS_Config` / `CLASS_DefaultConfig`, `CPF_Config` is clear on the real gate, and **0 of `ALokiGameState`'s 155 reflected properties are config**. ⇒ **there is no injection-free ini fix.** ⚠ The lead was not wasted — the same pass named the gate |
| **c** | **"Resolving vtable slot `0x2D0` must be done LIVE — stage a tutorial world, walk `GUObjectArray`, read `[obj]` then `[vtable+0x2D0]`; about a minute"** — `next-session-prompt-s130.md` §1, which budgeted **one staged launch** for it | S129's offline route (class name → `GetPrivateStaticClass` → `InternalConstructor` → `lea rax,[rip→vtable]`) had genuinely failed, and the handoff correctly labelled that *"a route not walked correctly, NOT evidence the vtable is unfindable"* | **The project already owned an offline vtable index that answers it in one command.** `tools/strxref/vtables.py classes "ActorPool"` → `UActorPoolManager` `.rdata 0x07DE3618` and `ULokiActorPoolManager` `.rdata 0x08877A80`, both 95 slots; `name <class> --slots 96` prints slot 90 directly. **Both gates read out in four tool calls, with no launch.** ★ `vtables.py` cuts vtable boundaries at constructor `lea` sites and names classes from UE's `IMPLEMENT_CLASS` boilerplate — it is the standing answer to *"what is at slot N of class X"* and **should be tried before any live vtable read** |
| **d** | **"`fkdis.py findptr <addr>` returns the fold multiplicity"** — used to grade whether an address identifies a function | the helper prints one row per matching qword, and the count looks like an answer | **It hard-caps at 200 rows.** It first produced *"`0x0B9E1F0` is a 200-way fold"*; uncapped it is **26,444**. Also measured uncapped: `0x0F7EC20` **165,789**, `0x0F7EB50` **27,217**, `0x12C7260` **2,823**. ⇒ **a row count from a capped listing is a FLOOR, never a count** — the same shape as `obj_by_class.py`'s 60-row cap, which this project had **already recorded once** |
| **e** | **"`strxref.py func`'s `extent … (N bytes) -- EXACT` is the function's size"** — the word EXACT reads as a guarantee | it *is* exact, about a `.pdata` **row** | **Extents are per-`.pdata` ROW, and this image chains rows heavily.** `SpawnPoolableActorFromClass` reports **59 B**; the pooled acquire reports **1,086 B** and is **3,702 B** over three chained rows. **A linear sweep bounded by the reported size misses the entire tail — which is exactly where the null-return branches live.** Same defect S124 hit on `GoToPhase` (`0x271` vs `0x2C0`): **systemic, not incidental.** Union the rows from `tools/strxref/index/pdata_union.csv` |
| **f** | **"A blank result from `fkdis.py d <rva>` means the page is not decrypted"** | a zero page and a failed decode print the same nothing | **It is a QUERY failure.** Capstone decodes nothing when `<rva>` is not an instruction boundary: `0x56481D0` → one empty line, `0x56481D7` → 109 instructions, **same page**. ⇒ an absence-of-coverage claim manufactured from a mis-anchored address. Print the coverage line **and** re-anchor before believing a blank |
| **g** | **"`propscan.py`'s `gen=` and `off=` columns describe the property"** — the natural things to read off its output | the decoder is real and its `--name` lookup is correct | **`FPropertyParams` has VARIANT per-type layouts, and the generic reader takes `Offset` from the field a bool record uses for `SetBitFunc`.** It prints `off=0x1 gen=WeakObject\|Config` for `bSupportsActorPoolPriming`, and `gen=Bool` for `MaxDepenetrationVelocity`. ⇒ **for bools the ground truth is the disassembled `SetBitFunc` and nothing else** — which is what `boolscan.py` prints and what the whole S130 naming rests on. ⚠ This is also why the offline attempt to name `AActor+0x6C` was **abandoned rather than answered** |
| **h** | ★★★ **"A workflow's final synthesis summarises what its lanes found"** — the orchestration itself | the script computed a lane packet, and the prompt said *"below is everything they found"* | **The packet was never interpolated into the prompt string.** The synthesiser received **no lane input**, silently re-derived the entire subject solo, and opened with *"the six lane reports were not present in my input"*. It was honest and right to say so — **but a less careful agent would have written a confident synthesis of nothing, and six lanes plus six verifiers (3.2 M tokens) would have been lost unread in `journal.jsonl`.** ⇒ **an orchestration bug is an instrument artifact: read the agent's provenance note, and read the journal before trusting a workflow's answer** |
| **i** | — (not a prior belief; recorded because it RECURRED) | — | **FK-22's own signature error — generalising from the variants you opened — was committed AGAIN, on this exact function family.** An agent wrote that `AuthSetSpawnTeamLeader`'s flag feeds only `ALokiDropShip::GetTeamDropLeader`; `IsSpawnTeamLeader` has **three** Angelscript readers, one of which (`QueueCrewForPodSpawn`) is on the leader-pod path being replicated. ⇒ *"the bypass avoids FK-1's stubs"* survives **only under `bIsTeamLeaderPod == false`**, and the route as transcribed from `SpawnDropPodForTeam` passes `true`. **Caught only by adversarial verification** |
| **j** | ★★★ **"`grep -cE '^\| \*\*(★+ )?S[0-9]+-[a-z]+\*\*' docs/method-rules.md` re-derives the instrument-artifact tally"** — written **this session**, to fix the very problem of a retyped total | it is the obvious transcription of the table's own row format, and it returns a plausible number | **POSIX `grep` is byte-oriented: `★` is 3 UTF-8 bytes, so `★+` quantifies only the LAST byte** and cannot match `★★★`. It returned **33 instead of 68** — silently dropping every multi-star row, i.e. **half the table**. ⚠ It would have shipped as the project's own recommended command, so every future re-derivation would have reproduced the same wrong number and looked **self-consistent**. Caught only by **running the instruction before publishing it** and comparing against the Python count (which is correct, because Python applies the regex to a decoded `str`). Fix: `^\| \*\*[^|]*S[0-9]+-[a-z]+\*\*` — no multi-byte literal — which returns **68** and matches Python on **identical sets**, not merely an equal count. Tabulated as **S130-f**. |
| **k** | **“`bpdump <asset> @props` reports whether a Blueprint's CDO overrides a property”** | it is the project's standard instrument for exactly that, and it is right for every asset that has UFunction exports | **The `@props` branch sat behind a `continue` requiring UFunction exports** (`Program.cs:1137`), although it wants a UObject export and needs no UFunction — unlike the `@imports` needle beside it, which was correctly exempted. On a **data-only** Blueprint (0 UFunctions — exactly what `BP_DropPod_Tutorial` is) it printed **`No matching UFunction '@props' found`**, indistinguishable from *“the asset does not set that property”* while actually meaning *“not looked at”* — on the single load-bearing question of the session. **Fixed**, and validated by re-dumping a known-good asset first. ★ Tell: the error names a **UFunction** for a question about **properties** |
...
| ~~**4**~~ | ~~**DropPlane is falsified as reachable**~~ **✅ FELL 2026-08-16 — the wall was fake, but so was the expected answer.** | ~~N=1 against a tutorial-specific variant, with the source itself recording wrong arg types (FK-22)~~ **[M] The 3 `Comp_GameMode_DropPlane*` classes are SIBLINGS (no BP-to-BP inheritance), the general one queries 0 markers, and `LVL_Tutorial` HAS all three markers while `Skylands_WP` has none.** | ~~Read `LokiDropShip.as` for the markers it queries~~ **Done — `docs/fk22-dropphase-reachability.md`. Successor blockers are the stalled phase machine (193/193 `EGP_BeginInit`) and 13 empty C++ impls; next test is grading `GoToPhase` / `BP_AuthSetCurrentPhase`.** |
...
| **A-19** | ★★ **NEW (S122). Re-run the unserved-route sweep AFTER driving the UI.** The §1 sweep is a **lower bound**, not a map — it enumerates only endpoints the client *happened to exercise*, and it missed `setTargetQueues` because nobody clicked an activity tile. Click through every menu surface, then re-run `tools/re/endpoint_surface.py` + `unserved_routes.py`. Each hit is a candidate backend surface, and this method has now produced two (`/mmr/player-ratings/{id}/rank`, `setTargetQueues`) | FK-40e | minutes |
| **A-20** | ★★ **NEW (S122). Hunt other RUNTIME-CONSTRUCTED toggle keys.** `"queue.restrictions."+QueueID` is flown and works; it was invisible to both S121 censuses **by construction**. Grep every bpdump for `Concat_StrStr` feeding `GetFeatureToggle`/`IsFeatureEnabled` — a purely offline sweep that bounds a category nobody knew existed | FK-40d | minutes |
| **A-21** | **NEW (S122). Serve `FPartyMember.IsRankedEligible`.** [M] `UPartyModel.bIsRankedEligible` reads **False** live (`PartyModel+0x0528`), `buildSoloParty` has never served it, and it is the gate `CanControlQueue` bails on at statement 181 — the same shape as the S85 `PersonalizationLoadout` fix. `AccountLevel` (14) and `MasteryLevel` (15) on that struct are likewise unserved | FK-40a | minutes |
| **A-22** | **NEW (S122). Two unserved endpoints with real UI behind them:** `GET /referral/player/{id}` and `/points`. Also `POST /party/parties/{p}/refreshRanks`, which appeared only *after* the player-rank handler existed — serving a surface keeps revealing more surface | FK-40e | minutes |
...
| B-4 | ~~Capture `-State tutorial` during a force-open and re-merge~~ ✅ **DONE, AND THE SUCCESSOR IS RETIRED (S133) — see `docs/fk20-coverage-settled.md`.** Capture S111 (`dumps/tutorial-hero`), re-merge S121 (`merged2`), and **S133 folded in the last 10 orphan images → `dumps/merged6.dump.exe`, 16,694 / 30,281 pages = 55.13 %, the exact union of all 26 images on disk.** ⚠ **The stated successor — "capture hero select / drop phase / a live match / EoG" — is now measured to be worth far less than it reads:** the whole S107→S132 tutorial programme bought **216 pages (0.71 pp)**, the MENU family contributes **more** unique pages than the tutorial family (437 vs 216), **67.91 % of the dark set is unreachable by any game state**, and **125 crash lifetimes + our 26 captures only ever reached 55.27 %** ⇒ the dark 45 % is dark **because the game never ran it**. The lever is **drive new code, then RE-GRADE** — 31 coverage-blindness claims in the repo are already stale. | FK-20, FK-18 | ✅ settled |
...
| **B-10** | ★★★★★ **NEW (S132). Is the hero PLAYABLE at the point the dismount lands it?** The dismount is now a working **deploy primitive** — it places a possessed, un-hidden, collided, gravity-affected hero at a chosen actor on real terrain, where it stands (S132, five calls, three launches). What is unknown is whether it can be *played* from there. ⚠⚠ **The arm that looks like it answers this is DEGENERATE and that is MEASURED:** `RM_PLAY` relocates the hero first (hardcoded teleport), and with that skipped (`KNOTELE=1`) `KFLYMODE` still defaults to `5 = MOVE_Flying`, so the control **moved 2,926 uu at constant Z = 13,240 — 13 km in mid-air**. Only **`play-atlanding-walk`** (`-DKFLYMODE=1`, `.text 944a27728053359e`) can answer it, against a never-dismounted control. Both arms are built and the readings are pre-registered. ⚠ Walking mode carries its own recorded crash history (S75/S81) — grade a death there as the mode hazard, not as the landing point. Also fix the dismount arm's no-GATE-5-candidate branch to REFUSE (it currently proceeds and faults). | `docs/next-session-prompt-s133.md` §1.2 | one launch, arms already built |
| **B-11** | ★★★★ **NEW (S132). Hand-assemble the MOUNT and close the ride end to end.** The fifth wall's round-game-mode value is a dead guard (S131) and its only persistent component-state output is the `PlayersAttached` append — which S132 proved is performable by hand. Everything else in its success tail is a real, named function: `LokiTeleportActor` (REFLECTED), `SetActorEnableCollision`, a `GetServerTime` stamp into `hero+0x1C10`, then the append. ⇒ teleport the hero to the pod, collision off, append, stamp — and the S132 detach drops it. **Board → fly → dismount, with no `.text` write.** ⚠ `SpawnAndMoveLokiCharacter_MoveStep` (`0x55C1B20`) is **not** reflected (raw native address, 2 folds of its own) and may be skippable; **transcribe it before deciding** — free, offline, unstarted. | `docs/next-session-prompt-s133.md` §2 | offline first, then one launch |
