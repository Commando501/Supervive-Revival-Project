...
| **★★★★★ S132 FOURTH UPDATE (2026-08-20) — THE DISMOUNT RUNS, AND IT IS USABLE. THE HERO LEAVES THE POD AND STANDS ON REAL GROUND.** → `docs/s132-dismount-settled.md` | **FIVE detach calls across TWO launches, risk class DATA, zero `.text` writes, zero PI hooks, zero CDO pokes.** The recipe is exactly S131's prediction: append the PlayerState to `ULokiRideableComponent::PlayersAttached` (`+0x130`/`+0x138`/`+0x13C`) with the GAME'S OWN `ResizeGrow` at **`0x00F988D0`** — the same function the fifth wall's own tail calls at `0x55CD75B`, so the ABI and element size are correct **by construction** — then call **`AuthPlayerDetachPlayerFromRidable`** (impl `0x55CCCB0`, thunk `0x5456100`) via the S55 direct `UFunction.Func` thunk. **[M] FLIGHT 1: four dismounts, hero X = 1.45M → 4.86M → 11.65M → 14.43M**, each matching the flying pod's X at its own call time, with run 2's hero Y **BIT-IDENTICAL** to the pod's (`==` on raw doubles in a live read) — a payload fingerprint no static explanation survives. **[M] FLIGHT 2: with a `LokiPlayerStart` passed as `LandingLocationActor`, 1,488,146 uu from the pod at that instant, the hero landed at the PLAYERSTART**, settled to `Z = 90.15` and held position **bit-for-bit across 9 s** while the pod flew on ⇒ **`GetLandingTeleportLocation` consumes its `LandingLocationActor` argument**, and the hero is un-hidden, collided, gravity-affected and standing. **The within-run NEGATIVE CONTROL — the same detach, same component, same primitive, `PlayersAttached` EMPTY — never moved the hero, in all five runs**, against a prediction the shim printed before each call. ⚠⚠ **S131's "expect a PARTIAL dismount" was TOO PESSIMISTIC**: the teleport, un-hide, collision restore and movement restore are every one a real body, and the two `0xF7EC20` folds are void side effects whose returns are **never tested**. ★★ **FREE THREE-WAY RECEIPT on a function with ZERO log strings:** `PlayersAttached.Remove(PS)` at `0x55CCE23` runs on **every** path past GATE 4, so `Num` staying 1 vs dropping to 0 separates "bailed at a gate" from "ran past GATE 4" with no log dependence — observed **1 → 0** every run. ⚠⚠ **TRAP: `ContainsPlayer` scans `PlayersInside` (`+0x120`), NOT `PlayersAttached` (`+0x130`)** (measured at `0x55D0270`) — using it as the append receipt manufactures a false negative on a working append; it was disassembled before being trusted and turned into a pre-registered "MUST still be false" prediction. ★★★ **NEW [M, strong]: `AuthAddPlayer`, `AuthRemovePlayer` and `AuthSetCanJump` are ALSO stripped folds** (`impl 0x0F7EC20`), so the component has **FOUR** empty `Auth*` stubs and **the only reflected writers of either player array do nothing in this client** — which is why both read `Data=0 Num=0 Max=0` in a fully staged world and why a data poke is the only route *by construction*. The obvious shortcut was **checked, not assumed**. ⛔ **`AuthPlayerEnterWorld` (`0x55CCE70`) is FORECLOSED** — its two terminal actions are direct calls to the stripped `0xF7EB50` and it writes **no** actor or component transform. ⛔ **This is a DIAGNOSIS, not a shipping fix**: it writes a live component's replicated array by hand and drives an authority-only entry point. Regression gates `play` / `dropplane_b1only` / `droppod-pe-cdopoke` all MATCH throughout. |
