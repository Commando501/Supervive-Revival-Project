...
for ent,label in [(0x055AB8C0,'ULokiCMC::GetGravityZ  (vt disp 0x4C0)'),
                  (0x055B6AD0,'ULokiCMC::NewFallVelocity (vt disp 0x7A0)')]:
    c=CFGMOD.CFG(im,ent); ins=c.insns
    print(f"\n=========== {label}  entry {ent:#x} ===========")
    print(f"  insns={len(ins)} calls={len(c.calls)} indirect_jmp={len(c.indirect_jumps)} decfail={len(c.decode_failures)} page={im.page_nonzero(ent)}/4096")
    for r in sorted(ins):
        i=ins[r]; ex=''
        for op in i.operands:
            if op.type==X.X86_OP_MEM and op.mem.base==X.X86_REG_RIP:
                ...
                try:
                    b=im.read(t,8)
                    ex=f"  ; ->{t:#x} f={struct.unpack('<f',b[:4])[0]!r} d={struct.unpack('<d',b)[0]!r}"
