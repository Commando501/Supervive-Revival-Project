...
d=json.load(open('stores_v2.json')); hits=d['hits']; fns={int(k):v for k,v in d['fns'].items()}
...
json.dump({t:[[fb,m,obj,why] for fb,m,obj,why in v] for t,v in tiers.items()}, open('cmc_tiers.json','w'))
