...
def main():
    ...
    print()
    print("### RANK-1 VERDICT (the bisector) ###")
    pl = B.get("R1.latch@0x16C8")
    bl = A.get("R1.latch@0x16C8")
    if pl != 1:
        print("  !! CONTROL FAILED: the PLAYER's latch reads %s, expected 1." % fmt(pl))
        print("     Either the offset is wrong or the player's PerformMovement has never reached")
        print("     StartNewPhysics either. THE BISECTOR IS UNINTERPRETABLE THIS SITTING.")
    elif bl is None:
        print("  bot latch unreadable -- no verdict.")
    elif bl == 1:
        print("  BOT LATCH = 1  ==>  S1.  PerformMovement REACHED StartNewPhysics and the pawn is")
        print("     still frozen  ==>  the DeltaTime kill (ULokiCMC HitStop) is the leading cause.")
        print("     S2 (an early-out at or above PerformMovement) is ELIMINATED.")
        print("     Confirm with R2: TimeSinceFallingStart must be FROZEN while the player's advances.")
    else:
        print("  BOT LATCH = 0  ==>  S2.  The ladder broke at or ABOVE engine PerformMovement.")
        print("     S1 is dead unless HitStop also fires upstream. Localise with R4 and R5 above:")
        print("       UpdatedComponent NULL / RF_Garbage set  -> E1/E3 (HasValidData)")
        print("       Mobility != Movable                     -> E2 (ShouldSkipUpdate)")
        print("       Role != 3                               -> E6")
        print("       Controller NULL + bRunPhysics clear     -> E7  (and that has a one-bit lever)")
    print()
