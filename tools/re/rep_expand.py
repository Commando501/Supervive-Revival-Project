# Expand a UScriptStruct the way FRepLayout does, over live client RPM, to compare
# with the stub's expansion. Recurse struct members unless STRUCT_NetSerializeNative(0x400);
# arrays emit a cmd then recurse Inner. usage: rep_expand.py <PID> <BASE-hex> <StructObj-hex>
# Offsets (this build): Child@+0x58 Next@+0x18 Name@+0x20 FFieldClass@+0x08 Flags@+0x38
#                       StructProp.Struct@+0x70 ArrayProp.Inner@+0x78 UScriptStruct.Flags@+0xB8
import ctypes, sys
...
CPF_RepSkip=0x80000000; STRUCT_NetSer=0x400
cmd=[0]
def expand(f, depth):
    ind="  "*depth
    tp=fldtype(f); nm=fldname(f)
    if tp=="ArrayProperty":
    ...
    if tp=="StructProperty":
        st=p(f+0x70)
        sf=structflags(st) if looksptr(st) else 0
        stn=""
        if looksptr(st):
            b=rpm(st+0x20,4); stn=fname(u32(b,0)) if b else "?"
        if sf & STRUCT_NetSer:
            print(f"{ind}[{cmd[0]}] NETSER-STRUCT {nm} ({stn} flags=0x{sf:X})"); cmd[0]+=1
            return
        print(f"{ind}(recurse {stn} flags=0x{sf:X})")
        c=p(st+0x58); i=0
...
print(f"=== CLIENT expansion of struct @0x{ROOT:X} (flags=0x{structflags(ROOT):X}) ===")
c=p(ROOT+0x58); i=0
while looksptr(c) and i<40:
    if not (fldflags(c) & CPF_RepSkip): expand(c,0)
    c=p(c+0x18); i+=1
print(f"=== CLIENT total leaf cmds = {cmd[0]} ===")
