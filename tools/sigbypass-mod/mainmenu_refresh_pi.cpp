// mainmenu_refresh_pi — SESSION 50 Phase-3, PIVOT to ProcessInternal.
// The ProcessEvent hook (0x12C5A10, vtable slot 56) fired ZERO times even under heavy real clicking, so it is
// NOT this build's BP dispatch path. This variant instead hooks ProcessInternal (0x7FF683DC54A0 = the shared BP
// VM entry every BP UFunction runs through) to get reliable GAME-THREAD execution, and from there CALLS
// ProcessEvent(subject, Refresh, null) directly to run the refresh. Hit counters confirm whether ProcessInternal
// is hot during clicks. Mirror (picker SelectedHeroAsset -> member HeroAssetID) is unchanged and proven.
//
// ProcessInternal @ base+0x13454A0, prologue 48 89 5C 24 08 (mov [rsp+8],rbx; 5 relocatable bytes).
// ProcessEvent     @ base+0x12C5A10 (called directly to invoke Refresh; NOT hooked here).
// Build:  clang++ -shared -O2 mainmenu_refresh_pi.cpp -o mainmenu_refresh_pi.dll -lkernel32
// Inject: tools/inject/inject.exe mmap <PID> tools/sigbypass-mod/mainmenu_refresh_pi.dll
// Marker: docs/mainmenu-refresh-pi-marker.txt   Crash: docs/mainmenu-refresh-pi-crash.txt
#include <windows.h>
#include <tlhelp32.h>
#include <cstdint>
#include <cstdio>
...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi-marker.txt";
static const char* kCrashPath  = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi-crash.txt";

constexpr uintptr_t kPiRva        = 0x13454A0;    // ProcessInternal (HOOK target)
constexpr uintptr_t kPeRva        = 0x12C5A10;    // ProcessEvent (CALL target to invoke Refresh)
...
static volatile PFN_PE g_tramp = nullptr;       // trampoline to original ProcessInternal (passthrough)
static PFN_PE      g_processEvent = nullptr;    // real ProcessEvent, called to invoke Refresh
...
static uint8_t*    g_pi        = nullptr;        // ProcessInternal entry (hooked)
static uint8_t     g_stolen[5] = {0};
static volatile long g_pending = 0;
static volatile long g_inHook  = 0;
static volatile long g_done    = 0;
static volatile long g_refreshes = 0;
static volatile long g_hitsAny = 0;             // every ProcessInternal entry (any thread)
static volatile long g_hitsGT  = 0;             // ProcessInternal entries on the game thread
...
// game-thread rendezvous at ProcessInternal entry
extern "C" void OnBP(void*,void*,void*){
    InterlockedIncrement(&g_hitsAny);
    DWORD tid=GetCurrentThreadId();
    if(tid==g_gameTid) InterlockedIncrement(&g_hitsGT);
    if(!g_pending) return;
    if(g_inHook)   return;
    if(tid!=g_gameTid) return;
    g_inHook=1;
    for(int i=0;i<g_nsubj;i++){ if(g_subjects[i]&&g_refreshFn&&g_processEvent) g_processEvent(g_subjects[i],g_refreshFn,nullptr); }
...
static bool InstallHook(){ if(!g_pi||!g_stub)return false; int32_t rel=(int32_t)((intptr_t)g_stub-((intptr_t)g_pi+5)); DWORD op=0; if(!VirtualProtect(g_pi,5,PAGE_EXECUTE_READWRITE,&op))return false; g_pi[0]=0xE9;g_pi[1]=(uint8_t)rel;g_pi[2]=(uint8_t)(rel>>8);g_pi[3]=(uint8_t)(rel>>16);g_pi[4]=(uint8_t)(rel>>24); DWORD d=0;VirtualProtect(g_pi,5,op,&d); return true; }
static void UninstallHook(){ if(!g_pi)return; DWORD op=0; if(VirtualProtect(g_pi,5,PAGE_EXECUTE_READWRITE,&op)){ memcpy(g_pi,g_stolen,5); DWORD d=0; VirtualProtect(g_pi,5,op,&d); } }
...
static void ResolveSubjects(){
    ...
    if(!g_subjClass)return; g_nsubj=0; g_member=0;
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] mainmenu_refresh_pi worker started\r\n");
    ...
    Markerf("[0] modBase=0x%llX processEvent=%p\r\n",(unsigned long long)g_modBase,(void*)g_processEvent);
    ...
    DWORD dl=GetTickCount()+120000; while(GetTickCount()<dl){ ResolveSubjects(); if(g_nsubj>0&&g_refreshFn&&g_member)break; Sleep(500);}
    ...
    Markerf("[2] resolved subjects=%d refreshFn=%p member=0x%llX\r\n",g_nsubj,g_refreshFn,(unsigned long long)g_member);
    g_pi=(uint8_t*)(g_modBase+kPiRva);
    if(!SafeReadable(g_pi,5)||memcmp(g_pi,kPiProlog,5)!=0){Markerf("[2] FAIL PI prologue %02X %02X %02X %02X %02X\r\n",g_pi[0],g_pi[1],g_pi[2],g_pi[3],g_pi[4]);return 4;}
    memcpy(g_stolen,g_pi,5); g_stub=BuildHook((uintptr_t)g_pi,g_stolen); if(!g_stub||!g_tramp){Marker("[2] FAIL BuildHook\r\n");return 5;}
    Markerf("[3] ProcessInternal hook built (tramp=%p stub=%p). Open HUNTERS + click a hunter.\r\n",(void*)g_tramp,(void*)g_stub);
    ...
    while(GetTickCount()-start<28800000u){
        ...
        if(desired){ uint32_t cur=ReadMemberHero(); if(cur!=desired){ WriteMemberHero(desired); if(desired!=lastLogged){ char s[64]="?"; GetFNameStr(desired,s,sizeof(s)); Markerf("[MIRROR] member <- Hero:%s\r\n",s); lastLogged=desired; }
            if(!hooked){ g_pending=1; g_done=0; if(InstallHook()){ hooked=true; hookedAt=GetTickCount(); Marker("[armed] ProcessInternal hook installed\r\n"); } } } }
        if(hooked){ if(g_done){ UninstallHook(); hooked=false; char s[64]="?"; GetFNameStr(ReadMemberHero(),s,sizeof(s)); Markerf("[REFRESH DONE] Refresh ran on game thread (#%ld). Center should show Hero:%s.\r\n",(long)g_refreshes,s); }
            else if(GetTickCount()-hookedAt>=6000){ UninstallHook(); hooked=false; g_pending=0; Markerf("[refresh timeout] hitsAny=%ld hitsGT=%ld (PI not hot while armed).\r\n",(long)g_hitsAny,(long)g_hitsGT); } }
        if(GetTickCount()-lastHb>=10000){ char mh[64]="<none>"; GetFNameStr(ReadMemberHero(),mh,sizeof(mh)); Markerf("[hb] member=Hero:%s desired=%u refreshes=%ld hitsAny=%ld hitsGT=%ld hooked=%d\r\n",mh,desired,(long)g_refreshes,(long)g_hitsAny,(long)g_hitsGT,hooked?1:0); lastHb=GetTickCount(); }
        Sleep(60);
    }
