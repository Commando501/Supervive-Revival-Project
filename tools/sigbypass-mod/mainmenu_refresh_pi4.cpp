...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi4-marker.txt";
static const char* kCrashPath  = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi4-crash.txt";
...
static volatile long g_logging = 0;                // trace UFunctions during the Refresh cascade
static uint32_t    g_seen[512] = {0};
...
extern "C" void OnBP(void* /*ctx*/, void* frame, void*){
    if(g_logging && LooksLikePtr((uintptr_t)frame)){          // trace the executing UFunction (FFrame.Node@+0x10)
        uintptr_t node=*(uintptr_t*)((uint8_t*)frame+0x10);
        if(LooksLikePtr(node)){
            uint32_t nid=*(uint32_t*)(node+0x20);
            long n=g_nseen; bool seen=false; for(long i=0;i<n;i++){ if(g_seen[i]==nid){seen=true;break;} }
            if(!seen && n<512){ g_seen[n]=nid; g_nseen=n+1; }
        }
    }
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] mainmenu_refresh_pi4 (subject.Refresh + UFunction-trace of the cascade) started\r\n");
    ...
    while(GetTickCount()-start<28800000u){
        ...
        if(desired){ uint32_t cur=ReadMemberHero(); if(cur!=desired){ WriteMemberHero(desired); if(desired!=lastLogged){ char s[64]="?"; GetFNameStr(desired,s,sizeof(s)); Markerf("[MIRROR] member <- Hero:%s\r\n",s); LogSpawners("before"); lastLogged=desired; }
            if(!hooked){ g_pending=1; g_done=0; if(InstallHook()){ hooked=true; hookedAt=GetTickCount(); Marker("[armed] calling subject.Refresh on next game-thread BP...\r\n"); } } } }
        if(hooked){ if(g_done){ UninstallHook(); hooked=false; Markerf("[CALLED] subject.Refresh x%d ran on game thread (#%ld). Spawner TargetAssetID AFTER:\r\n",g_nsubj,(long)g_calls); LogSpawners("after");
                Markerf("[TRACE] %ld distinct UFunctions executed during the Refresh cascade:\r\n",(long)g_nseen);
                long n=g_nseen; for(long i=0;i<n;i++){ char nm[128]="?"; GetFNameStr(g_seen[i],nm,sizeof(nm)); Markerf("   %s\r\n",nm); } }
