...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi5-marker.txt";
static const char* kCrashPath  = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi5-crash.txt";
...
static void*       g_updateSlot = nullptr;           // pi5: POSITIVE CONTROL — UpdateSlot(int) writes Slot@+0xE8
static uintptr_t   g_subject0  = 0;                  // the Slot==0 subject
static uint64_t    g_param     = 0;                  // param frame for UpdateSlot: {int32 Slot=0}
...
extern "C" void OnBP(void*,void*,void*){
    ...
    g_inHook=1;
    // POSITIVE CONTROL: UpdateSlot(g_subject0, {Slot=0}) — if ProcessEvent invokes it, Slot@+0xE8 goes 99->0.
    g_param=0; if(g_subject0 && g_updateSlot && g_processEvent) g_processEvent((void*)g_subject0,g_updateSlot,&g_param);
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] mainmenu_refresh_pi5 (POSITIVE CONTROL: UpdateSlot via ProcessEvent) started\r\n");
    ...
    while(GetTickCount()-start<28800000u){
        ...
        if(desired){ uint32_t cur=ReadMemberHero(); if(cur!=desired){ WriteMemberHero(desired); if(desired!=lastLogged){ char s[64]="?"; GetFNameStr(desired,s,sizeof(s)); Markerf("[MIRROR] member <- Hero:%s\r\n",s); lastLogged=desired; }
            if(!hooked){ *(uint32_t*)(g_subject0+0xE8)=99; g_pending=1; g_done=0; if(InstallHook()){ hooked=true; hookedAt=GetTickCount(); Markerf("[armed] subject0.Slot sentinel=99; calling UpdateSlot(subject0,{0}) via ProcessEvent...\r\n"); } } } }
        if(hooked){ if(g_done){ UninstallHook(); hooked=false; uint32_t after=*(uint32_t*)(g_subject0+0xE8);
                Markerf("[VERDICT] UpdateSlot called #%ld. subject0.Slot AFTER = %u  ==>  %s\r\n",(long)g_calls,after, after==0?"ProcessEvent WORKS (invoked UpdateSlot: 99->0)":(after==99?"ProcessEvent NO-OP (Slot stayed 99, function NOT invoked)":"UNEXPECTED")); }
