...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi6-marker.txt";
static const char* kCrashPath  = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi6-crash.txt";
...
static void*       g_onPartyUpd = nullptr;         // spawner OnPartyUpdated UFunction (unused in pi3; kept for logging resolve)
static uintptr_t   g_spawnClass = 0;
static void*       g_spawners[8] = {0};
static int         g_nspawn    = 0;
static uintptr_t   g_subjClass = 0;                // pi3: call subject.Refresh (proven no-crash)
static void*       g_subjects[8] = {0};
static int         g_nsubj    = 0;
static void*       g_refreshFn = nullptr;
static void*       g_updateSlot = nullptr;           // pi6: OPTION B test — UpdateSlot(int) via ProcessInternal+FFrame
static uintptr_t   g_subject0  = 0;                  // the Slot==0 subject
static uint64_t    g_param     = 0;
// Option B: capture a live FFrame as a template (all fields valid), repoint Node/Object/Code/Locals to
// UpdateSlot(subject0,{0}), and call the ORIGINAL ProcessInternal (via g_tramp). FFrame: Node@+0x10,
// Object@+0x18, Code@+0x20, Locals@+0x28, MostRecentProp@+0x30/+0x38. UFunction Script(Data ptr)@+0x68.
static uint8_t     g_template[0x180] = {0};
static uint8_t     g_myframe [0x180] = {0};
static uint64_t    g_locals  [24]    = {0};          // params+locals buffer (Slot=0 at [0])
...
extern "C" void OnBP(void* /*ctx*/, void* frame, void*){
    ...
    if(!LooksLikePtr((uintptr_t)frame)) return;        // need a live FFrame as template
    InterlockedIncrement(&g_hitsGT);
    g_inHook=1;
    // OPTION B: build an FFrame from the live template, repoint to UpdateSlot(subject0,{0}), call ProcessInternal.
    ...
    *(void**)(g_myframe+0x10)=g_updateSlot;                                           // Node
    *(void**)(g_myframe+0x18)=(void*)g_subject0;                                      // Object
    *(uint64_t*)(g_myframe+0x20)=*(uint64_t*)((uint8_t*)g_updateSlot+0x68);           // Code = Script.Data
    *(void**)(g_myframe+0x28)=(void*)g_locals;                                        // Locals (Slot=0 at [0])
    *(uint64_t*)(g_myframe+0x30)=0; *(uint64_t*)(g_myframe+0x38)=0;                    // MostRecentProperty/Addr
    if(g_tramp) g_tramp((void*)g_subject0, g_myframe, g_result);                      // = ProcessInternal(Ctx,Stack,Res)
...
static void Resolve(){
    ...
    if(!g_spawnClass||!g_subjClass){ for(int ci=0;ci<numChunks && (!g_spawnClass||!g_subjClass);ci++){ if(!SafeReadable((void*)(objectsPtr+ci*8),8))break; uintptr_t chunk=*(uintptr_t*)(objectsPtr+ci*8); if(!LooksLikePtr(chunk))continue; int cnt=(ci==numChunks-1)?(numEl-ci*PERCHUNK):PERCHUNK;
        for(int j=0;j<cnt;j++){ uintptr_t item=chunk+(uintptr_t)j*ITEMSTRIDE; if(!SafeReadable((void*)item,8))continue; uintptr_t obj=*(uintptr_t*)item; if(!LooksLikePtr(obj))continue;
            if(!g_spawnClass && NameIs(obj,"BP_MainMenuSpawner_MainMenu_PartySlot_C")){ uintptr_t cls=ClassOf(obj); char cn[96]="?"; if(cls)GetFNameStr(NameId(cls),cn,sizeof(cn)); if(strstr(cn,"Class")) g_spawnClass=obj; }
            else if(!g_subjClass && NameIs(obj,"Comp_MainMenu_PartySlotSubject_C")){ uintptr_t cls=ClassOf(obj); char cn[96]="?"; if(cls)GetFNameStr(NameId(cls),cn,sizeof(cn)); if(strstr(cn,"Class")) g_subjClass=obj; } } } }
    if(!g_spawnClass||!g_subjClass)return; g_nspawn=0; g_nsubj=0; g_member=0;
    for(int ci=0;ci<numChunks;ci++){ if(!SafeReadable((void*)(objectsPtr+ci*8),8))break; uintptr_t chunk=*(uintptr_t*)(objectsPtr+ci*8); if(!LooksLikePtr(chunk))continue; int cnt=(ci==numChunks-1)?(numEl-ci*PERCHUNK):PERCHUNK;
        for(int j=0;j<cnt;j++){ uintptr_t item=chunk+(uintptr_t)j*ITEMSTRIDE; if(!SafeReadable((void*)item,8))continue; uintptr_t obj=*(uintptr_t*)item; if(!LooksLikePtr(obj))continue;
            ...
            else if(ClassOf(obj)==g_subjClass && NameIs(obj,"Comp_MainMenu_PartySubject")){ if(g_nsubj<8)g_subjects[g_nsubj++]=(void*)obj; if(SafeReadable((void*)(obj+0xE8),4) && *(uint32_t*)(obj+0xE8)==0) g_subject0=obj; }
            else if(!g_refreshFn && NameIs(obj,"Refresh") && OuterOf(obj)==g_subjClass) g_refreshFn=(void*)obj;
            else if(!g_updateSlot && NameIs(obj,"UpdateSlot") && OuterOf(obj)==g_subjClass) g_updateSlot=(void*)obj;
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] mainmenu_refresh_pi6 (OPTION B: UpdateSlot via ProcessInternal + built FFrame) started\r\n");
    ...
    DWORD dl=GetTickCount()+120000; while(GetTickCount()<dl){ Resolve(); if(g_updateSlot&&g_subject0&&g_member)break; Sleep(500);}
    if(!g_updateSlot||!g_subject0||!g_member){Markerf("[2] FAIL resolve updateSlot=%p subject0=0x%llX member=0x%llX\r\n",g_updateSlot,(unsigned long long)g_subject0,(unsigned long long)g_member);return 3;}
    Markerf("[2] resolved updateSlot=%p subject0=0x%llX (Slot@+0xE8=%u) member=0x%llX\r\n",g_updateSlot,(unsigned long long)g_subject0,*(uint32_t*)(g_subject0+0xE8),(unsigned long long)g_member);
    ...
    while(GetTickCount()-start<28800000u){
        ...
        if(desired){ uint32_t cur=ReadMemberHero(); if(cur!=desired){ WriteMemberHero(desired); if(desired!=lastLogged){ char s[64]="?"; GetFNameStr(desired,s,sizeof(s)); Markerf("[MIRROR] member <- Hero:%s\r\n",s); lastLogged=desired; }
            if(!hooked){ *(uint32_t*)(g_subject0+0xE8)=99; g_pending=1; g_done=0; if(InstallHook()){ hooked=true; hookedAt=GetTickCount(); Markerf("[armed] subject0.Slot sentinel=99; calling UpdateSlot(subject0,{0}) via ProcessInternal+FFrame...\r\n"); } } } }
        if(hooked){ if(g_done){ UninstallHook(); hooked=false; uint32_t after=*(uint32_t*)(g_subject0+0xE8);
                Markerf("[VERDICT] UpdateSlot called #%ld. subject0.Slot AFTER = %u  ==>  %s\r\n",(long)g_calls,after, after==0?"OPTION B WORKS (ProcessInternal+FFrame invoked UpdateSlot: 99->0)":(after==99?"OPTION B NO-OP (Slot stayed 99)":"OTHER (frame ran, wrote something else)")); }
