...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi7-marker.txt";
static const char* kCrashPath  = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi7-crash.txt";
...
extern "C" void OnBP(void* /*ctx*/, void* frame, void*){
    ...
    uint64_t code=*(uint64_t*)((uint8_t*)g_refreshFn+0x68);            // Refresh Script.Data (bytecode)
    for(int i=0;i<g_nsubj;i++){
        if(!g_subjects[i]||!g_refreshFn||!g_tramp) continue;
        memset(g_locals,0,sizeof(g_locals)); memset(g_result,0,sizeof(g_result));
        memcpy(g_myframe, g_template, sizeof(g_myframe));
        *(void**)(g_myframe+0x10)=g_refreshFn;                          // Node
        *(void**)(g_myframe+0x18)=g_subjects[i];                        // Object
        *(uint64_t*)(g_myframe+0x20)=code;                              // Code
        *(void**)(g_myframe+0x28)=(void*)g_locals;                      // Locals
        *(uint64_t*)(g_myframe+0x30)=0; *(uint64_t*)(g_myframe+0x38)=0; // MostRecentProperty/Addr
        g_tramp(g_subjects[i], g_myframe, g_result);                    // = ProcessInternal(subject, Stack, Result)
    }
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] mainmenu_refresh_pi7 (OPTION B: Refresh via ProcessInternal+FFrame) started\r\n");
    ...
    Markerf("[2] resolved subjects=%d refreshFn=%p spawners=%d member=0x%llX\r\n",g_nsubj,g_refreshFn,g_nspawn,(unsigned long long)g_member);
    LogSpawners("init");
    ...
    while(GetTickCount()-start<28800000u){
        ...
        if(desired){ uint32_t cur=ReadMemberHero(); if(cur!=desired){ WriteMemberHero(desired); if(desired!=lastLogged){ char s[64]="?"; GetFNameStr(desired,s,sizeof(s)); Markerf("[MIRROR] member <- Hero:%s\r\n",s); LogSpawners("before"); lastLogged=desired; }
            if(!hooked){ g_pending=1; g_done=0; if(InstallHook()){ hooked=true; hookedAt=GetTickCount(); Marker("[armed] calling Refresh via ProcessInternal+FFrame on next game-thread BP...\r\n"); } } } }
        if(hooked){ if(g_done){ UninstallHook(); hooked=false; Markerf("[CALLED] Refresh x%d ran on game thread (#%ld). Spawner TargetAssetID AFTER (should change to the pick):\r\n",g_nsubj,(long)g_calls); LogSpawners("after"); }
