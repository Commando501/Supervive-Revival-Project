...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi8-marker.txt";
static const char* kCrashPath  = "G:\\git\\Supervive Revival Project\\docs\\mainmenu-refresh-pi8-crash.txt";
...
constexpr uintptr_t MEMBER_ONCHANGED = 0x88;       // OnHeroAssetIDChanged MulticastInlineDelegate (Data@+0x88, Num@+0x90)
...
// OPTION B (PROVEN in pi6): invoke a BP function on the game thread by capturing a live FFrame template,
// repointing Node@+0x10 / Object@+0x18 / Code@+0x20 (=UFunc Script.Data@+0x68) / Locals@+0x28, and calling
// the ORIGINAL ProcessInternal via g_tramp. Bypasses ProcessEvent's guard (which no-ops re-entrant calls).
static uint8_t     g_template[0x180] = {0};
static uint8_t     g_myframe [0x180] = {0};
static uint64_t    g_locals  [24]    = {0};
static uint64_t    g_result  [4]     = {0};
// pi8: broadcast member.OnHeroAssetIDChanged (invoke handlers) so the HUNTERS preview / collection / mastery
// refresh live, exactly like a real pick. All 3 handlers are parameterless (OnPartyHeroUpdated has 0 params).
// Cache the handler UFuncs by name+outer (they are stable per-run); resolve handler OBJECTS live from the delegate.
struct HFn { uint32_t nameid; uintptr_t outer; void* ptr; };
static HFn         g_hfn[16] = {0};
static int         g_nhfn    = 0;
static const char* kHandlerNames[3] = {"OnPartyHeroUpdated","SetSelectedHero","HeroIDChanged"};
...
// Invoke a parameterless BP UFunction on the game thread via option B (ProcessInternal + built FFrame).
static void InvokeBP(void* obj, void* ufunc){
    ...
    *(void**)(g_myframe+0x10)=ufunc;                                    // Node
    *(void**)(g_myframe+0x18)=obj;                                      // Object
    *(uint64_t*)(g_myframe+0x20)=*(uint64_t*)((uint8_t*)ufunc+0x68);    // Code = Script.Data
    *(void**)(g_myframe+0x28)=(void*)g_locals;                          // Locals
    *(uint64_t*)(g_myframe+0x30)=0; *(uint64_t*)(g_myframe+0x38)=0;
    g_tramp(obj, g_myframe, g_result);
}
extern "C" void OnBP(void* /*ctx*/, void* frame, void*){
    if(!g_pending) return;
    if(g_inHook)   return;
    if(GetCurrentThreadId()!=g_gameTid) return;
    if(!LooksLikePtr((uintptr_t)frame)) return;                         // need a live FFrame template
    ...
    if(g_member && SafeReadable((void*)(g_member+MEMBER_ONCHANGED),16)){
        ...
        if(LooksLikePtr(data) && num<=32 && SafeReadable((void*)data,(size_t)num*16)){
            for(uint32_t k=0;k<num;k++){
                uint32_t objIdx=*(uint32_t*)(data+(uintptr_t)k*16); uint32_t fn=*(uint32_t*)(data+(uintptr_t)k*16+8);
                uintptr_t hobj=ObjAt(objIdx); if(!LooksLikePtr(hobj)) continue;
                uintptr_t hcls=ClassOf(hobj); if(!hcls) continue;
                for(int c=0;c<g_nhfn;c++){ if(g_hfn[c].nameid==fn && g_hfn[c].outer==hcls){ InvokeBP((void*)hobj, g_hfn[c].ptr); break; } }
            }
        }
    }
...
static void Resolve(){
    ...
    if(!g_spawnClass||!g_subjClass)return; g_nspawn=0; g_nsubj=0; g_member=0; g_nhfn=0;
    for(int ci=0;ci<numChunks;ci++){ if(!SafeReadable((void*)(objectsPtr+ci*8),8))break; uintptr_t chunk=*(uintptr_t*)(objectsPtr+ci*8); if(!LooksLikePtr(chunk))continue; int cnt=(ci==numChunks-1)?(numEl-ci*PERCHUNK):PERCHUNK;
        for(int j=0;j<cnt;j++){ uintptr_t item=chunk+(uintptr_t)j*ITEMSTRIDE; if(!SafeReadable((void*)item,8))continue; uintptr_t obj=*(uintptr_t*)item; if(!LooksLikePtr(obj))continue;
            ...
            else if(ClassOf(obj)==g_subjClass && g_nsubj<8 && NameIs(obj,"Comp_MainMenu_PartySubject")) g_subjects[g_nsubj++]=(void*)obj;
            ...
            else if(g_nhfn<16 && ClassNameIs(obj,"Function")){ char fn[96]; if(GetFNameStr(NameId(obj),fn,sizeof(fn))){ for(int t=0;t<3;t++) if(strcmp(fn,kHandlerNames[t])==0){ uintptr_t ou=OuterOf(obj); if(ou){ g_hfn[g_nhfn].nameid=NameId(obj); g_hfn[g_nhfn].outer=ou; g_hfn[g_nhfn].ptr=(void*)obj; g_nhfn++; } break; } } } } }
}
...
static DWORD WINAPI Worker(LPVOID){
    ...
    if(g_nsubj==0||!g_refreshFn||g_nspawn==0||!g_member){Markerf("[2] FAIL resolve subj=%d refreshFn=%p spawners=%d member=0x%llX\r\n",g_nsubj,g_refreshFn,g_nspawn,(unsigned long long)g_member);return 3;}
