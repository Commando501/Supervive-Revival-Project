// missions_assettest — SESSION 52, "untried avenue": can mission DAs be RESOLVED via a native call?
// Calls LokiAssetManager::GetLokiDataAsset(FPrimaryAssetId) via the verified ProcessEvent primitive
// for a mission id (Mission:ArmoryDaily_PlayAGame). Non-null return => mission assets DO resolve
// (manual model-construction path becomes viable). Null => asset resolution is the confirmed blocker.
// Also calls it for the DA_Hero:Alchemist sanity control (that DOES resolve) to prove the call works.
//
// GetLokiDataAsset: [in] StructProperty PrimaryAssetId(16) ; [ret] ObjectProperty(8).
// FPrimaryAssetId = {FName Type(8), FName Name(8)}. FNames(this exe): Mission=0x000162B8,
//   ArmoryDaily_PlayAGame=0x003FB29E ; Hero=0x0001A568 (control), Alchemist=<resolved live>.
// Build:  clang++ -shared -O2 missions_assettest.cpp -o missions_assettest.dll -lkernel32
// Inject: tools/inject/inject.exe mmap <PID> tools/sigbypass-mod/missions_assettest.dll
// Marker: docs/missions-assettest-marker.txt   (ONE ProcessInternal-hooking shim per client!)
#include <windows.h>
#include <tlhelp32.h>
#include <cstdint>
#include <cstdio>
...
static const char* kMarkerPath="G:\\git\\Supervive Revival Project\\docs\\missions-assettest-marker.txt";
...
constexpr uintptr_t CLASS_OFF=0x18,NAME_OFF=0x20,OUTER_OFF=0x28;
...
static uintptr_t g_mgr=0; static void* g_getAsset=nullptr;
...
// two tests: mission + a hero control
static uint32_t g_missName=0x003FB29E, g_missType=0x000162B8;    // Mission:ArmoryDaily_PlayAGame
static uint32_t g_ctlName=0, g_ctlType=0x0001A568;               // Hero:<Alchemist>  (name resolved live)
static uint64_t g_params[4]={0};
static uintptr_t g_missObj=0, g_ctlObj=0;
...
static uintptr_t CallGetAsset(uint32_t type,uint32_t name){
    memset(g_params,0,sizeof(g_params));
    *(uint32_t*)((uint8_t*)g_params+0x00)=type; *(uint32_t*)((uint8_t*)g_params+0x04)=0;   // Type FName
    *(uint32_t*)((uint8_t*)g_params+0x08)=name; *(uint32_t*)((uint8_t*)g_params+0x0C)=0;   // Name FName
    // [16..23] = ReturnValue
    g_processEvent((void*)g_mgr,g_getAsset,g_params);
    return (uintptr_t)g_params[2];
}
extern "C" void OnBP(void*,void*,void*){
    ...
    g_missObj=CallGetAsset(g_missType,g_missName);
    if(g_ctlName) g_ctlObj=CallGetAsset(g_ctlType,g_ctlName);
...
static void Resolve(){
    uintptr_t oo=g_modBase+kObjObjectsRva;if(!SafeReadable((void*)oo,0x18))return;uintptr_t objectsPtr=*(uintptr_t*)oo;int32_t numEl=*(int32_t*)(oo+0x14);if(!LooksLikePtr(objectsPtr)||numEl<=0||numEl>8000000)return;int numChunks=(numEl+PERCHUNK-1)/PERCHUNK;uintptr_t lamClass=0,lamInst=0,lamCdo=0;
    for(int ci=0;ci<numChunks;ci++){if(!SafeReadable((void*)(objectsPtr+ci*8),8))break;uintptr_t chunk=*(uintptr_t*)(objectsPtr+ci*8);if(!LooksLikePtr(chunk))continue;int cnt=(ci==numChunks-1)?(numEl-ci*PERCHUNK):PERCHUNK;for(int j=0;j<cnt;j++){uintptr_t item=chunk+(uintptr_t)j*ITEMSTRIDE;if(!SafeReadable((void*)item,8))continue;uintptr_t obj=*(uintptr_t*)item;if(!LooksLikePtr(obj))continue;
        if(ClassNameIs(obj,"LokiAssetManager")){char nm[96];if(GetFNameStr(NameId(obj),nm,sizeof(nm))){if(strncmp(nm,"Default__",9)==0)lamCdo=obj;else lamInst=obj;}}
        // resolve the Alchemist control name id from a loaded hero asset name (BP_HeroAsset_Alchemist)
        if(!g_ctlName){char nm[96];if(GetFNameStr(NameId(obj),nm,sizeof(nm))&&strcmp(nm,"Alchemist")==0)g_ctlName=NameId(obj);}
    }}
    g_mgr = lamInst? lamInst : lamCdo;
    if(g_mgr){ lamClass=ClassOf(g_mgr);
        uintptr_t f=0;if(SafeReadable((void*)(lamClass+0x50),8))f=*(uintptr_t*)(lamClass+0x50);int i=0;
        while(LooksLikePtr(f)&&i<40){if(NameIs(f,"GetLokiDataAsset")){g_getAsset=(void*)f;break;}uintptr_t nx=0;if(SafeReadable((void*)(f+0x30),8))nx=*(uintptr_t*)(f+0x30);f=nx;i++;}
    }
}
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_assettest (GetLokiDataAsset for a mission) started\r\n");
    ...
    DWORD dl=GetTickCount()+120000;while(GetTickCount()<dl){Resolve();if(g_mgr&&g_getAsset)break;Sleep(500);}
    if(!g_mgr||!g_getAsset){Markerf("[2] FAIL resolve mgr=0x%llX getAsset=%p\r\n",(unsigned long long)g_mgr,g_getAsset);return 3;}
    Markerf("[2] mgr=0x%llX GetLokiDataAsset=%p ctlName(Alchemist)=0x%X\r\n",(unsigned long long)g_mgr,g_getAsset,g_ctlName);
    ...
    Marker("[3] armed GetLokiDataAsset calls (mission + hero control)...\r\n");
    ...
    char mc[96]="<null>",cc[96]="<null>";
    if(LooksLikePtr(g_missObj))ClassName(g_missObj,mc,sizeof(mc));
    if(LooksLikePtr(g_ctlObj))ClassName(g_ctlObj,cc,sizeof(cc));
    Markerf("[4] MISSION  GetLokiDataAsset(Mission:ArmoryDaily_PlayAGame) = 0x%llX class=%s\r\n",(unsigned long long)g_missObj,mc);
    Markerf("[4] CONTROL  GetLokiDataAsset(Hero:Alchemist)               = 0x%llX class=%s\r\n",(unsigned long long)g_ctlObj,cc);
    if(LooksLikePtr(g_missObj)) Marker("[4] => MISSION ASSETS RESOLVE. Manual model-construction path is viable.\r\n");
    else Marker("[4] => mission asset does NOT resolve (control shows if the call works). Resolution is the blocker.\r\n");
    return 0;
}
