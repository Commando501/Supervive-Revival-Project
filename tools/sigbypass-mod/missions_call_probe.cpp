// missions_call_probe — SESSION 52, Route A Phase-2a: can we call a NATIVE MissionsModel method
// on the game thread via ProcessEvent? (pi8's ProcessInternal+FFrame trick only runs BP bytecode;
// these methods are native/Script.Num==0, so we must use ProcessEvent. pi8 saw ProcessEvent no-op
// when re-entrant, but that was a BP func — for a NATIVE func ProcessEvent dispatches to the native
// thunk directly, which may work.) SAFE: only calls GetMissions() (read-only, returns the map keys,
// zero side effects). Logs the returned TArray.Num. NO writes to game state.
//
// Mechanism: hook ProcessInternal (base+0x13454A0) to get on the game thread at a safe point, then
// from the hook call ProcessEvent(base+0x12C5A10)(MissionsModel, GetMissions_UFunc, &resultTArray).
...
// Build:  clang++ -shared -O2 missions_call_probe.cpp -o missions_call_probe.dll -lkernel32
// Inject: tools/inject/inject.exe mmap <PID> tools/sigbypass-mod/missions_call_probe.dll
// Marker: docs/missions-call-probe-marker.txt   (ONE ProcessInternal-hooking shim per client!)
#include <windows.h>
#include <tlhelp32.h>
#include <cstdint>
#include <cstdio>
...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-call-probe-marker.txt";
...
static uintptr_t g_mm=0;                 // MissionsModel instance
static void* g_getMissions=nullptr;      // GetMissions UFunction
...
static uint64_t g_result[4]={0};         // TArray<FString> result: {Data, Num, Max} in first 16 bytes
...
// minimal VEH so a crash lands in the marker, not a silent client death
...
extern "C" void OnBP(void* /*ctx*/, void* /*frame*/, void*){
    ...
    // Call the native GetMissions() via ProcessEvent. result buffer = TArray header (16 bytes) zeroed.
    memset(g_result,0,sizeof(g_result));
    if(g_processEvent && g_mm && g_getMissions){ g_processEvent((void*)g_mm, g_getMissions, g_result); }
    g_pending=0; g_done=1; g_inHook=0;
}

// ── hook plumbing (from pi8) ──
...
static void Resolve(){
    ...
    if(!LooksLikePtr(objectsPtr)||numEl<=0||numEl>8000000)return; int numChunks=(numEl+PERCHUNK-1)/PERCHUNK; uintptr_t progMgr=0, mmClass=0;
    ...
    if(!progMgr)return; if(SafeReadable((void*)(progMgr+PM_MM),8)){ uintptr_t mm=*(uintptr_t*)(progMgr+PM_MM); if(LooksLikePtr(mm)){ g_mm=mm; mmClass=ClassOf(mm); } }
    if(!mmClass)return;
    // GetMissions UFunction: walk MissionsModel UClass.Children (UField*) @+0x50 via Next@+0x30
    uintptr_t f=0; if(SafeReadable((void*)(mmClass+0x50),8)) f=*(uintptr_t*)(mmClass+0x50); int i=0;
    while(LooksLikePtr(f)&&i<40){ if(NameIs(f,"GetMissions")){ g_getMissions=(void*)f; break; } uintptr_t nx=0; if(SafeReadable((void*)(f+0x30),8))nx=*(uintptr_t*)(f+0x30); f=nx; i++; }
}
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_call_probe (call GetMissions via ProcessEvent on game thread) started\r\n");
    ...
    g_gameTid=WaitTid(120000); if(!g_gameTid){Marker("[1] FAIL gameTid\r\n");return 2;} Markerf("[1] gameTid=%lu PE=0x%llX\r\n",g_gameTid,(unsigned long long)g_processEvent);
    DWORD dl=GetTickCount()+120000; while(GetTickCount()<dl){ Resolve(); if(g_mm&&g_getMissions)break; Sleep(500);}
    if(!g_mm||!g_getMissions){Markerf("[2] FAIL resolve mm=0x%llX getMissions=%p\r\n",(unsigned long long)g_mm,g_getMissions);return 3;}
    Markerf("[2] MissionsModel=0x%llX GetMissions=%p\r\n",(unsigned long long)g_mm,g_getMissions);
    ...
    Marker("[3] hook built; arming one GetMissions call...\r\n");
    g_pending=1; g_done=0; if(!InstallHook()){Marker("[3] FAIL InstallHook\r\n");return 6;}
    DWORD t0=GetTickCount();
    while(!g_done && GetTickCount()-t0<8000) Sleep(20);
    UninstallHook();
    if(g_done){ uint64_t data=g_result[0]; uint32_t num=(uint32_t)(g_result[1]&0xFFFFFFFF); uint32_t mx=(uint32_t)(g_result[1]>>32);
        Markerf("[4] CALLED ok. GetMissions -> TArray{Data=0x%llX Num=%u Max=%u} (hitsGT=%ld)\r\n",(unsigned long long)data,num,mx,(long)g_hitsGT);
        Marker("[4] => ProcessEvent CAN call native MissionsModel methods on the game thread. Phase-2 primitive UNLOCKED.\r\n");
