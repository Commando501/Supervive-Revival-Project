// missions_nativecall_probe — SESSION 54 (Option 1): DEFINITIVELY test whether ProcessEvent,
// called from the ProcessInternal (pi8) game-thread hook, EXECUTES a NATIVE UFunction.
//
// Session 52's tests were ambiguous: GetMissions()->Num=0 is what BOTH a real empty-map read AND a
// no-op leave behind; PrimaryAssetIDFromString->{0,0} could be a wrong param buffer. This probe removes
// the ambiguity: call ProgressionManager.GetMissionsModel() (NATIVE, no params, read-only) and compare
// the returned pointer against the MissionsModel we read INDEPENDENTLY via RPM (ProgMgr+0x3B8).
//   result == known MissionsModel  => ProcessEvent EXECUTES native funcs. PRIMITIVE WORKS.
//   result == 0                    => no-op (or returned null). Primitive does NOT work as-is.
//   result == other non-zero       => it ran but returned something unexpected (still = it executes).
// SAFE: read-only getter, zero writes to game state.
//
// Reuses the pi8 hook plumbing (ProcessInternal @base+0x13454A0, ProcessEvent @base+0x12C5A10).
// UObject: Class@+0x18 Name@+0x20 Outer@+0x28. ProgMgr.MissionsModel@+0x3B8. UClass.Children@+0x50,
// UField.Next@+0x30. GUObjectArray@base+0x9E38930, FNamePool@base+0x9D81450, GGameThreadId@+0x9D49158.
// Build:  clang++ -shared -O2 missions_nativecall_probe.cpp -o missions_nativecall_probe.dll -lkernel32
// Inject: tools/inject/inject.exe mmap <PID> tools/sigbypass-mod/missions_nativecall_probe.dll
// Marker: docs/missions-nativecall-marker.txt   (ONE ProcessInternal-hooking shim per client!)
#include <windows.h>
#include <tlhelp32.h>
#include <cstdint>
#include <cstdio>
...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall-marker.txt";
...
static uintptr_t g_progMgr=0;            // ProgressionManager instance
static uintptr_t g_knownMM=0;            // MissionsModel read via RPM (ProgMgr+0x3B8)
static void* g_getMissionsModel=nullptr; // GetMissionsModel UFunction (on ProgressionManager)
...
static uint64_t g_result[4]={0};         // return buffer (result[0] = returned UMissionsModel*)
...
static LONG CALLBACK CrashVEH(EXCEPTION_POINTERS* ep){
    ...
    uint64_t rip=ep->ContextRecord->Rip; Markerf("[VEH] fatal code=0x%lX RIP=0x%llX rva=0x%llX inHook=%ld\r\n",code,(unsigned long long)rip,(unsigned long long)(rip>g_modBase&&rip<g_modBase+0xC000000?rip-g_modBase:0),(long)g_inHook);
    return EXCEPTION_CONTINUE_SEARCH;
}

extern "C" void OnBP(void* /*ctx*/, void* /*frame*/, void*){
...
// ── hook plumbing (verbatim from pi8/missions_call_probe) ──
...
static void Resolve(){
    ...
    // GetMissionsModel UFunction on ProgressionManager UClass.Children (UField*) @+0x50 via Next@+0x30
    ...
    while(LooksLikePtr(f)&&i<60){ if(NameIs(f,"GetMissionsModel")){ g_getMissionsModel=(void*)f; break; } uintptr_t nx=0; if(SafeReadable((void*)(f+0x30),8))nx=*(uintptr_t*)(f+0x30); f=nx; i++; }
}
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe (GetMissionsModel via ProcessEvent, unambiguous) started\r\n");
    ...
    g_gameTid=WaitTid(120000); if(!g_gameTid){Marker("[1] FAIL gameTid\r\n");return 2;} Markerf("[1] gameTid=%lu PE=0x%llX base=0x%llX\r\n",g_gameTid,(unsigned long long)g_processEvent,(unsigned long long)g_modBase);
    DWORD dl=GetTickCount()+120000; while(GetTickCount()<dl){ Resolve(); if(g_progMgr&&g_getMissionsModel&&g_knownMM)break; Sleep(500);}
    if(!g_progMgr||!g_getMissionsModel){Markerf("[2] FAIL resolve progMgr=0x%llX getMM=%p knownMM=0x%llX\r\n",(unsigned long long)g_progMgr,g_getMissionsModel,(unsigned long long)g_knownMM);return 3;}
    Markerf("[2] ProgMgr=0x%llX GetMissionsModel=%p knownMM(RPM ProgMgr+0x3B8)=0x%llX\r\n",(unsigned long long)g_progMgr,g_getMissionsModel,(unsigned long long)g_knownMM);
    ...
    Marker("[3] hook built; arming one GetMissionsModel call...\r\n");
    g_pending=1; g_done=0; if(!InstallHook()){Marker("[3] FAIL InstallHook\r\n");return 6;}
    DWORD t0=GetTickCount();
    while(!g_done && GetTickCount()-t0<8000) Sleep(20);
    UninstallHook();
    if(g_done){ uint64_t ret=g_result[0];
        Markerf("[4] CALLED. GetMissionsModel returned 0x%llX ; knownMM=0x%llX (hitsGT=%ld)\r\n",(unsigned long long)ret,(unsigned long long)g_knownMM,(long)g_hitsGT);
        if(ret==g_knownMM && ret!=0) Marker("[4] *** MATCH => ProcessEvent EXECUTES NATIVE UFunctions on the game thread. PRIMITIVE WORKS. ***\r\n");
        else if(ret==0) Marker("[4] => returned NULL: either a no-op OR GetMissionsModel returned null. INCONCLUSIVE-lean-noop.\r\n");
        else Marker("[4] => returned a DIFFERENT non-zero ptr: the call RAN (executes native) but returned unexpected value.\r\n");
    } else Markerf("[4] TIMEOUT no game-thread call in 8s (hitsGT=%ld)\r\n",(long)g_hitsGT);
    return 0;
}
