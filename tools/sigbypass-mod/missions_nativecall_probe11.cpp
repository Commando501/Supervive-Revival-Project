// missions_nativecall_probe11 — SESSION 58: FIX OUT-param calls, then run the registered-vs-unloaded diagnostic.
// ROOT CAUSE of the probe6/8/10 crashes (found by disasm of StepExplicitProperty @rva 0x1345FE0): for an OUT
// param, StepExplicitProperty reads FFrame.OutParms @ +0x80 and walks a FOutParmRec list {Property@0, PropAddr@8,
// NextOutParm@16} matching by Property ptr — it does NOT use Locals+Offset. My FFrame's +0x80 was garbage
// (from the captured template) => walk off => __fastfail. FIX: build a FOutParmRec list for each CPF_OutParm
// (non-return) param (PropAddr = Locals+Offset) and set FFrame+0x80 = &head.
// Then: PrimaryAssetIDFromString to get the "Hero"/"Mission" type FNames live (per-process), then
// GetPrimaryAssetIdList(type) [OUT TArray] for each. Hero Num>0 = control. Mission Num>0 => registered (just
// unloaded, fix=AsyncLoadPrimaryAssets); Mission Num==0 => not registered (need raw scan).
// Build:  clang++ -shared -O2 missions_nativecall_probe11.cpp -o missions_nativecall_probe11.dll -lkernel32
// Inject: tools/inject/inject.exe mmap <PID> ...  Marker: docs/missions-nativecall11-marker.txt
#include <windows.h>
#include <tlhelp32.h>
#include <cstdint>
#include <cstdio>
...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall11-marker.txt";
...
static uintptr_t g_lam=0;
static void* g_pafs=nullptr; static uintptr_t g_pafsThunk=0, g_pafsChild=0;      // PrimaryAssetIDFromString (LAM)
static void* g_gpail=nullptr; static uintptr_t g_gpailThunk=0, g_gpailChild=0;    // GetPrimaryAssetIdList (KSL)
...
static uint32_t g_heroType=0, g_missionType=0; static int32_t g_heroNum=-1, g_missionNum=-1;
...
// Build the FOutParmRec OutParms list for CPF_OutParm (non-return) params of `childProps`, PropAddr=locals+off.
static void BuildOutParms(uintptr_t childProps, uint8_t* locals){
    *(uint64_t*)(g_myframe+FF_OUTPARMS)=0;
    ...
    while(LooksLikePtr(f) && n<8){
        ...
        if((flags&CPF_OutParm) && !(flags&CPF_ReturnParm)){
            ...
            uint8_t* rec=g_outparms+n*24;
            *(uintptr_t*)(rec+0)=f;                          // FOutParmRec.Property
            *(uintptr_t*)(rec+8)=(uintptr_t)(locals+off);    // FOutParmRec.PropAddr
            *(uintptr_t*)(rec+16)=0;                         // FOutParmRec.NextOutParm
...
static void CallNative(void* func, uintptr_t thunk, uintptr_t childProps, void* context, void* paramsBuf, void* resultBuf){
    memset(g_outparms,0,sizeof(g_outparms));
    ...
    *(uint64_t*)(g_myframe+FF_PROPCHAIN)=(uint64_t)childProps;
    BuildOutParms(childProps,(uint8_t*)paramsBuf);          // <-- the fix
...
static uint32_t TypeFromPAFS(const wchar_t* idstr){
    ...
    CallNative(g_pafs,g_pafsThunk,g_pafsChild,(void*)g_lam,g_pbuf,g_rbuf);
    return (uint32_t)(g_rbuf[0]&0xFFFFFFFF);   // FPrimaryAssetId.Type FName index (result[0] low32)
}
static int32_t QueryIdList(uint32_t typeNameId){
    memset(g_pbuf,0,sizeof(g_pbuf)); memset(g_rbuf,0,sizeof(g_rbuf));
    ((uint32_t*)g_pbuf)[0]=typeNameId; ((uint32_t*)g_pbuf)[1]=0;    // FPrimaryAssetType(FName) @ Locals+0
    CallNative(g_gpail,g_gpailThunk,g_gpailChild,(void*)g_lam,g_pbuf,g_rbuf);
...
extern "C" void OnPI(void* /*ctx*/, void* frame, void*){
    ...
    g_heroType=TypeFromPAFS(L"Hero:Alchemist");
    g_missionType=TypeFromPAFS(L"Mission:ArmoryDaily_PlayAGame");
    g_heroNum=QueryIdList(g_heroType);
    g_missionNum=QueryIdList(g_missionType);
    g_done=1; g_inHook=0;
}
...
static void Resolve(){
    ...
    if(!ksl||!lam)return; g_lam=lam;
    uintptr_t lamCls=ClassOf(lam); if(lamCls) ResolveFuncOnClass(lamCls,"PrimaryAssetIDFromString",&g_pafs,&g_pafsThunk,&g_pafsChild);
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe11 (OUT-param FIX via FFrame.OutParms@+0x80 + GetPrimaryAssetIdList Hero/Mission) started\r\n");
    ...
    DWORD dl=GetTickCount()+120000; while(GetTickCount()<dl){ Resolve(); if(g_lam&&g_pafsThunk&&g_pafsChild&&g_gpailThunk&&g_gpailChild)break; Sleep(500);}
    if(!g_lam||!g_pafsThunk||!g_gpailThunk){Markerf("[2] FAIL resolve lam=0x%llX pafs=0x%llX gpail=0x%llX\r\n",(unsigned long long)g_lam,(unsigned long long)g_pafsThunk,(unsigned long long)g_gpailThunk);return 3;}
    Markerf("[2] LAM=0x%llX pafsThunk=0x%llX gpailThunk=0x%llX gameTid=%lu\r\n",(unsigned long long)g_lam,(unsigned long long)g_pafsThunk,(unsigned long long)g_gpailThunk,g_gameTid);
    ...
    Marker("[3] hook built; PAFS(Hero/Mission) + GetPrimaryAssetIdList WITH OutParms...\r\n");
    if(!InstallHook()){Marker("[3] FAIL InstallHook\r\n");return 6;}
    DWORD t0=GetTickCount(); while(!g_done && GetTickCount()-t0<8000) Sleep(20);
    UninstallHook();
    if(g_done){
        char ht[64]="?",mt[64]="?"; GetFNameStr(g_heroType,ht,sizeof(ht)); GetFNameStr(g_missionType,mt,sizeof(mt));
        Markerf("[4] types: Hero='%s'(0x%X) Mission='%s'(0x%X). GetPrimaryAssetIdList Num: Hero=%d Mission=%d\r\n",ht,g_heroType,mt,g_missionType,g_heroNum,g_missionNum);
        if(g_heroNum<=0) Marker("[4] => OUT-param call still failing (Hero Num<=0). Inspect OutParms/offsets.\r\n");
        else if(g_missionNum>0) Marker("[4] *** OUT-PARAM FIX WORKS + Mission IDs ARE registered => just NOT LOADED => AsyncLoadPrimaryAssets. ***\r\n");
        else Marker("[4] *** OUT-PARAM FIX WORKS (Hero>0) + Mission NOT registered (Num=0) => need raw ScanPathsForPrimaryAssets. ***\r\n");
