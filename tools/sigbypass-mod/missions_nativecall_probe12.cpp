...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall12-marker.txt";
...
static uintptr_t g_lam=0, g_mm=0;                 // LokiAssetManager singleton, live MissionsModel (knownMM)
static Fn g_pafs{}, g_gpail{}, g_factory{}, g_getMissions{};
...
static uint8_t g_element[0x60]={0};
static uint64_t g_missionId[2]={0}, g_poolId[2]={0}, g_retModel=0; static int32_t g_missNum=-1, g_poolNum=-1, g_modelNum=-1;
static char g_modelCls[96]={0};
...
// gpail(type) -> reads OUT array; returns Num and copies Data[0] (16B FPrimaryAssetId) into out2[2].
static int32_t QueryFirstId(uint32_t typeNameId, uint64_t out2[2]){
    ...
    out2[0]=0; out2[1]=0;
    if(num>0 && LooksLikePtr((uintptr_t)data) && SafeReadable((void*)data,16)){ out2[0]=*(uint64_t*)data; out2[1]=*(uint64_t*)(data+8); }
    return num;
}
extern "C" void OnPI(void* /*ctx*/, void* frame, void*){
    ...
    uint32_t missionType=TypeFromPAFS(L"Mission:x");
    uint32_t poolType=TypeFromPAFS(L"MissionPool:x");
    g_missNum=QueryFirstId(missionType,g_missionId);
    g_poolNum=QueryFirstId(poolType,g_poolId);
    // Build 1 FMissionProgress with the real registered ids.
    memset(g_element,0,sizeof(g_element));
    *(uint64_t*)(g_element+0x10)=g_missionId[0]; *(uint64_t*)(g_element+0x18)=g_missionId[1];   // AssetId
    *(uint64_t*)(g_element+0x20)=g_poolId[0];    *(uint64_t*)(g_element+0x28)=g_poolId[1];       // PoolId
    *(int64_t*)(g_element+0x48)=86400000LL;               // MillisUntilExpiry
    *(int64_t*)(g_element+0x50)=638000000000000000LL;      // Expiry
    *(int64_t*)(g_element+0x58)=638000000000000000LL;      // GrantedAt
    memset(g_pbuf,0,sizeof(g_pbuf)); memset(g_rbuf,0,sizeof(g_rbuf));
    ((uint64_t*)g_pbuf)[0]=(uint64_t)g_element; ((uint32_t*)g_pbuf)[2]=1; ((uint32_t*)g_pbuf)[3]=1;  // TArray{&elem,1,1} @ Locals+0
...
static void Resolve(){
    ...
    if(!ksl||!lam||!pm)return; g_lam=lam;
    ...
    if(lamCls) ResolveFn(lamCls,"PrimaryAssetIDFromString",&g_pafs);
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe12 (factory re-test w/ OUT-param fix + real registered mission id) started\r\n");
    ...
    DWORD dl=GetTickCount()+120000; while(GetTickCount()<dl){ Resolve(); if(g_lam&&g_mm&&g_pafs.thunk&&g_gpail.thunk&&g_factory.thunk&&g_getMissions.thunk)break; Sleep(500);}
    if(!g_lam||!g_mm||!g_pafs.thunk||!g_gpail.thunk||!g_factory.thunk||!g_getMissions.thunk){Markerf("[2] FAIL resolve lam=%llX mm=%llX pafs=%llX gpail=%llX fac=%llX gm=%llX\r\n",(unsigned long long)g_lam,(unsigned long long)g_mm,(unsigned long long)g_pafs.thunk,(unsigned long long)g_gpail.thunk,(unsigned long long)g_factory.thunk,(unsigned long long)g_getMissions.thunk);return 3;}
    ...
    Marker("[3] hook built; get real mission/pool ids -> factory -> GetMissions...\r\n");
    if(!InstallHook()){Marker("[3] FAIL InstallHook\r\n");return 6;}
    DWORD t0=GetTickCount(); while(!g_done && GetTickCount()-t0<8000) Sleep(20);
    UninstallHook();
    if(g_done){
        Markerf("[4] missionIds=%d (id0={0x%llX,0x%llX}) poolIds=%d (id0={0x%llX,0x%llX})\r\n",g_missNum,(unsigned long long)g_missionId[0],(unsigned long long)g_missionId[1],g_poolNum,(unsigned long long)g_poolId[0],(unsigned long long)g_poolId[1]);
        Markerf("[4] factory ret model=0x%llX class='%s' ; model.GetMissions().Num=%d\r\n",(unsigned long long)g_retModel,g_modelCls,g_modelNum);
        if(LooksLikePtr(g_retModel)&&g_modelNum>0) Marker("[4] *** FACTORY BUILDS A NON-EMPTY MODEL from synthetic FMissionProgress! => wire to knownMM + broadcast + render. ***\r\n");
        else if(LooksLikePtr(g_retModel)) Marker("[4] => factory returned a model but GetMissions().Num<=0 (empty; may need the DA LOADED, or it ignores input). \r\n");
        else Marker("[4] => factory returned NULL (ignores synthetic input / needs real final progress).\r\n");
