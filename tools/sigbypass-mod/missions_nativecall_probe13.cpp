...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall13-marker.txt";
// probe13: populate a MissionsModel with N real missions across real pools, SWAP it into ProgMgr.MissionsModel,
// then the modal is opened externally (computer-use) so the widgets read the populated model on Construct.
...
static Fn g_pafs{}, g_gpail{}, g_factory{}, g_getMissions{}, g_glda{};
...
extern "C" void OnPI(void* /*ctx*/, void* frame, void*){
    ...
    // Build NMISS FMissionProgress: mission[i] tagged with pool[i] (spread across pools -> land in a visible tab).
    ...
    for(int i=0;i<NMISS;i++){
        ...
        *(int64_t*)(el+0x48)=86400000LL; *(int64_t*)(el+0x50)=638000000000000000LL; *(int64_t*)(el+0x58)=638000000000000000LL;
        GetFNameStr((uint32_t)(g_poolIds[i][1]&0xFFFFFFFF), g_poolNames[i], 48);              // pool Name (for logging)
    }
    memset(g_pbuf,0,sizeof(g_pbuf)); memset(g_rbuf,0,sizeof(g_rbuf));
    ((uint64_t*)g_pbuf)[0]=(uint64_t)g_elems; ((uint32_t*)g_pbuf)[2]=NMISS; ((uint32_t*)g_pbuf)[3]=NMISS;  // TArray{&elems,N,N}
    ...
    if(LooksLikePtr(g_retModel)){
        ...
        // Is mission[0]'s DA LOADED? (registered != loaded; needed for a non-blank tile.)
        memset(g_pbuf,0,sizeof(g_pbuf)); memset(g_rbuf,0,sizeof(g_rbuf));
        g_pbuf[0]=g_missionIds[0][0]; g_pbuf[1]=g_missionIds[0][1];
        Call(g_glda,(void*)g_lam,g_pbuf,g_rbuf); g_glLoaded=g_rbuf[0];
        if(LooksLikePtr(g_glLoaded)) ClassNameOf(g_glLoaded,g_glCls,sizeof(g_glCls));
        // *** SWAP: ProgressionManager.MissionsModel = the new populated model (roots it via the UPROPERTY). ***
...
static void Resolve(){
    ...
    if(lamCls){ ResolveFn(lamCls,"PrimaryAssetIDFromString",&g_pafs); ResolveFn(lamCls,"GetLokiDataAsset",&g_glda); }
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe13 (populate model w/ N real missions + SWAP into ProgMgr.MissionsModel) started\r\n");
    ...
    DWORD dl=GetTickCount()+120000; while(GetTickCount()<dl){ Resolve(); if(g_lam&&g_mm&&g_pm&&g_pafs.thunk&&g_gpail.thunk&&g_factory.thunk&&g_getMissions.thunk&&g_glda.thunk)break; Sleep(500);}
    if(!g_lam||!g_mm||!g_pm||!g_pafs.thunk||!g_gpail.thunk||!g_factory.thunk||!g_getMissions.thunk||!g_glda.thunk){Markerf("[2] FAIL resolve lam=%llX mm=%llX pm=%llX pafs=%llX gpail=%llX fac=%llX gm=%llX glda=%llX\r\n",(unsigned long long)g_lam,(unsigned long long)g_mm,(unsigned long long)g_pm,(unsigned long long)g_pafs.thunk,(unsigned long long)g_gpail.thunk,(unsigned long long)g_factory.thunk,(unsigned long long)g_getMissions.thunk,(unsigned long long)g_glda.thunk);return 3;}
    ...
    Marker("[3] hook built; get real ids -> build N-mission model -> swap into ProgMgr.MissionsModel...\r\n");
