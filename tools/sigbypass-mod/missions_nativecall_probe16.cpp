...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall16-marker.txt";
...
static uint8_t g_loadAssets[NMAX*16]={0};         // TArray data for AsyncLoadPrimaryAssets (all mission ids)
static wchar_t g_idbufs[NMAX][8]={{0}};           // distinct FMissionProgress.ID "m<i>"
...
static uint8_t g_objBuf[0x38]={0};   // one FMissionObjectiveProgress: ObjectiveName@0, Progress@0x08(f), MaxProgress@0x0C(f)
...
// Enumerate ALL missions (id+name) + ALL pools (id+name); map each mission to a pool by NAME PREFIX.
static void EnumerateAndMap(){
    ...
    // map: mission name prefix -> pool (by substring). Non-hero missions land in the modal's tabs; heroes -> HunterMissions.
    for(int i=0;i<g_missNum;i++){
        ...
        else pj=FindPoolSub("Hunter");   // hero/class missions
        if(pj<0) pj=0;
...
static void FireLoad(){
    ...
    *(uint32_t*)((uint8_t*)g_pbuf+16)=(uint32_t)g_missNum;       // Assets.Num
    *(uint32_t*)((uint8_t*)g_pbuf+20)=(uint32_t)g_missNum;       // Assets.Max
...
static void BuildAndSwap(){
    ...
    // TEST objective-progress entry: Progress=0, MaxProgress=42 (distinctive) to confirm the bar reads our data.
    memset(g_objBuf,0,sizeof(g_objBuf)); { float p=0.0f, mx=42.0f; memcpy(g_objBuf+0x08,&p,4); memcpy(g_objBuf+0x0C,&mx,4); }
    for(int i=0;i<g_missNum;i++){
        ...
        *(uint64_t*)(el+0x20)=g_missPool[i][0];   *(uint64_t*)(el+0x28)=g_missPool[i][1];     // PoolId (prefix-mapped)
        *(uint64_t*)(el+0x38)=(uint64_t)g_objBuf; *(uint32_t*)(el+0x40)=1; *(uint32_t*)(el+0x44)=1;  // ObjectiveProgress {Data,Num,Max}
...
extern "C" void OnPI(void* /*ctx*/, void* frame, void*){
    ...
    if(g_state==0){
        EnumerateAndMap();                           // all missions+pools, name-prefix pool mapping
        FireLoad();                                  // async-load ALL mission DAs
        g_t0=GetTickCount(); g_state=1;
    } else if(g_state==1){
        uint64_t da=GLDA(g_missionIds[0][0],g_missionIds[0][1]);
        g_loadedCheck = LooksLikePtr((uintptr_t)da)?1:0;
        if(g_loadedCheck || GetTickCount()-g_t0>20000){   // loaded, or give up after 20s (330 DAs)
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe16 (all missions + TEST ObjectiveProgress 0/42) started\r\n");
    ...
    UninstallHook();
    if(g_done){
        Markerf("[4] missions registered=%d, pools=%d. Built %d-mission model=0x%llX GetMissions().Num=%d\r\n",g_missNum,g_poolNum,g_nBuilt,(unsigned long long)g_retModel,g_modelNum);
        char pl[512]={0}; for(int j=0;j<g_poolNum;j++){ char one[64]; _snprintf_s(one,sizeof(one),_TRUNCATE,"%s%s",j?",":"",g_poolNames[j]); strncat_s(pl,sizeof(pl),one,_TRUNCATE);}
        Markerf("[4] pools(%d): %s\r\n",g_poolNum,pl);
        // per-pool mission counts
        for(int j=0;j<g_poolNum;j++){ int c=0; for(int i=0;i<g_missNum;i++) if(g_missPool[i][1]==g_poolIds[j][1]) c++; if(c>0) Markerf("[4]   pool '%s': %d missions\r\n",g_poolNames[j],c); }
        Markerf("[4] mission[0]='%s' DA loaded? GetLokiDataAsset=0x%llX class='%s'\r\n",g_missNames[0],(unsigned long long)g_glLoaded,g_glCls);
        Markerf("[4] SWAP ProgMgr.MissionsModel -> new model: %s\r\n",g_swapped?"DONE":"SKIPPED");
        if(g_swapped) Marker("[4] *** all missions populated + swapped => reopen the Missions modal (computer-use). ***\r\n");
        else Marker("[4] => not swapped (model empty/null).\r\n");
    } else Markerf("[4] TIMEOUT (hitsGT=%ld state=%ld)\r\n",(long)g_hitsGT,(long)g_state);
    return 0;
}
