...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall17-marker.txt";
...
static int BuildObjectivesForMission(int i){
    ...
    for(int j=0;j<onum;j++){
        ...
        *(float*)(op+0x08)=0.0f;        // Progress (not started)
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe17 (Option 1: bars 0/real-max via GetUniqueObjectiveName-keyed model) started\r\n");
    ...
    Marker("[3] hook built; load DAs -> per-objective GetUniqueObjectiveName keys (Progress=0) -> factory -> swap...\r\n");
    ...
    if(g_done){
        ...
        Marker("[4] --- per-objective keys (first 8 missions): key must be a REAL objective name, NOT 'None' ---\r\n");
        for(int i=0;i<8 && i<g_missNum;i++) Markerf("[4]   m%d '%s': objN=%d key0='%s' total0=%.1f\r\n",i,g_vMiss[i],g_vObjN[i],g_vKeyName[i],g_vTotal[i]);
        Markerf("[4] SWAP ProgMgr.MissionsModel -> new model: %s\r\n",g_swapped?"DONE":"SKIPPED");
        if(g_swapped) Marker("[4] *** keyed model swapped. CLOSE+REOPEN the Missions modal -> bars should read 0/<real max>. ***\r\n");
