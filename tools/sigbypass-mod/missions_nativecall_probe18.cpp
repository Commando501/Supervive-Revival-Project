// missions_nativecall_probe18 — SESSION 59, OPTION 2 (read/apply loop): fetch per-account mission
// progress from the ags backend (GET http://127.0.0.1:8080/revival/missions/progress -> JSON
// {"objectives":{"<uniqueName>":<progress>,...}}) and apply each value into the mission model, so the
// bars show REAL progress (e.g. 17/50) instead of just 0/max. Identical to probe17 except:
//   FMissionProgress.ObjectiveProgress[i].Progress = objectives[ GetUniqueObjectiveName(objective) ]  (else 0)
// The bar then reads ObjectiveModel.CurrentProgress = that value; max is still the DA's TotalProgress.
// Seed the backend first, e.g.:  curl -s -X POST http://127.0.0.1:8080/revival/missions/progress \
//   -d '{"objectives":{"BR_Knocks_Assists":17,"PlayAGame":3}}'
// (Original probe17 header follows.)
...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall18-marker.txt";
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe18 (Option 2: bars reflect ags-fetched progress) started\r\n");
    ...
    FetchProgress();
    Markerf("[3] fetched ags progress: ok=%d entries=%d (%s)\r\n",g_fetchOk,g_progNum,kProgressUrl);
    Marker("[3] hook built; load DAs -> keyed model w/ FETCHED Progress -> factory -> swap...\r\n");
    ...
    if(g_done){
        ...
        Markerf("[4] ags progress: fetchOk=%d entries=%d appliedNonZero=%d\r\n",g_fetchOk,g_progNum,g_appliedNonZero);
        Marker("[4] --- first 8 missions: key0='<objectiveName>' prog0/total0 (prog from ags) ---\r\n");
        for(int i=0;i<8 && i<g_missNum;i++) Markerf("[4]   m%d '%s': objN=%d key0='%s' prog0=%.1f total0=%.1f\r\n",i,g_vMiss[i],g_vObjN[i],g_vKeyName[i],g_vProg[i],g_vTotal[i]);
        Markerf("[4] SWAP ProgMgr.MissionsModel -> new model: %s\r\n",g_swapped?"DONE":"SKIPPED");
        if(g_swapped) Marker("[4] *** keyed model swapped w/ fetched progress. CLOSE+REOPEN the Missions modal -> bars reflect ags progress. ***\r\n");
