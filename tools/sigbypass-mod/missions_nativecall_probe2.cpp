// missions_nativecall_probe2 — SESSION 54 (Option 1): DEFINITIVELY answer "does ProcessEvent execute a
// NATIVE UFunction?" by RUN-THROUGH hooking the target's native thunk and counting entries ONLY during
// our ProcessEvent call. Unambiguous: thunkHits>0 => the native function body ran => PRIMITIVE WORKS.
//
// Two hooks: (1) ProcessInternal @base+0x13454A0 (pi8 plumbing) to get on the game thread + call
// ProcessEvent(progMgr, GetMissionsModel, result). (2) GetMissionsModel's native thunk (UFunction+0xE0),
// a run-through detour that increments g_thunkHits IFF g_inHook (i.e. during our call), then continues.
// SAFE: read-only getter; the thunk detour preserves original behavior.
// GetMissionsModel thunk prologue = 48 8B 42 20 | 45 33 C9 (2 instrs = 7 bytes to steal).
// Build:  clang++ -shared -O2 missions_nativecall_probe2.cpp -o missions_nativecall_probe2.dll -lkernel32
// Inject: tools/inject/inject.exe mmap <PID> ...  Marker: docs/missions-nativecall2-marker.txt
#include <windows.h>
#include <tlhelp32.h>
#include <cstdint>
#include <cstdio>
...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall2-marker.txt";
...
static uint8_t* g_pi=nullptr; static uint8_t g_stolenPi[5]={0};
static uint8_t g_stolenThunk[7]={0}; static int g_thunkSteal=7;
static volatile PFN_PE g_piTramp=nullptr; static void* g_thunkTramp=nullptr;
static volatile long g_pending=0,g_inHook=0,g_done=0,g_hitsGT=0,g_thunkHits=0; static DWORD g_gameTid=0;
static uint64_t g_result[4]={0};
...
// PI hook callback: on the game thread, call ProcessEvent(progMgr, GetMissionsModel, result).
extern "C" void OnPI(void*, void*, void*){
    if(!g_pending || g_inHook) return;
    if(GetCurrentThreadId()!=g_gameTid) return;
    InterlockedIncrement(&g_hitsGT); g_inHook=1;
    memset(g_result,0,sizeof(g_result));
    if(g_processEvent && g_progMgr && g_getMissionsModel){ g_processEvent((void*)g_progMgr, g_getMissionsModel, g_result); }
    g_pending=0; g_done=1; g_inHook=0;
}
// Thunk run-through callback: count entries only during our PE call.
extern "C" void OnThunk(void*, void*, void*){ if(g_inHook) InterlockedIncrement(&g_thunkHits); }

// ── generalized hook builder (variable steal length) ──
...
// Returns the detour STUB (to jump to from fn); sets *tramp to the trampoline (stolen+continue).
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe2 (thunk run-through hook — definitive) started\r\n");
    ...
    Markerf("[2] ProgMgr=0x%llX GetMissionsModel=%p thunk=0x%llX knownMM=0x%llX\r\n",(unsigned long long)g_progMgr,g_getMissionsModel,(unsigned long long)g_thunk,(unsigned long long)g_knownMM);
    // Build thunk run-through hook (steal 7 bytes = 48 8B 42 20 | 45 33 C9)
    ...
    memcpy(g_stolenPi,g_pi,5); void* piTr=nullptr; uint8_t* piStub=BuildHookN((uintptr_t)g_pi,g_stolenPi,5,OnPI,&piTr); g_piTramp=(PFN_PE)piTr;
    if(!piStub||!g_piTramp){Marker("[2] FAIL PI hook build\r\n");return 7;}
    // Install thunk hook first, then PI hook + arm.
    if(!InstallJmp((uint8_t*)g_thunk,thunkStub,7)){Marker("[3] FAIL install thunk jmp\r\n");return 8;}
    Marker("[3] thunk hook installed; arming one GetMissionsModel via ProcessEvent...\r\n");
    g_pending=1; g_done=0;
    if(!InstallJmp(g_pi,piStub,5)){Marker("[3] FAIL install PI jmp\r\n"); SafeWrite((uint8_t*)g_thunk,g_stolenThunk,7); return 9;}
    DWORD t0=GetTickCount(); while(!g_done && GetTickCount()-t0<8000) Sleep(20);
    SafeWrite(g_pi,g_stolenPi,5);                 // uninstall PI hook
    SafeWrite((uint8_t*)g_thunk,g_stolenThunk,7); // uninstall thunk hook
    if(g_done){ uint64_t ret=g_result[0];
        Markerf("[4] PE CALLED. thunkHits=%ld (during our call). GetMissionsModel ret=0x%llX knownMM=0x%llX hitsGT=%ld\r\n",(long)g_thunkHits,(unsigned long long)ret,(unsigned long long)g_knownMM,(long)g_hitsGT);
        if(g_thunkHits>0) Marker("[4] *** thunkHits>0 => ProcessEvent EXECUTES NATIVE UFunctions on the game thread. PRIMITIVE WORKS. ***\r\n");
        else Marker("[4] => thunkHits==0 => ProcessEvent did NOT enter the native thunk. ProcessEvent no-ops for native (or wrong PE addr).\r\n");
    } else Markerf("[4] TIMEOUT (hitsGT=%ld thunkHits=%ld)\r\n",(long)g_hitsGT,(long)g_thunkHits);
    return 0;
}
