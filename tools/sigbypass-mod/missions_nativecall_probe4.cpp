// missions_nativecall_probe4 — SESSION 55 (Avenue A): call a NATIVE UFunction's THUNK DIRECTLY, bypassing
// ProcessEvent entirely. This is the ultimate primitive: a direct thunk call ignores ProcessEvent's guards,
// so re-entrancy is irrelevant. Safety comes from using a REAL, live FFrame as the template (valid vtable +
// internals) rather than a synthetic one — for a no-param getter the thunk only touches Frame+0x20 (Code),
// but a captured frame makes any incidental read safe.
//
// Mechanism (pi8 template-capture, but calling the THUNK not the interpreter):
//   1. Hook ProcessInternal @base+0x13454A0 to run on the game thread with a live FFrame.
//   2. Capture that FFrame as a template; build myframe = template with Object=ProgMgr, Node=GetMissionsModel,
//      Code=null (P_FINISH does `Code += !!Code` => +0, no deref for a no-param getter), Locals=my buffer.
//   3. Call GetMissionsModel's thunk directly: ((thunk)(Context=ProgMgr, FFrame& Stack=myframe, Result=buf)).
//   result == knownMM (read independently via RPM at ProgMgr+0x3B8) => the native ran. *** PRIMITIVE WORKS ***
// SAFE-ish: read-only getter; VEH logs any fault. This is the "risky but definitive" avenue — run only if
// probe3 (Avenue B) reports thunkHits==0. ONE ProcessInternal-hooking shim per client.
// Build:  clang++ -shared -O2 missions_nativecall_probe4.cpp -o missions_nativecall_probe4.dll -lkernel32
// Inject: tools/inject/inject.exe mmap <PID> ...  Marker: docs/missions-nativecall4-marker.txt
#include <windows.h>
#include <tlhelp32.h>
#include <cstdint>
#include <cstdio>
...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall4-marker.txt";
...
typedef void (*PFN_THUNK)(void* Context, void* Frame, void* Result);   // execFunc(UObject*, FFrame&, void*)

static uintptr_t g_modBase=0;
static volatile PFN_PE g_tramp=nullptr;      // original ProcessInternal (unused for the call; kept for hook plumbing)
static uintptr_t g_progMgr=0, g_knownMM=0;
static void* g_getMissionsModel=nullptr;
static uintptr_t g_thunk=0;
static uint8_t* g_pi=nullptr; static uint8_t g_stolen[5]={0}; static uint8_t* g_stub=nullptr;
static volatile long g_inHook=0,g_done=0,g_hitsGT=0,g_matches=0; static DWORD g_gameTid=0;
static uint8_t g_template[0x180]={0}, g_myframe[0x180]={0}; static uint64_t g_locals[24]={0}, g_result[4]={0}, g_lastRet=0;
...
static LONG CALLBACK CrashVEH(EXCEPTION_POINTERS* ep){
    ...
    uint64_t rip=ep->ContextRecord->Rip; Markerf("[VEH] fatal 0x%lX RIP=0x%llX rva=0x%llX inHook=%ld thunk=0x%llX\r\n",code,(unsigned long long)rip,(unsigned long long)(rip>g_modBase&&rip<g_modBase+0xC000000?rip-g_modBase:0),(long)g_inHook,(unsigned long long)(g_thunk>g_modBase?g_thunk-g_modBase:0));
    return EXCEPTION_CONTINUE_SEARCH;
}

// Direct thunk call, on the game thread, using the captured live FFrame as template.
static void CallThunkDirect(){
    memset(g_locals,0,sizeof(g_locals)); memset(g_result,0,sizeof(g_result));
    memcpy(g_myframe, g_template, sizeof(g_myframe));
    *(void**)(g_myframe+0x10)=g_getMissionsModel;          // Node = UFunction
    *(void**)(g_myframe+0x18)=(void*)g_progMgr;             // Object = Context
    *(uint64_t*)(g_myframe+0x20)=0;                         // Code = null => P_FINISH is a no-op (no deref)
    *(void**)(g_myframe+0x28)=(void*)g_locals;              // Locals
    *(uint64_t*)(g_myframe+0x30)=0; *(uint64_t*)(g_myframe+0x38)=0;
    ((PFN_THUNK)g_thunk)((void*)g_progMgr, g_myframe, g_result);   // execGetMissionsModel(Context, Stack, Result)
...
extern "C" void OnPI(void* /*ctx*/, void* frame, void*){
    ...
    memcpy(g_template, frame, sizeof(g_template));   // capture a live, valid FFrame
    CallThunkDirect();
    g_done=1; g_inHook=0;
}
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe4 (Avenue A: DIRECT native-thunk call w/ captured live FFrame) started\r\n");
    ...
    Markerf("[2] ProgMgr=0x%llX GetMissionsModel=%p thunk=0x%llX(rva 0x%llX) knownMM=0x%llX gameTid=%lu\r\n",(unsigned long long)g_progMgr,g_getMissionsModel,(unsigned long long)g_thunk,(unsigned long long)(g_thunk-g_modBase),(unsigned long long)g_knownMM,g_gameTid);
    ...
    Marker("[3] hook built; calling GetMissionsModel THUNK directly on the game thread...\r\n");
    if(!InstallHook()){Marker("[3] FAIL InstallHook\r\n");return 6;}
    DWORD t0=GetTickCount(); while(!g_done && GetTickCount()-t0<8000) Sleep(20);
    UninstallHook();
    if(g_done){ Markerf("[4] CALLED. thunk ret=0x%llX knownMM=0x%llX matches=%ld hitsGT=%ld\r\n",(unsigned long long)g_lastRet,(unsigned long long)g_knownMM,(long)g_matches,(long)g_hitsGT);
        if(g_lastRet==g_knownMM && g_knownMM!=0) Marker("[4] *** ret==knownMM => DIRECT THUNK CALL EXECUTES NATIVE UFunctions on the game thread. AVENUE A PRIMITIVE WORKS. ***\r\n");
        else if(g_lastRet==0) Marker("[4] => ret NULL: thunk no-op OR getter returned null OR faulted (see [VEH]).\r\n");
        else Marker("[4] => ret is a DIFFERENT non-zero ptr: it RAN (executes native) but returned unexpected.\r\n");
