// missions_nativecall_probe5 — SESSION 55: extend the proven Avenue-A primitive to RETURN-BY-VALUE.
// Calls UMissionsModel::GetMissions() (native, no in-params, returns TArray<MissionModel*>) on the LIVE
// MissionsModel (ProgMgr+0x3B8) via a direct thunk call, and reads the returned TArray {Data@+0, Num@+8}.
// Proves the primitive handles TArray return-by-value (the S56 verification strategy reads GetMissions().Num
// to confirm ingestion) AND establishes the baseline mission count (expected 0 at an empty menu).
// Same mechanism as probe4 (capture live FFrame, Code=NULL, call thunk @UFunction+0xE0), Context = MissionsModel.
// Build:  clang++ -shared -O2 missions_nativecall_probe5.cpp -o missions_nativecall_probe5.dll -lkernel32
// Inject: tools/inject/inject.exe mmap <PID> ...  Marker: docs/missions-nativecall5-marker.txt
#include <windows.h>
#include <tlhelp32.h>
#include <cstdint>
#include <cstdio>
...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall5-marker.txt";
...
static void* g_getMissions=nullptr;
...
static uint8_t g_template[0x180]={0}, g_myframe[0x180]={0}; static uint64_t g_locals[24]={0}, g_result[4]={0};
static uint64_t g_arrData=0; static int32_t g_arrNum=-1, g_arrMax=-1;
...
static void CallThunkDirect(){
    ...
    *(void**)(g_myframe+0x10)=g_getMissions;               // Node = UFunction
    *(void**)(g_myframe+0x18)=(void*)g_missionsModel;       // Object = Context
    *(uint64_t*)(g_myframe+0x20)=0;                         // Code = null (no in-params)
    *(void**)(g_myframe+0x28)=(void*)g_locals;              // Locals
    *(uint64_t*)(g_myframe+0x30)=0; *(uint64_t*)(g_myframe+0x38)=0;
    ((PFN_THUNK)g_thunk)((void*)g_missionsModel, g_myframe, g_result);   // execGetMissions(Context, Stack, Result=&TArray)
    g_arrData=g_result[0];                                  // TArray.Data
    g_arrNum=(int32_t)(g_result[1]&0xFFFFFFFF);             // TArray.ArrayNum
    g_arrMax=(int32_t)((g_result[1]>>32)&0xFFFFFFFF);       // TArray.ArrayMax
}
...
static void Resolve(){
    ...
    while(LooksLikePtr(f)&&i<120){ if(NameIs(f,"GetMissions")){ g_getMissions=(void*)f; if(SafeReadable((void*)(f+UFUNC_FUNC),8)){uintptr_t th=*(uintptr_t*)(f+UFUNC_FUNC); if(LooksLikePtr(th))g_thunk=th;} break; } uintptr_t nx=0; if(SafeReadable((void*)(f+0x30),8))nx=*(uintptr_t*)(f+0x30); f=nx; i++; }
}
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe5 (Avenue A return-by-value: GetMissions() on live MissionsModel) started\r\n");
    ...
    DWORD dl=GetTickCount()+120000; while(GetTickCount()<dl){ Resolve(); if(g_missionsModel&&g_getMissions&&g_thunk)break; Sleep(500);}
    if(!g_missionsModel||!g_getMissions||!g_thunk){Markerf("[2] FAIL resolve mm=0x%llX getMissions=%p thunk=0x%llX\r\n",(unsigned long long)g_missionsModel,g_getMissions,(unsigned long long)g_thunk);return 3;}
    Markerf("[2] MissionsModel=0x%llX GetMissions=%p thunk=0x%llX(rva 0x%llX) gameTid=%lu\r\n",(unsigned long long)g_missionsModel,g_getMissions,(unsigned long long)g_thunk,(unsigned long long)(g_thunk-g_modBase),g_gameTid);
    ...
    Marker("[3] hook built; calling GetMissions() THUNK directly on the game thread...\r\n");
    if(!InstallHook()){Marker("[3] FAIL InstallHook\r\n");return 6;}
    DWORD t0=GetTickCount(); while(!g_done && GetTickCount()-t0<8000) Sleep(20);
    UninstallHook();
    if(g_done){ Markerf("[4] CALLED. GetMissions() -> TArray{ Data=0x%llX Num=%d Max=%d } hitsGT=%ld\r\n",(unsigned long long)g_arrData,g_arrNum,g_arrMax,(long)g_hitsGT);
        if(g_arrNum>=0 && g_arrMax>=g_arrNum && (g_arrNum==0 || LooksLikePtr(g_arrData))) Marker("[4] *** return-by-value WORKS: a well-formed TArray came back => Avenue A handles TArray returns. (Num is the baseline mission count.) ***\r\n");
        else Marker("[4] => TArray looks malformed; inspect (thunk may need a different frame for return-by-value).\r\n");
