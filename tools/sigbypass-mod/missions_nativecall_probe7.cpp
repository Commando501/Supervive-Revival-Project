// missions_nativecall_probe7 — SESSION 56: FIRST PARAM-PASSING test via Avenue A.
...
// Build:  clang++ -shared -O2 missions_nativecall_probe7.cpp -o missions_nativecall_probe7.dll -lkernel32
// Inject: tools/inject/inject.exe mmap <PID> ...  Marker: docs/missions-nativecall7-marker.txt
#include <windows.h>
#include <tlhelp32.h>
#include <cstdint>
#include <cstdio>
...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall7-marker.txt";
...
static wchar_t g_idstr[]=L"Daily";   // FString param value (real non-empty string exercises the deep-copy path)
...
static void CallWithParams(){
    // params frame: FString ID {Data=&g_idstr, Num=6, Max=6} at Locals+0 (param[0] ID Offset=0).
    memset(g_params,0,sizeof(g_params)); memset(g_result,0,sizeof(g_result));
    ((uint64_t*)g_params)[0]=(uint64_t)g_idstr;      // FString.Data -> L"Daily\0"
    ((uint32_t*)g_params)[2]=6;                       // FString.ArrayNum (incl null terminator)
    ((uint32_t*)g_params)[3]=6;                       // FString.ArrayMax
...
static void Resolve(){
    ...
    while(LooksLikePtr(f)&&i<200){ if(NameIs(f,"GetMissionModel")){ g_func=(void*)f; if(SafeReadable((void*)(f+UFUNC_FUNC),8)){uintptr_t th=*(uintptr_t*)(f+UFUNC_FUNC); if(LooksLikePtr(th))g_thunk=th;} if(SafeReadable((void*)(f+UFUNC_CHILDPROPS),8)){uintptr_t cp=*(uintptr_t*)(f+UFUNC_CHILDPROPS); if(LooksLikePtr(cp))g_childProps=cp;} break; } uintptr_t nx=0; if(SafeReadable((void*)(f+0x30),8))nx=*(uintptr_t*)(f+0x30); f=nx; i++; }
}
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe7 (Avenue A + PARAMS: GetMissionModel(FString) — safe param-mechanism validation) started\r\n");
    ...
    Marker("[3] hook built; calling GetMissionModel(\"Daily\") via direct thunk + compiled-in params...\r\n");
    if(!InstallHook()){Marker("[3] FAIL InstallHook\r\n");return 6;}
    DWORD t0=GetTickCount(); while(!g_done && GetTickCount()-t0<8000) Sleep(20);
    UninstallHook();
    if(g_done){ Markerf("[4] CALLED (no crash). GetMissionModel(\"Daily\") ret=0x%llX class='%s' hitsGT=%ld\r\n",(unsigned long long)g_retModel,g_retClass,(long)g_hitsGT);
        Marker("[4] *** reached here = the FString param was marshalled + the thunk ran to completion WITHOUT crashing => PARAM-PASSING MECHANISM WORKS. (ret null is expected: model is empty.) ***\r\n");
