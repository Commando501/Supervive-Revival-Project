// missions_nativecall_probe8 — SESSION 56: FIRST PARAM-PASSING test via Avenue A.
...
// Build:  clang++ -shared -O2 missions_nativecall_probe8.cpp -o missions_nativecall_probe8.dll -lkernel32
// Inject: tools/inject/inject.exe mmap <PID> ...  Marker: docs/missions-nativecall8-marker.txt
#include <windows.h>
#include <tlhelp32.h>
#include <cstdint>
#include <cstdio>
...
static const char* kMarkerPath = "G:\\git\\Supervive Revival Project\\docs\\missions-nativecall8-marker.txt";
...
// One FMissionProgress element (layout RE'd s56, size 0x60): ID@0x00(FString), AssetId@0x10(PrimaryAssetId),
// PoolId@0x20, Complete@0x30, Failed@0x31, ObjectiveProgress@0x38(TArray), MillisUntilExpiry@0x48(i64),
// Expiry@0x50(FDateTime ticks), GrantedAt@0x58. First test: asset ids None, valid numeric/date fields.
...
static void CallWithParams(){
    // Build 1 FMissionProgress element with valid numeric/date fields (asset ids left None for this first test).
    memset(g_element,0,sizeof(g_element));
    *(int64_t*)(g_element+0x48)=86400000LL;                 // MillisUntilExpiry = 24h
    *(int64_t*)(g_element+0x50)=638000000000000000LL;        // Expiry  (FDateTime ticks ~ 2023)
    *(int64_t*)(g_element+0x58)=638000000000000000LL;        // GrantedAt
    // params frame: TArray<FMissionProgress> {Data=&g_element, Num=1, Max=1} at Locals+0 (param[0].Offset=0).
    memset(g_params,0,sizeof(g_params)); memset(g_result,0,sizeof(g_result));
    ((uint64_t*)g_params)[0]=(uint64_t)g_element;            // TArray.Data
    ((uint32_t*)g_params)[2]=1;                              // TArray.Num
    ((uint32_t*)g_params)[3]=1;                              // TArray.Max
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Marker("[0] missions_nativecall_probe8 (Avenue A + PARAMS: CreateMissionModelFromFinalProgress(1 element)) started\r\n");
    ...
    Marker("[3] hook built; calling CreateMissionModelFromFinalProgress(1 element, ids=None) via direct thunk + compiled-in params...\r\n");
