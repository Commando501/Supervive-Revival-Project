...
// S60 match-setup phase: BasicTraining (?game=TutorialMode) loads+Login-OK but the tutorial systems find NO
// game state and there's NO PlayerStart (blown-out spectator view). Try the FULL tutorial mode
// BP_LokiGameMode_Tutorial, which carries the tutorial match machinery (Comp_BP_BotSpawner, DropPlane_Tutorial,
// DeathCircle, TutorialObjectives PC) + presumably a LokiGameState the spawners can find + the drop-in spawn.
static const wchar_t* kCommand = L"open LVL_Tutorial?game=/Game/Loki/Core/GameModes/BP_LokiGameMode_Tutorial.BP_LokiGameMode_Tutorial_C";
...
extern "C" void OnPI(void* /*ctx*/, void* frame, void*){
    ...
    if(g_offCmd!=0xFFFFFFFF) SetFStringAt(pb,g_offCmd,kCommand);               // Command FString
...
static DWORD WINAPI Worker(LPVOID){
    ...
    Markerf("[2] worldCtx=0x%llX kslCDO=0x%llX eccThunk=0x%llX(rva 0x%llX) child=0x%llX offWCO=0x%X offCmd=0x%X offSP=0x%X gameTid=%lu cmd='open LVL_Tutorial'\r\n",(unsigned long long)g_worldCtx,(unsigned long long)g_kslCDO,(unsigned long long)g_eccThunk,(unsigned long long)(g_eccThunk-g_modBase),(unsigned long long)g_eccChild,g_offWCO,g_offCmd,g_offSP,g_gameTid);
    ...
    UninstallHook();
    if(g_done) Markerf("[4] CALLED ExecuteConsoleCommand (called=%ld hitsGT=%ld). If the map travels, the tutorial loads. Watch Loki.log for LoadMap/Browse LVL_Tutorial.\r\n",(long)g_called,(long)g_hitsGT);
